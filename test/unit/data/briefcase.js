...
describe('.csv.zip briefcase output @slow', () => {
  [false, true].forEach(s3enabled => {
    // REVIEW s3 enabled/disabled doesn't make any difference to these tests
    describe(`with s3 blob storage ${s3enabled ? 'enabled' : 'disabled'}`, () => {
      const mockS3 = { enabled: s3enabled };

      const callAndParse = (inStream, formXml, xmlFormId, callback) => {
        fieldsFor(formXml).then((fields) => {
          zipStreamToFiles(zipStreamFromParts(streamBriefcaseCsvs(mockS3, inStream, fields, xmlFormId)), callback);
        });
      };
...
`); // eslint-disable-line function-paren-newline
      ...
      it('should split known select many fields into value columns', (done) => {
        ...
        fieldsFor(testData.forms.selectMultiple).then((fields) => {
          zipStreamToFiles(zipStreamFromParts(streamBriefcaseCsvs(mockS3, inStream, fields, 'selectMultiple', { '/q1': [ 'x', 'y', 'z' ], '/g1/q2': [ 'm', 'n' ] })), (err, result) => {
            // eslint-disable-next-line keyword-spacing
            if(err) return done(err);
...
`); // eslint-disable-line function-paren-newline
      ...
      it('should leave out structure names if groupPaths = false', (done) => {
        ...
        fieldsFor(formXml).then((fields) => {
          zipStreamToFiles(zipStreamFromParts(streamBriefcaseCsvs(mockS3, inStream, fields, 'structuredform', undefined, undefined, false, { groupPaths: false })), (err, result) => {
            // eslint-disable-next-line keyword-spacing
            if(err) return done(err);
...
`); // eslint-disable-line function-paren-newline
      ...
      it('should split known select many fields And omit group names if asked', (done) => {
        ...
        fieldsFor(testData.forms.selectMultiple).then((fields) => {
          zipStreamToFiles(zipStreamFromParts(streamBriefcaseCsvs(mockS3, inStream, fields, 'selectMultiple', { '/q1': [ 'x', 'y', 'z' ], '/g1/q2': [ 'm', 'n' ] }, undefined, false, { groupPaths: false })), (err, result) => {
            // eslint-disable-next-line keyword-spacing
            if(err) return done(err);
