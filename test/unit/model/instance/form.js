  ...
  describe('fromXml', () => {
    ...
    it('should pick up additional form metadata', () => {
      ...
      return Form.fromXml(xml).then((form) => {
        ...
        form.def.version.should.equal('1.0');
        form.def.hash.should.equal('5ba55d383e978f07ee906fc62ff1b288');
        form.def.sha.should.equal('89a2f70b74c690a128afce777a7c4b63d737e9be');
        form.def.sha256.should.equal('103c7a532de07f6a429a55f90949bb010297562c3731feea1131b54b9088c221');
      });
    });
    ...
    it('should squash null version to empty-string', () => {
      const xml = '<html><head><model><instance><data id="mycoolform"><field/></data></instance></model></head></html>';
      return Form.fromXml(xml).then((form) => {
        (form.def.version === '').should.equal(true);
      });
    });
  });
});
