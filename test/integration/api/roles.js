...
describe('api: /roles', () => {
  describe('GET', () => {
    it('should prohibit anonymous users from listing roles', testService((service) =>
      service.get('/v1/roles').expect(403)));

    it('should prohibit unprivileged users from listing roles', testService((service) =>
      service.login('chelsea', (asChelsea) =>
        asChelsea.get('/v1/roles').expect(403))));

    it('should return all roles', testService((service) =>
      service.login('alice', (asAlice) =>
        asAlice.get('/v1/roles')
          .expect(200)
          .then(({ body }) => {
  ...
  describe('/:id GET', () => {
    it('should prohibit anonymous users from getting roles', testService((service) =>
      service.get('/v1/roles/admin').expect(403)));

    it('should prohibit unprivileged users from getting roles', testService((service) =>
      service.login('chelsea', (asChelsea) =>
        asChelsea.get('/v1/roles/admin').expect(403))));

    it('should notfound a role by system id', testService((service) =>
      service.login('alice', (asAlice) =>
        asAlice.get('/v1/roles/aoeu').expect(404))));

    it('should retrieve a role by system id', testService((service) =>
      service.login('alice', (asAlice) =>
        asAlice.get('/v1/roles/admin')
          .expect(200)
          .then(({ body }) => {
