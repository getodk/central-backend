...
describe('form forward versioning', () => {
  ...
  it('should create a new def and update the version', testService((_, { Project, Form, FormDef }) =>
    Promise.all([
  ...
  it('should set an identity transformation', testService((_, { db, Project, Form, FormDef }) =>
    Promise.all([
      FormDef.parseXml(newXml).then((data) => FormDef.fromData(data)),
      Project.getById(1).then(force)
        .then((project) => project.getFormByXmlFormId('simple')).then(force)
  ...
  it('should preserve submissions', testService((service, { Project, Blob, Form, FormDef, FormAttachment }) =>
  ...
  it('should copy forward matching attachments', testService((_, { Project, Blob, Form, FormDef, FormAttachment }) =>
    Promise.all([
      Project.getById(1).then(force),
      ...
      .then(([ project, form, blob ]) => form.with({ projectId: project.id }).create()
        .then((savedForm) => Promise.all([ 'goodone.csv', 'goodtwo.mp3' ]
          ...
          .then((secondForm) => secondForm.def)
  ...
  it('should not copy forward nonmatching attachments', testService((_, { Project, Blob, Form, FormDef, FormAttachment }) =>
    Promise.all([
      Project.getById(1).then(force),
      Form.fromXml(testData.forms.withAttachments),
      Blob.fromFile(__filename).then((blob) => blob.create())
