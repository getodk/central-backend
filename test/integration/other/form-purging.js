...
describe('query module form purge', () => {
  ...
  it('should log the purge action in the audit log', testService((service, container) =>
    service.login('alice', (asAlice) =>
      container.Forms.getByProjectAndXmlFormId(1, 'simple', false, Form.NoDefRequired).then((o) => o.get()) // get the form before we delete it
  ...
  it('should log purge action in the audit log for each form', testService(async (service, container) => {
    ...
    const simpleForm = await container.Forms.getByProjectAndXmlFormId(1, 'simple', false, Form.NoDefRequired).then((o) => o.get());
    const repeatForm = await container.Forms.getByProjectAndXmlFormId(1, 'withrepeat', false, Form.NoDefRequired).then((o) => o.get());
    ...
    service.login('alice', (asAlice) =>
      asAlice.post('/v1/projects/1/forms')
        ...
        .then(() => asAlice.post('/v1/projects/1/forms/withAttachments/draft/publish')
          .expect(200))
        .then(() => container.Forms.getByProjectAndXmlFormId(1, 'withAttachments', false, Form.NoDefRequired).then((o) => o.get()))
