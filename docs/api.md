...
To use Field Key Authentication, first obtain a Field Key, typically by using the configuration panel in the user interface, or else by using the [Field Key API Resource(/reference/accounts-and-users/field-keys). Once you have the token, you can apply it to any eligible action by prefixing the URL with `/key/{fieldKey}` as follows:
