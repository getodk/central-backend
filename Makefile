...
run-docker-postgres: stop-docker-postgres
	docker start odk-postgres || (docker run -d --name odk-postgres -p 5432:5432 -e POSTGRES_PASSWORD=odktest postgres:14 && sleep 5 && node lib/bin/create-docker-databases.js)
