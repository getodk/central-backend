...
run-docker-postgres: stop-docker-postgres
	docker start odk-postgres15 || (docker run -d --name odk-postgres15 --network host -p 5432:5432 -e POSTGRES_PASSWORD=odktest postgres:14.10-alpine postgres -c log_statement=all -c log_destination=stderr && sleep 5 && node lib/bin/create-docker-databases.js)
