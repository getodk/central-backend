...
async function uploadBlobIfAvailable(container) {
  ...
  try {
    ...
    try {
      ...
      blob = rows[0].blob;
      log('Found blob?:', blob);
      log('blob.content.type:', typeof blob.content);

      const objectName = s3.objectNameForBlob(blob);
      log('Blob s3 object name:', objectName);

      const stream = new require('stream').Readable();
      //require('fs').writeFileSync('blob-dump.base64', blob.content);
      //const buf = Buffer.from(blob.content, 'base64');
      //require('fs').writeFileSync('blob-dump.buffer', buf);
      const buf = Buffer.from(blob.content.substring(2), 'hex');
      stream.push(buf);
      stream.push(null);
