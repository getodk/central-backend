...
async function uploadBlobIfAvailable(container) {
  ...
  try {
    ...
    try {
      ...
      log('Uploading blob...');
      await s3.uploadFromStream(stream, objectName, blob.contentType);
      log('Blob upload successful!');

      const newStatus = 'uploaded';
      log('Updating blob status to:', newStatus);
      return container.db.run(sql`
        UPDATE blobs
    ...
    const newStatus = 'upload_failed';
    log('Updating blob status to:', newStatus);
    return container.db.run(sql`UPDATE blobs SET s3_status=${newStatus} WHERE id=${blob.id}`);
  }
}
