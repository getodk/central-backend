...
const runner = (container, jobMap) => {
  ...
  const log = (...args) => console.error('worker.runner.<anonymous>()', ...args);
  const { run } = container;
  const report = reporter(container.Sentry);
  return (event, reschedule) => {
    if (event == null) {
      // eslint-disable-next-line no-use-before-define
      if (s3.isEnabled()) uploadBlobIfAvailable(container);

      return false; // seems wrong, but if we don't then all the tests break
    }
...
module.exports = { runner, checker, worker, exhaust, exhaustBlobs };


async function uploadBlobIfAvailable(container) {
  // eslint-disable-next-line no-console
  const logFor = level => (...args) => console.error(`[${level}] uploadBlobIfAvailable()`, ...args);
    ...
    const res = await container.db.query(sql`
      ...
      SELECT JSON_BUILD_OBJECT(
        ...
        'filename', COALESCE(
          s_a.name,
          f_a.name,
          f_d.name || CASE WHEN blob."contentType"='application/vnd.ms-excel' THEN '.xls' ELSE '.xlsx' END
        )
      ) AS blob
        FROM blob
        LEFT JOIN submission_attachments AS s_a ON s_a."blobId"   = blob.id
        LEFT JOIN form_attachments       AS f_a ON f_a."blobId"   = blob.id
        LEFT JOIN form_defs              AS f_d ON f_d."xlsBlobId"= blob.id
    `);
    //const res = await container.db.query(sql`
    //  UPDATE blobs
    //    SET s3_status='in_progress'
