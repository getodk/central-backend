...
const runner = (container, jobMap) => {
  ...
  return (event, reschedule) => {
    ...
    process.stdout.write(`start processing event ${event.action} (${jobs.length} jobs)\n`);
    container.transacting(
      ...
      .then(() => { process.stdout.write(`finish processing event ${event.action}\n`); })
      .catch((err) => {
...
// for testing: chews through the event queue until there is nothing left to process.
const exhaust = async (container, jobMap = defaultJobMap) => {
  const check = checker(container);
  const run = runner(container, jobMap);
  while (await check().then((event) => run(event, noop))); // eslint-disable-line no-await-in-loop
};
