...
const runner = (container, jobMap) => {
  const log = (...args) => console.log('worker.runner.<anonymous>()', ...args);
  const { run } = container;
  ...
  return (event, reschedule) => {
    ...
    if (event == null) {
      ...
      // TODO if uploading return true
      // TODO if not uploading return false
      return true;
    }
...
async function uploadBlobIfAvailable(container) {
  const logFor = level => (...args) => console.log(`[${level}] uploadBlobIfAvailable()`, ...args);
  ...
  try {
    ...
    const { rows } = await container.db.query(sql`
      UPDATE blobs
        ...
        RETURNING (id, "contentType", md5, sha)
    `);
    ...
    try {
      const blobFilename = filenameForBlob(blob);
      log('Blob filename:', blobFilename);

      const stream = await container.db.stream(sql`SELECT content FROM blobs WHERE id=${blob.id}`);
      log('Created stream from db:', stream);

      log('Uploading blob...');
      await s3.uploadFromStream(stream, blobFilename, blob.contentType);
      log('Blob upload successful!');

      await updateBlobStatus(blob, 'uploaded');
    } catch(err) {
      log.err('Caught error uploading blob:', err);
      await updateBlobStatus(blob, 'upload_failed');
    }
  } catch(err) {
...
function updateBlobStatus({ id }, newStatus) {
  log('Updating blob status to:', newStatus);
  return container.db.run(sql`UPDATE blobs SET s3_status='upload_failed' WHERE id=${id}`);
}
