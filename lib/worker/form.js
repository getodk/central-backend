...
const pushDraftToEnketo = ({ Forms }, event) =>
  Forms.getByActeeIdForUpdate(event.acteeId, Form.DraftVersion)
...
const pushFormToEnketo = ({ Forms }, event) =>
  Forms.getByActeeIdForUpdate(event.acteeId, Form.NoDefRequired)
    .then((maybeForm) => maybeForm.map((form) => {
      ...
      console.log('[worker/forms] Pushing form to enketo...'); // eslint-disable-line no-console
      return Forms.pushFormToEnketo(form)
        .then((enketoIds) => console.log('[worker/forms] updating form...') || Forms.update(form, new Form(enketoIds))) // eslint-disable-line no-console
        .then(res => console.log('[worker/forms] updated.') || res); // eslint-disable-line no-console
    }).orNull());
...
const updateEntitiesVersion = async ({ Forms }, event) => {
  const { projectId, xmlFormId } = await Forms.getByActeeIdForUpdate(event.acteeId, Form.NoDefRequired).then(o => o.get());
