...
const { submissionToOData } = require('../data/odata');
...
const filterFields = (formFields, select, table) => {
  ...
  const fieldMap = formFields.reduce((map, field) => ({ ...map, [field.path.toLowerCase()]: field }), {});
  for (const property of select.split(',').map(p => p.trim().toLowerCase())) {
...
const selectFields = (query, table) => (fields) => (query.$select ? resolve(filterFields(fields, query.$select, table)) : resolve(fields));
