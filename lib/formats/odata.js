  ...
  const parserStream = new Transform({
    ...
    transform(row, _, done) {
      ...
      submissionToOData(fields, table, row, options).then((data) => {
        ...
        const parentIdProperty = Object.keys(data[0] ?? true).filter(p => /^__.*-id$/.test(p))[0];

        for (const field of data) {

          // if ID (of subtable) is not requested then remove it
          const fieldRefined = (options.metadata && !options.metadata.__id) ? (({ [parentIdProperty]: __, ...item }) => item)(field) : field;
...
const singleRowToOData = (fields, row, domain, originalUrl, query) => {
  ...
  return submissionToOData(fields, table, row, options).then((subrows) => {
    ...
    // if ID (of subtable) is not requested then remove it
    const paredRefined = (options.metadata && !options.metadata.__id) ? pared.map(({ [filterField]: __, ...item }) => item) : pared;
