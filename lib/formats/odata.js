...
const filterFields = (formFields, select, table) => {
  ...
  const fieldMap = formFields.reduce((map, field) => ({ ...map, [`${field.path.replace(/[^/]*$/, '')}${sanitizeOdataIdentifier(field.name)}`]: field }), {});
  for (const property of select.split(',').map(p => p.trim())) {
    ...
    if (property.startsWith('__')) {
      if (!(property === '__id' || (table !== 'Submissions' && property === `__${table.replace(/^(.*)\.[^.]+\.[^.]+$/, '$1').replace(/\./g, '-')}-id`) || (table === 'Submissions' && systemFields.has(property))))
      ...
      if (table !== 'Submissions') {
        for (const tableSegment of table.replace(/Submissions\./, '').split('.')) {
          path += `/${tableSegment}`;
          filteredFields.add(fieldMap[path]); // fieldMap is guaranteed to have structure field
        }
      }
