...
const formListTemplate = template(200, `<?xml version="1.0" encoding="UTF-8"?>
  ...
  {{#forms}}
    <xform>
      ...
      <downloadUrl>{{{domain}}}{{{basePath}}}/forms/{{urlSafeXmlFormId}}{{#draft}}/draft{{/draft}}{{^draft}}/versions/{{urlSafeVersion}}{{/draft}}.xml</downloadUrl>
      {{#aux.openRosa.hasAttachments}}
      <manifestUrl>{{{domain}}}{{{basePath}}}/forms/{{urlSafeXmlFormId}}{{#draft}}/draft{{/draft}}{{^draft}}/versions/{{urlSafeVersion}}{{/draft}}/manifest</manifestUrl>
      {{/aux.openRosa.hasAttachments}}
    </xform>
  {{/forms}}
  </xforms>`);
const formList = (data) => formListTemplate(mergeRight(data, {
  forms: data.forms.map((form) => form.with({
    urlSafeXmlFormId: encodeURIComponent(form.xmlFormId),
    urlSafeVersion: encodeURIComponent(form.def.version || Form.EmptyVersion)
  }))
}));
