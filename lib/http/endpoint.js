...
// just break this logic out for readability:
const isWriteRequest = (request) => (request.method === 'POST') || (request.method === 'PUT') ||
  (request.method === 'PATCH') || (request.method === 'DELETE');

// wrapper around finalize that starts a transaction if necessary, and sets up
// the finalizer as appropriate.
const initiateAndFinalize = (resource, success, failure, request, response) => {
  const finalizer = finalize(success, failure, request, response);

  // automatically start a transaction if write request:
  if (isWriteRequest(request) && (request.container._alreadyTransacting !== true))
    return request.container.db.transaction((trxn) =>
      finalizer(resource(merge(request.container, { db: trxn, _alreadyTransacting: true }), request, response)));

  // otherwise just run it.
  return finalizer(resource(request.container, request, response));
};
...
const endpoint = (f) => (request, response, next) => {
  ...
  initiateAndFinalize(f, success, next, request, response);
};
