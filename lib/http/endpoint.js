...
const isWriteRequest = (request) => (request.method === 'POST') || (request.method === 'PUT') ||
...
// naming this finalize2 for now since other parts of the codebase currently
// (over)rely on finalize itself, and it can't change from its weird semantics.
const finalize2 = (rawResult, request, response) => {
  // first flatten empty returns into an internal developer error.
  const result = Option.of(rawResult).orElse(Problem.internal.emptyResponse());

  if (typeof result === 'function')
    return resolve(result(request, response)).then(finalize2);
...
//
// the resource result is fed through finalize2 for refinement (see comments above),
...
const endpointBase = ({ preprocessor = noop, before = noop, resultWriter, errorWriter }) => (container, preprocessors = []) => (resource) => (request, response) => {
  ...
  return maybeTransact((localContainer) =>
    ...
    reduce(
      ...
      .then((result) => finalize2(result, request, response))
...
const defaultResultWriter = (result, request, response) => {
...
module.exports = {
  ...
  builder, finalize, defaultErrorWriter,
