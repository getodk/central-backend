...
const endpointBase = ({ preprocessor = noop, before = noop, resultWriter, errorWriter }) => (container, preprocessors = []) => (resource) => (request, response, next) => {
  ...
  //
    .catch((err) => {
      ...
      else if (err?.code !== 'ERR_REQUEST_ABORTED') {
...
const defaultErrorWriter = (error, request, response) => {
  ...
  if (response.headersSent) {
    if (response.getHeader('Content-Type')?.startsWith('text/event-stream') && !response.writableEnded) {
      const event = error?.isProblem === true
        ? { message: error.message, code: error.problemCode, details: error.problemDetails }
