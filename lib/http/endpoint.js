...
const { queryFuncs } = require('../util/db');
...
const endpointBase = ({ preprocessor = noop, before = noop, resultWriter, errorWriter }) => (container, preprocessors = []) => (resource) => (request, response, next) => {
  ...
  const maybeTransact = isWriteRequest(request)
    ? (cb) => container.pool.connect((cxn) => cxn.transaction((trxn) => container.with(Object.assign({ q: trxn }, queryFuncs(trxn))).transacting(cb)))
    : (cb) => cb(container.with(Object.assign({ q: container.pool }, queryFuncs(container.pool))));
