...
const defaultResultWriter = (result, request, response, next) => {
  ...
  if (result instanceof PartialPipe) {
    const errHandler = streamErrorHandler(response, next);
    result.streams[result.streams.length-1].on('error', errHandler);
    result.with(response).pipeline(() => {});
  } else if (result.pipe != null) {
    const errHandler = streamErrorHandler(response, next);
    result.on('error', errHandler);
