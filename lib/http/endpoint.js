...
const defaultResultWriter = (result, request, response) => {
  ...
  if (result instanceof PartialPipe) {
    const piped = result.with(response).pipeline(streamErrorHandler(response));
    request.on('close', () => { piped.end(); });
  } else if (result.pipe != null) {
    // automatically stop a database query if the request is aborted:
    if (typeof result.end === 'function') request.on('close', () => { result.end(); });
    pipeline(result, response, streamErrorHandler(response));
  } else {
...
const defaultErrorWriter = (error, request, response) => {
  if (error.isProblem === true) {
    if ((error.httpCode === 403) && (request.get('user-agent') != null) && (request.get('user-agent').match(/tableau/i))) {
