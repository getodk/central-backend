...
const authHandler = ({ Sessions, Users, Auth }, context) => {
  ...
  if (context.fieldKey.isDefined()) {
    ...
    // '$' & '!' are supported for legacy output of crypto.generateToken()
    // Allow escaping of regex chars to make intention of `-` char more explicit.
    // See: https://github.com/eslint/eslint/discussions/19072
    // eslint-disable-next-line no-useless-escape
    if (!/^[a-z0-9!$_\-]{64}$/i.test(key)) return reject(Problem.user.authenticationFailed());
