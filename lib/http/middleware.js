...
const sessionParser = ({ Session, User, Auth }) => (request, response, rawNext) => {
  ...
  const authBySessionToken = (token, onFailure) => Session.getByBearerToken(token)
    .then((session) => {
...
const fieldKeyParser = ({ Session, Auth }) => (request, response, next) => {
  ...
  Session.getByBearerToken(match[1])
    .then(getOrReject(Problem.user.authenticationFailed()))
    .then((session) => {
...
const headerOptionsParser = (request, response, next) => {
  ...
  if (request.params.offset != null) options.offset = parseInt(request.params.offset, 10);
  if (request.params.limit != null) options.limit = parseInt(request.params.limit, 10);

  request.queryOptions = new QueryOptions(options);
  next();
};

// drops the given container onto the request object for use by the endpoint wrapper
// to start transactions if necessary.
// TODO: will be removed again at some point in this refactor i think.
const containerInjector = (container) => (request, response, next) => {
  request.container = container;
  next();
};


module.exports = { versionParser, sessionParser, fieldKeyParser, headerOptionsParser, containerInjector };
