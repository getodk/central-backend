...
const simpleTransport = (transport, config, callback) => (to, message, data) =>
  new Promise((resolve, reject) =>
    transport.sendMail(merge({ to, from: config.serviceAccount }, messages[message](data)), (err, info) =>
...
const sendmail = (config) => {
  const transport = nodemailer.createTransport(merge({ "sendmail": true }, config.transportOpts));
  return simpleTransport(transport, config, (err, info, resolve, reject) => {
...
const json = (config) => {
  global.inbox = [];
  const transport = nodemailer.createTransport(merge({ "jsonTransport": true }, config.transportOpts));
  return simpleTransport(transport, config, (err, info, resolve, reject) => {
    if (err != null) return reject(err);
    global.inbox.push(info.message);
    process.stdout.write(`>> Outbound email: ${JSON.stringify(info.message)}\n`);
    return resolve(info);
  });
};
const transports = { sendmail, json };
const mailer = (config) => transports[config.transport](config);
