...
const messages = {
  ...
  // * {{ oldEmail }} the email previously associated with the account
  // * {{ newEmail }} the email now associated with the account
...
const ses = (options) => {
  const { transportOpts } = options;
  const requestOptions = { host: transportOpts.host, path: transportOpts.path, method: 'POST' };
  const transport = {
