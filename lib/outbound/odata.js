...
const jsonDataPreamble = template(`{"@odata.context":"{{{domain}}}{{{formUrl}}}/$metadata#Submissions","value":[`);

const rowStreamToJson = (form, tableUrl, inStream) => {
  ...
  const parserStream = new Transform({
    ...
    transform(row, _, done) {
      // first see if we have to write a header out (TODO: is there a cleverer way?)
      if (wroteHeader === false) {
        this.push(jsonDataPreamble({ form, formUrl }));
        wroteHeader = true;
      }
      ...
      extractFields(schemaLookup, row).then((fields) => {
        if (sequentialId > 1) this.push(',');
        fields.__id = (row.instanceId || sequentialId); // this object is ours alone so just mutate it for perf.
