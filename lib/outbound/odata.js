...
const { flattenSchemaStructures, schemaAsLookup } = require('../data/schema');
    ...
    {{#entityTypes}}
      ...
      {{#properties}}
        ...
        <NavigationProperty Name="{{name}}" Type="{{type}}">
          <ReferentialConstraint Property="__id" ReferencedProperty="{{fkname}}"/>
        </NavigationProperty>
        {{/navigation}}
        {{^navigation}}
...
const fieldToProperty = (field) => ({ name: field.path.join('__'), type: (typeMap[field.type] || 'Edm.String') });
...
const fieldsToEdmx = (fields, fqdnBase, path = []) => {
  ...
  for (const field of fields) {
    if (Array.isArray(field.children)) {
      const subpath = path.concat(field.path);

      const result = fieldsToEdmx(field.children, fqdnBase, subpath);
      entityTypes.push(...result.entityTypes);

      const fqdnPart = subpath.join('.');
      if (field.type === 'repeat') {
        const parentRepeatId = `__Submissions${path.map((part) => `-${part}`).join('')}-id`;
        ...
        properties.push({ name: field.path.join('__'), type: `Collection(${fqdnBase}.Submissions.${fqdnPart})`, navigation: true, fkname: `__Submissions${subpath.map((part) => `-${part}`).join('')}-id` });
      }
    } else {
  ...
  return { properties, entityTypes };
};

const edmxFor = (form) => {
  const fqdnBase = `${fqdnRoot}.${form.xmlFormId}`;
  const { properties, entityTypes } = fieldsToEdmx(flattenSchemaStructures(form.schema()), fqdnBase);
  entityTypes.unshift({ name: 'Submissions', key: '__id', primary: true, properties });
  return edmxTemplater({ form, entityTypes, fqdnBase });
};
