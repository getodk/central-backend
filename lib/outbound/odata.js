...
const getServiceRoot = (subpath, xmlFormId) => {
  const target = `${xmlFormId}.svc`;
...
const edmxFor = (xmlFormId, fields) => {
  const fqdnBase = `${fqdnRoot}.${xmlFormId}`;
  ...
  for (const field of fields) {
    let out = ptr(stack);
    if (!field.path.startsWith(out.path || '')) {
      stack.pop();
      out = ptr(stack);
    }
...
const rowStreamToOData = (xmlFormId, fields, table, domain, originalUrl, query, inStream, tableCount) => {
  // cache values we'll need repeatedly.
  const serviceRoot = getServiceRoot(originalUrl, xmlFormId);
...
// originalUrl: String is the request URL; we need it as well to formulate response URLs.
// query: Object is the Express Request query object indicating request querystring parameters.
const singleRowToOData = (xmlFormId, fields, row, domain, originalUrl, query) => {
