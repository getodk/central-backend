...
<atom:feed xmlns="http://docs.oasis-open.org/odata/ns/data" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:meta="http://docs.oasis-open.org/odata/ns/metadata" meta:context="{{{domain}}}{{{formUrl}}}/$metadata#Records">
...
const atomDataEntry = template(stripWhitespace(`<atom:entry>
  ...
  <atom:summary/>
  <atom:updated>{{row.createdAt}}</atom:updated>
  ...
  <atom:content type="application/xml">
    <meta:properties>{{{row.xml}}}</meta:properties>
  </atom:content>
</atom:entry>`));
...
const rowStreamToAtom = (form, formUrl, inStream) => {
  ...
  const parserStream = new Transform({
    ...
    transform(row, _, done) {
      ...
      // here we have the whole submission database row.
      this.push(atomDataEntry({ form, formUrl, row }));
