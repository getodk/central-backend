...
const serviceDocumentFor = (xform, domain, serviceUrl) => xform.tables().then((tables) => ({
...
const edmxFor = (form) => form.xform.schema().then((schema) => {
...
const rowStreamToOData = (form, table, domain, originalUrl, query, inStream, tableCount) => form.xform.schema().then((schema) => {
...
// originalUrl: String is the request URL; we need it as well to formulate response URLs.
// query: Object is the Express Request query object indicating request querystring parameters.
const singleRowToOData = (form, submission, domain, originalUrl, query) => form.xform.schema().then((schema) => {
