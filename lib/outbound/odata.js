...
// Given a querystring object, processes the paging information off of it and
// returns { limit: Int, offset: Int, shouldCount: Bool }.
...
const edmxFor = (form) => form.schema().then((schema) => {
  ...
  const { properties, entityTypes, complexTypes } = fieldsToEdmx(schema, fqdnBase);
...
const rowStreamToOData = (form, table, domain, originalUrl, query, inStream) => form.schema().then((schema) => {
  // cache values we'll need repeatedly.
  const serviceRoot = getServiceRoot(originalUrl, form);
  const schemaLookup = schemaAsLookup(schema);
