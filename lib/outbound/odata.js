...
const extractPaging = (query) => {
  ...
  return { limit, offset, shouldCount };
};
const nextUrlFor = (limit, offset, count, originalUrl) =>
  ((Number.isNaN(limit) || (offset + limit >= count))
    ? null
...
const serviceDocumentFor = (form, serviceUrl) => ({
  '@odata.context': `${env.domain}${urlPathname(serviceUrl)}/$metadata`,
...
const rowStreamToOData = (form, table, originalUrl, query, inStream) => {
  ...
  const parserStream = new Transform({
    ...
    transform(row, _, done) {
      ...
      submissionToOData(schemaLookup, table, row, options).then((fields) => {
        for (field of fields) {
          if ((count >= offset) && (Number.isNaN(limit) || (count < (offset + limit)))) {
    ...
    flush(done) {
      ...
      this.push(jsonDataFooter({ form, table, serviceRoot, nextUrl, count: (shouldCount ? count : null) }));
      done();
    }
  });
...
const singleRowToOData = (form, submission, originalUrl, query) => {
  ...
  return submissionToOData(schemaLookup, table, submission, options).then((subrows) => {
    ...
    return '{"value":[' + pared.map(JSON.stringify).join(',') + '],' +
      jsonDataFooter({ form, table, serviceRoot, nextUrl, count: (shouldCount ? count : null) });
  });
};
