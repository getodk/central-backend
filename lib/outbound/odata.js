...
const jsonServiceDocumentFor = (form, formUrl) => ({
  ...
  value: [ 'Submissions' ].concat(form.tables())
...
const edmxFor = (form) => {
  ...
  entityTypes.unshift({ name: 'Submissions', key: '__id', properties });
...
const rowStreamToAtom = (form, tableUrl, inStream) => {
...
const jsonDataPreamble = template(stripWhitespace(`{
  "@odata.context":"{{{domain}}}{{{formUrl}}}/$metadata#Submissions",
  {{#nextUrl}}"@odata.nextLink":"{{{domain}}}{{{nextUrl}}}",{{/nextUrl}}
  "value":[`));

const rowStreamToJson = (form, query, tableUrl, inStream) => {
  // cache values we'll need repeatedly.
  const formUrl = path.resolve(urlPathname(tableUrl), '..');
  const schemaLookup = schemaAsLookup(form.schema());

  const limit = parseInt(query['$top']);
  // TODO: exclude nextLink for final page.
  const nextUrl = Number.isNaN(limit)
    ? null
    : urlWithQueryParams(tableUrl, { '$skip': ((parseInt(query['$skip']) || 0) + limit), '$top': null });
  ...
  const parserStream = new Transform({
    ...
    transform(row, _, done) {
      // first see if we have to write a header out (TODO: is there a cleverer way?)
      if (wroteHeader === false) {
        this.push(jsonDataPreamble({ form, formUrl, nextUrl }));
        wroteHeader = true;
      }

      // do transformation on the xml to strip the outer layers.
      extractFields(schemaLookup, row).then((fields) => {
        // fenceposting.
        if (isFirstRecord === false) this.push(',');
        isFirstRecord = false;

        fields.__id = row.instanceId; // this object is ours alone so just mutate it for perf.
