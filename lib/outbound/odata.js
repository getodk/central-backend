...
const extractPaging = (query) => {
  const limit = parseInt(query['$top']) || Infinity;
...
// of them, path refers to the path to the present level of recursion, whereas
// tablePath refers to the nearest containing repeat instance path; or in odata
// parlance, the closest entitytype.
const fieldsToEdmx = (fields, fqdnBase, path = [], tablePath = []) => {
  const properties = [];
  const entityTypes = [];
  const complexTypes = [];
...
const singleRowToOData = (form, submission, domain, originalUrl, query) => {
  ...
  return submissionToOData(schemaLookup, table, submission, options).then((subrows) => {
    ...
    return '{"value":[' + pared.map(JSON.stringify).join(',') + '],' +
      jsonDataFooter({ form, table, domain, serviceRoot, nextUrl, count: (shouldCount ? count : null) });
  });
};
