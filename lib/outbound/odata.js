...
const rowStreamToOData = (form, table, originalUrl, query, inStream, count) => {
  ...
  const parserStream = new Transform({
    ...
    flush(done) {
      if (wroteHeader === false)
        // we never got any rows to output, so write our preamble right away.
        this.push(jsonDataPreamble({ form, table, serviceRoot, nextUrl, count: (shouldCount ? count : null) }));

      // always close the data object.
      this.push(']}');
      done();
    }
  });
