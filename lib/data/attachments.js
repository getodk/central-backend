    ...
    write(x, _, done) {
      ...
      if (att.s3_status === 'uploaded') {
        s3.getContentFor(att)
          .then(appendContent) // eslint-disable-line no-use-before-define
          .catch(err => {
            console.log(err); // eslint-disable-line no-console
            // TODO remove this before merge.  but understand where the error should be sent; maybe emit()?
            process.exit(124);
          });
      } else if (att.content === null) {
        // TODO should not happen - investigate
        process.exit(123);
      } else {
        appendContent(att.content); // eslint-disable-line no-use-before-define
      }

      function appendContent(rawContent) {
        ...
        // TODO fetch content from s3?  make sure this is tested
