...
const submissionToOData = (fields, table, submission, options = {}) => new Promise((resolve) => {
  ...
  if (!options.metadata || options.metadata.includes('__id')) {
    root.__id = submission.instanceId;
  }
  if (table === 'Submissions') {
    const systemObj = {
      submissiondate: { key: 'submissionDate', value: submission.createdAt },
      updatedat: { key: 'updatedAt', value: submission.updatedAt },
      submitterid: { key: 'submitterId', value: (submitter.id == null) ? null : submitter.id.toString() }, // => string for future-proofing
      submittername: { key: 'submitterName', value: submitter.displayName || null },
      attachmentspresent: { key: 'attachmentsPresent', value: attachment.present || 0 },
      attachmentsexpected: { key: 'attachmentsExpected', value: attachment.expected || 0 },
      status: { key: 'status', value: encrypted ? (encHasData ? 'notDecrypted' : 'missingEncryptedFormData') : null },
      reviewstate: { key: 'reviewState', value: submission.reviewState || null },
      deviceid: { key: 'deviceId', value: submission.deviceId || null },
      edits: { key: 'edits', value: submission.aux.edit.count },
      formversion: { key: 'formVersion', value: submission.aux.exports.formVersion },
    };

    if (options.metadata) {
      const systemProp = options.metadata.filter(p => p.startsWith('__system')).map(p => p.substr(9));
      if (systemProp.length > 0) {
        root.__system = {};
        for (const property of systemProp) {
          root.__system[systemObj[property].key] = systemObj[property].value;
        }
      }
    } else {
      root.__system = {};
