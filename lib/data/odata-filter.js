...
const odataFilter = (expr, odataToColumnMap) => {
  ...
  const methodCall = (node) => {
    ...
    const fnName = node.value.method;
    const params = node.value.parameters;
    if (extractFunctions.includes(fnName)) {
      return sql`extract(${sql.identifier([fnName])} from ${op(params[0])})`; // eslint-disable-line no-use-before-define
    } else if (fnName === 'now') {
      return sql`now()`;
    } else {
