...
const odataOrderBy = (expr, odataToColumnMap, stableOrderColumn = null) => {
  ...
  const clauses = expr.split(',').map((exp) => {
    ...
    if (!odataToColumnMap.has(col))
      throw Problem.internal.unsupportedODataField({ at: 0, text: col });

    // Save the order of the initial property to use for the stable sort column order
    if (initialOrder == null)
      initialOrder = order;

    return sql`${sql.identifier(odataToColumnMap.get(col).split('.'))} ${((order?.toLowerCase() === 'asc') ? sql`ASC` : sql`DESC`)}`;
  });

  if (stableOrderColumn != null)
    clauses.push(sql`${sql.identifier(stableOrderColumn.split('.'))} ${((initialOrder?.toLowerCase() === 'asc') ? sql`ASC` : sql`DESC`)}`);
