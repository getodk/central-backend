...
const streamClientAudits = (s3, inStream, form, decryptor) => {
  ...
  const csvifier = new Transform({
    ...
    transform(x, _, done) {
      ...
      if (data.content != null || data.s3_status === 'uploaded') {
        blobContent(s3, data)
          .then(content => {
            ...
            const decrypted = (data.localKey == null)
              ? content
              : decryptor(content, data.keyId, data.localKey, data.instanceId, data.index);
            return parseClientAudits(decrypted)
              .then((rows) => {
