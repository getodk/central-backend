...
const submissionXmlToFieldStream = (formDef, xml) => formDef.schema().then((schema) => {
  ...
  const parser = new hparser.Parser({
    ...
    onclosetag: () => {
      ...
      if (textBuffer !== '') {
        if ((field != null) && (field.children == null)) // don't output useless whitespace
          // TODO/perf: cloning pathStack is expensive.
          outStream.push({ field, text: textBuffer, path: pathStack.slice() });
        textBuffer = '';
      }
