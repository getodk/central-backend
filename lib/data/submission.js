...
const getEntity = (entityFields, xml) => new Promise((resolve, reject) => {
  const values = {};
  const stream = submissionXmlToFieldStream(entityFields, xml);
  stream.on('error', reject);
  stream.on('data', ({ field, text }) => {
    if (field.path.indexOf('/meta/entity') === 0)
      values[field.name] = text;
    else if (field.propertyName != null)
      values[field.propertyName] = text;
  });
  stream.on('end', () => {
    // TODO: validate that uuid in proper format was provided, else log an error
    if (values.uuid == null)
      values.uuid = uuid();
    resolve(values);
  });
