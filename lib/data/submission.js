  ...
  const parser = new hparser.Parser({
      ...
      if ((structural != null) && (structural !== SchemaStack.Wrapper)) {
        if (structural.type === 'structure') {
          if (entityField != null && (entityField !== SchemaStack.Wrapper)) {
            const { propertyName, datasetId } = entityField.aux;
            lastDatasetId = datasetId;

            if (propertyName === '__entity') {
        ...
        } else if (structural.type === 'repeat') {
          // shove another entity in there?
          if (entities[lastDatasetId]) {
            entities[lastDatasetId].push({ system: Object.create(null), data: Object.create(null) });
            console.log("-- repeat tag", tagName, structural, entityField);
          }
        }
      }
    },
    ...
    onclosetag: (tagName) => {
      ...
      if (structural == null && entityField != null) {
        ...
        if (propertyName === '__label')
          entity.system.label = textBuffer;
        else
          entity.data[propertyName] = textBuffer;

      } else if (structural != null && structural.type === 'repeat') {
        console.log("-- closing tag repeat", tagName);
      }
    }
  }, { xmlMode: true, decodeEntities: true });
