...
const matchFieldsWithDatasets = (datasets, fields) => {
  ...
  for (const field of fields) {
    ...
    if (field.type === 'repeat') {
      ...
      const datasetField = { ...field, propertyName: '__repeat', internal: true };
      resultMap.get(matchedPath).fields.push(datasetField);
      // TODO: consider storing the repeat group field name here on the dataset
      // to eventually store in dataset_form_defs
      // resultMap.get(matchedPath).dataset.repeat = field.name;
    }
  }
...
module.exports = { getDatasets, validateDatasetName, validatePropertyName, prepareFieldsForDataset, matchFieldsWithDatasets };
