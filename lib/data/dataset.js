...
const matchFieldsWithDatasets = (datasets, fields) => {
  ...
  for (const field of fields) {
    ...
    if (exactMatch != null) {
      ...
      // Find the most specific repeat path that is a prefix of the field path
      const matchedRepeatPath = sortedRepeatPaths.find(rPath => (field.path + '/').startsWith(rPath));

      if (matchedPath != null) {
        if (matchedRepeatPath != null && matchedRepeatPath.length > matchedPath.length) {
