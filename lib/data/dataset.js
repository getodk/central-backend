...
const matchFieldsWithDatasets = (datasets, fields) => {
  ...
  // Create a map from dataset path to { dataset, fields: [], testFields: [] }
  // testFields are only used in unit tests. In application code, we only save the
  // fields with propertyNames and then look up entity, label, repeat fields by path.
  // In the tests, we don't have that ability to do the lookup, so we construct that
  // list of fields here.
  const resultMap = new Map(datasets.map(dataset => [dataset.path, { dataset, fields: [], testFields: [] }]));
  ...
  for (const field of fields) {
    if (field.type === 'repeat') {
      const matchedPath = sortedPaths.find(dsPath => field.path === dsPath);
      resultMap.get(matchedPath).dataset.isRepeat = true;

      // Save this field as a test field
      resultMap.get(matchedPath).testFields.push(field);
    } else {
