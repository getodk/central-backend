...
*/
const validateEntitySpecVersion = (maybeVersion) => {
  if (maybeVersion.isEmpty())
    throw Problem.user.invalidEntityForm({ reason: 'Entities specification version is missing.' });

  const version = maybeVersion.get();
  if (!semverSatisfies(version, '2022.1.0 - 2025.1.x'))
    throw Problem.user.invalidEntityForm({ reason: `Entities specification version [${version}] is not supported.` });

  const warnings = semverSatisfies(version, '>=2024.1.x')
    ? null
    : [{
      type: 'oldEntityVersion',
      details: { version },
      reason: `Entities specification version [${version}] is not compatible with Offline Entities. Please use version 2024.1.0 or later.`
    }];
...
Must be 2025.1 or higher if there are multiple dataset declarations OR at least one dataset declaration is a repeat.
*/
const validateEntityRepeatVersion = (version, datasets) => {
  const repeatDataset = datasets.some(d => d.dataset.isRepeat);
  if ((repeatDataset || datasets.length > 1) && semverSatisfies(version, '<2025.1.x'))
    throw Problem.user.invalidEntityForm({ reason: `Entities specification version [${version}] is not compatible with entities from repeats or multiple entity lists.  Please use version 2025.1.0 or later.` });
};
...
const getDatasets = async (xml) => {
  ...
  const [maybeVersion, entityBlocks] = await traverseXml(xml, [
  ...
  const { warnings, version } = validateEntitySpecVersion(maybeVersion);
  ...
  return Option.of({ datasets, warnings, version });
};
...
const matchFieldsWithDatasets = (datasets, fields) => {
  ...
  const datasetMap = new Map(datasets.map(dataset => [dataset.path, { dataset, fields: [] }]));

  // Helper functions sort sorting and searching throuhg path lists
  const _byDescendingPathLength = (a, b) => b.split('/').length - a.split('/').length;
  const _findClosestPath = (sortedPaths, fieldPath) =>
    sortedPaths.find(p => (fieldPath + '/').startsWith(p));

  // Sort dataset paths by length descending for specificity
  const sortedDatasetPaths = [...datasets.map(ds => ds.path)].sort(_byDescendingPathLength);

  // While matching up forms and datasets, this function also checks for invalid form definitions
  // where an entity property is in a repeat but the dataset is outside of the repeat.
  const sortedRepeatPaths = fields
    .filter(f => f.type === 'repeat')
    .map(f => f.path)
    .sort(_byDescendingPathLength);

  for (const field of fields) {
    const exactMatch = sortedDatasetPaths.find(dsPath => field.path + '/' === dsPath);
    if (exactMatch != null) {
      if (field.type === 'repeat')
        datasetMap.get(exactMatch).dataset.isRepeat = true;
    } else if (field.propertyName != null) {
      const matchedDatasetPath = _findClosestPath(sortedDatasetPaths, field.path);
      const matchedRepeatPath = _findClosestPath(sortedRepeatPaths, field.path);

      if (matchedDatasetPath != null) {
        if (matchedRepeatPath != null && matchedRepeatPath.length > matchedDatasetPath.length) {
          // Invalid: property is in a repeat group but that repeat group is outside the closest dataset
          throw Problem.user.invalidEntityForm({ reason: 'Cannot save properties from a repeat on an entity outside of a repeat.' });
        } else {
          datasetMap.get(matchedDatasetPath).fields.push(field);
        }
      }
    }
  }
...
module.exports = { getDatasets, matchFieldsWithDatasets, validateDatasetName, validatePropertyName, validateEntityRepeatVersion };
