...
const parseSubmissionXml = (entityFields, xml) => new Promise((resolve, reject) => {
  ...
  stream.on('end', () => {
    ...
    console.log('entity parts:', dataset, label, id);
    if (!dataset || dataset.trim() === '')
      reject(Problem.user.entityViolation({ reason: 'Dataset empty or missing.' }));
    if (!label || label.trim() === '')
      reject(Problem.user.entityViolation({ reason: 'Label empty or missing.' }));
    if (!id || id.trim() === '')
      reject(Problem.user.entityViolation({ reason: 'ID empty or missing.' }));
    const uuidPattern = /^(uuid:)?([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
    const matches = uuidPattern.exec(id);
    if (matches == null) reject(Problem.user.entityViolation({ reason: `ID [${id}] not a valid UUID.` }));
    else {
      // eslint-disable-next-line prefer-destructuring
      entity.system.uuid = matches[2];
      delete entity.system.id;
      resolve(entity);
    }
  });
});
