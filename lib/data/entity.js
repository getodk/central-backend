...
const validateEntity = (entity) => {
  ...
  if (!id || id.trim() === '')
    throw Problem.user.missingParameter({ field: 'ID' });

  const uuidPattern = /^(uuid:)?([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
  const matches = uuidPattern.exec(id);

  if (matches == null) throw Problem.user.invalidDataTypeOfParameter({ value: 'id', expected: 'valid UUID' });
...
const extractEntity = (body, propertyNames, existingEntity) => {
  ...
  if (unexpectedKeys.length > 0)
    throw Problem.user.unexpectedAttributes({ fields: unexpectedKeys });
  ...
  if (existingEntity) {
    ...
    if (body.uuid && typeof body.uuid !== 'string')
      throw Problem.user.invalidDataTypeOfParameter({ value: 'uuid', expected: 'string' });
    entity.system.id = body.uuid;
  }

  if (body.label != null)
    if (typeof body.label !== 'string')
      throw Problem.user.invalidDataTypeOfParameter({ value: 'label', expected: 'string' });
    else
  ...
  if (body.data != null)
    for (const [k, v] of Object.entries(body.data)) {
      if (typeof v !== 'string')
        throw Problem.user.invalidDataTypeOfParameter({ value: k, expected: 'string' });
