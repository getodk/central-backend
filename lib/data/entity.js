...
// Paging is done at the database level
  ...
  const rootStream = new Transform({
    ...
    transform(entity, _, done) {
      try {
        ...
        const { uuid, ...values } = entity;
        this.push(JSON.stringify(mergeLeft({ name: uuid }, values, schema)));
...
module.exports = { parseSubmissionXml, validateEntity, streamEntityCsvs, streamEntityOdata };
