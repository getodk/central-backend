  ...
  const parser = new hparser.Parser({
    onopentag: (name) => {
      ...
      if (field == null) {
        ...
        const subrow = [];
        subrow[field.meta.parentKey] = [ instanceId ].concat(schemaStack.parentIterationPath()).join('/');
        subrow[field.meta.key] = [ instanceId ].concat(schemaStack.iterationPath()).join('/');
...
const streamBriefcaseCsvs = (inStream, inFields, xmlFormId, decryptor) => {
...

  {
    ...
    for (let idx = 0; idx < inFields.length; idx += 1) {
      ...
      if (field.type === 'repeat') {
        ...
        const basename = field.path.slice(ptr(tableStack).path.length + 1).replace(/\//g, '-');
        if (field.type === 'geopoint') {
      ...
      while ((nextField == null)
        ? (ptr(tableStack).type === 'repeat') // if we are at the end pop all
  ...
  const rootStream = new Transform({
    ...
    transform(row, _, done) {
      try {
        ...
        processRow(xml, submission.instanceId, fields).then((result) => {
