...
const streamBriefcaseCsvs = (s3, inStream, inFields, xmlFormId, selectValues, decryptor, rootOnly = false, { groupPaths = true } = {}) => {
    ...
    transform(submission, _, done) {
      try {
        ...
        if (submission.def.localKey == null) processXml(submission.xml);
          ...
          blobContent(s3, { s3_status: encryption.encS3Status, sha: encryption.encSha, md5: encryption.encMd5, content: encryption.encData })
            .then(decryptXml)
            .then(processXml)
            .catch(done);
        }
