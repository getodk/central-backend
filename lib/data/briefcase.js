...
const streamBriefcaseCsvs = (inStream, inFields, xmlFormId, selectValues, decryptor, rootOnly = false, { groupPaths = true } = {}) => {
...

  {
    ...
    for (let idx = 0; idx < inFields.length; idx += 1) {
      ...
      if (field.type === 'repeat') {
        if (rootOnly === true) {
          // skip the children as well for the rootOnly. To determine if a field is a child of
          // current repeat is to match the prefix of its path
          while (idx + 1 < inFields.length && inFields[idx + 1].path.startsWith(field.path + '/')) {
            idx += 1;
          }
          continue;
        }
