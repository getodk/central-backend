...
const writeMetadata = (row, field, submission, submitter, status) => {
  ...
  if (submitter != null) {
    row[field.children.SubmitterID.idx] = submitter.id;
    row[field.children.SubmitterName.idx] = submitter.displayName;
  }
  if (status != null)
    row[field.children.Status.idx] = status;
};
...
const processRow = (xml, { submission, submitter }, lookup) => new Promise((resolve) => {
  ...
  const status =
    (xml === 'missing') ? 'missing encrypted form data' : // eslint-disable-line
    (xml === null) ? 'not decrypted' : null; // eslint-disable-line
  if (status != null) {
    const result = [];
    writeMetadata(result, fieldStack[0], submission, submitter, status);
  ...
  const parser = new hparser.Parser({
    ...
    onclosetag: () => {
      ...
      if (fieldStack.length === 0) {
        parser.reset();
        writeMetadata(row, field, submission, submitter);
        resolve(row);
      }
    }
  }, { xmlMode: true, decodeEntities: true });

  if (typeof xml.pipe === 'function') xml.pipe(parser);
  else parser.write(xml);
});
...
const streamBriefcaseCsvs = (inStream, form, decryptor) => form.def.schema().then((inSchema) => {
  ...
  const rootStream = new Transform({
    ...
    transform(row, _, done) {
      ...
      const xml =
        (row.localKey == null) ? row.xml : // eslint-disable-line
        (row.encHasData === false) ? 'missing' : // eslint-disable-line
        decryptor(row.encData, row.encKeyId, row.localKey, row.submission.instanceId, row.encIndex); // eslint-disable-line
