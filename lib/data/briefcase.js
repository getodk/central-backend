  ...
  const rootStream = new Transform({
    ...
    transform(submission, _, done) {
      try {
        ...
        console.log('briefcase.transform()', 'encryption:', encryption, JSON.stringify(encryption));
        ...
        function decryptXml(data) {
          return decryptor(data, encryption.encKeyId, submission.def.localKey, submission.instanceId, encryption.encIndex); // eslint-disable-line indent
        }
