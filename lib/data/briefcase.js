...
const { getSubmissionKey, getSubmissionIv, streamSubmissionCleartext } = require('../util/crypto');
...
const streamBriefcaseCsvs = (inStream, form, privs = {}) => form.def.schema().then((inSchema) => {
  ...
  const rootStream = new Transform({
    ...
    transform(row, _, done) {
      ...
      let { xml } = row;
      if (row.encData != null) {
        const priv = privs[row.encKeyId];
        if (priv == null) {
          xml = '<data/>'; // feed the xml parser dummy data. TODO: just abort out?
        } else {
          const aeskey = getSubmissionKey(priv, Buffer.from(row.localKey, 'base64'));
          const iv = getSubmissionIv(row.submission.instanceId, aeskey, row.encIndex);
          xml = streamSubmissionCleartext(aeskey, iv, row.encData);
        }
      }
