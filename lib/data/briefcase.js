  ...
  const parser = new hparser.Parser({
    ...
    onclosetag: () => {
      ...
      if (fieldStack.length === 0) {
        parser.reset();
        resolve(writeMetadata(row, field, submission, submitter));
      }
    }
...
const streamBriefcaseCsvs = (inStream, form, decryptor) => form.def.schema().then((inSchema) => {
  ...
  const rootStream = new Transform({
    ...
    transform(row, _, done) {
      ...
      let { xml } = row;
      if (row.encData != null) {
        xml = decryptor(row.encData, row.encKeyId, row.localKey, row.submission.instanceId, row.encIndex);

        // if the decryptor failed to work, it returns null. in that case feed the
        // xml parser dummy data. TODO: faster shortcut out?
