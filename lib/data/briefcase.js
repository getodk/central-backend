...
const addCentralColumns = (schema) => {
  ...
  schema.push({ name: 'AttachmentsPresent', type: 'special:attachmentsPresent' });
  schema.push({ name: 'AttachmentsExpected', type: 'special:attachmentsExpected' });
  schema.push({ name: 'Status', type: 'special:status' });
};
...
const writeMetadata = (out, field, submission, submitter, attachments, status) => {
  out[field.children.SubmissionDate.idx] = (new Date(submission.createdAt)).toISOString();
  out[field.children.KEY.idx] = submission.instanceId;
  if (submitter != null) {
    out[field.children.SubmitterID.idx] = submitter.id;
    out[field.children.SubmitterName.idx] = submitter.displayName;
  }
  out[field.children.AttachmentsPresent.idx] = attachments.present || 0;
  out[field.children.AttachmentsExpected.idx] = attachments.expected || 0;
  if (status != null)
    out[field.children.Status.idx] = status;
};
...
const processRow = (xml, { submission, submitter, attachments }, lookup) => new Promise((resolve, reject) => {
  // set up our state machine stacks:
  const fieldStack = [{ children: lookup }];
  const dataStack = [ [] ];
  const iterationStack = [{ self: submission.instanceId }];

  // there are some short-circuits if we don't have the right data.
  const status =
    (xml === 'missing') ? 'missing encrypted form data' :
    (xml === null) ? 'not decrypted' : null; // eslint-disable-line indent
  if (status != null) {
    const result = [];
    writeMetadata(result, fieldStack[0], submission, submitter, attachments, status);
  ...
  const parser = new hparser.Parser({
    ...
    onclosetag: () => {
      ...
      if (fieldStack.length === 0) {
        parser.reset();
        writeMetadata(row, field, submission, submitter, attachments);
        resolve(row);
      }
    }
...
const streamBriefcaseCsvs = (inStream, form, decryptor) => form.def.schema().then((inSchema) => {
    ...
    transform(row, _, done) {
      try {
        ...
        const xml =
          ...
          decryptor(row.encData, row.encKeyId, row.localKey, row.submission.instanceId, row.encIndex); // eslint-disable-line indent

        // write the root row we get back from parsing the xml.
        processRow(xml, row, lookup).then((x) => {
          this.push(x);
          done();
  ...
  const name = `${sanitize(form.xmlFormId)}.csv`;
