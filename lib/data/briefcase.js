...
const { schemaAsLookup } = require('./schema');
...
// for each repeat field, generates a stream, adds it to the archive with the
// appropriate name, decorates it into the field data, streams out the appropriate
// header row for it, and returns an array of all streams it generated.
// mutates the given data!
const decorateSubstreams = (archive, lookup, path = []) => {
  const result = [];
  for (const fieldName of Object.keys(lookup)) {
    const field = lookup[fieldName];
    ...
    if (field.type === 'repeat') {
      const stream = csv();
      archive.append(stream, { name: `${sanitize(path.join('-'))}-${sanitize(field.name)}.csv` });
      field.stream = stream;
    ...
    if ((field.type === 'repeat') || (field.type === 'structure')) {
      result.push(...decorateSubstreams(archive, field.children, path.concat([ field.name ])));
    }
  }
  return result;
};
...
const parentKeyForStacks = (fieldStack, iterationStack) => {
  for (let i = fieldStack.length - 2; i > 0; i--)
...
const streamBriefcaseCsvs = (inStream, form) => form.schema().then((schema) => {
  ...
  const substreams = decorateSubstreams(archive, lookup, [ form.xmlFormId ]);
  ...
  const rootStream = new Transform({
    ...
    transform(submission, _, done) {
      ...
      for (stream of substreams) { stream.push(null); }
      done();
    }
  });
