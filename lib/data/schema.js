...
const _recurseFormFields = (instance, bindings, repeats, envelope = instance.name, basePath = [], orderStart = 0) => {
  const fields = [];
  ...
  for (const tag of instance.children) {
    ...
    if (repeats.includes(bindingPath)) {
      field.type = 'repeat';
    } else {
    ...
    if (((field.type === 'repeat') || (field.type === 'structure')) && (tag.children != null)) {
      const children = _recurseFormFields(tag, bindings, repeats, envelope, path, order);
...
const getFormFields = (xml) => {
  ...
  return traverseXml(xml, [
    ...
    _recurseFormFields(instance.get(), getList(bindings), getList(repeats)));
};
...
class SchemaStack {
  ...
  children() {
    ...
    while (true) { // eslint-disable-line no-constant-condition
      ...
      if ((field == null) || !field.path.startsWith(this.path)) break;
      if ((this.path.length + 1 + field.name.length) !== field.path.length) continue; // eslint-disable-line no-continue
      children.push(field);
    }
    return children;
  }
