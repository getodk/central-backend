...
const _recurseFormSchema = (bindings, instance, path) => {
  ...
  for (const tag of instance.child) {
    ...
    if (tagName.startsWith('@_')) continue; // attribute, not node.

    if (findAndTraverse(tag, '@_template') != null) {
      // We have a repeat (don't need to check the binding to know); recurse.
      result.push({ name: tagName, type: 'repeat', children:
        _recurseFormSchema(bindings, tag, path.concat([ tagName ])) });
    } else {
      ...
      if (binding == null) {
        ...
        result.push({ name: tagName, type: 'structure', children:
          _recurseFormSchema(bindings, tag, path.concat([ tagName ])) });
      } else {
