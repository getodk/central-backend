...
const _recurseFormFields = (instance, bindings, repeats, selectManys, envelope = instance.name, basePath = [], orderStart = 0) => {
  const fields = [];
  ...
  for (const tag of instance.children) {
    ...
    if (selectManys.has(bindingPath)) // TODO: do we need to strip namespaces?
      field.selectMany = true;

    if (((field.type === 'repeat') || (field.type === 'structure')) && (tag.children != null)) {
      const children = _recurseFormFields(tag, bindings, repeats, selectManys, envelope, path, order);
...
const getFormFields = (xml) => {
  ...
  ]).then(([ instance, bindings, repeats, selectManys ]) =>
    _recurseFormFields(instance.get(), getList(bindings),
      new Set(stripNamespacesFromPaths(getList(repeats))), new Set(stripNamespacesFromPaths(getList(selectManys)))));
};
