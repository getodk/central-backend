...
const { always, last } = require('ramda');
...
const schemaToFields = (schema, prefix = '', orderStart = 0) => {
  const fields = [];
  let order = orderStart;
  for (const field of schema) {
    const path = `${prefix}/${field.name}`;
    const binary = (field.type === 'binary') || (path === '/meta/audit');
    fields.push({ path, name: field.name, type: field.type, binary, order });
    order += 1;

    if ((field.type === 'structure') || (field.type === 'repeat')) {
      const children = schemaToFields(field.children, path, order);
...
const keyForStacks = (fieldStack, iterationStack) => {
  ...
  for (let i = 0; i < fieldStack.length; i += 1) {
    ...
    if (field.type === 'structure')
      ...
      result.push(`${field.name}[${iterationStack[i][field.path]}]`);
  }
  return result;
};
...
class SchemaStack {
  ...
  // TODO: these are sort of specific to the briefcase output; the odata output
  // is a little different. how do we want to deal with this?
  iterationPath() { return keyForStacks(this.fieldStack, this.iterationStack); }
  parentIterationPath() {
    for (let i = this.fieldStack.length - 2; i >= 0; i -= 1)
      if (this.fieldStack[i].type === 'repeat')
        return keyForStacks(this.fieldStack.slice(0, i + 1), this.iterationStack.slice(0, i + 1));
    return [];
  }
  ...
  push(name) {
    ...
    if ((field == null) || (field.type !== 'repeat')) {
      ...
      iterationCtr[field.path] = (iterationCtr[field.path] || 0) + 1;
...
module.exports = {
  getFormSchema,
  schemaToFields, getSchemaTables, schemaAsLookup, stripNamespacesFromSchema, sanitizeOdataIdentifiers,
  SchemaStack,
