...
const init = (config) => {
  ...
  function destroy() {
    ...
    return new Promise(resolve => {
      let remaining = 0;
      log('s3.destroy()', 'aborting', inflight.size, 'requests...');
      for (const req of inflight) {
        ++remaining;
        req.once('close', () => {
          if(!--remaining) resolve();
        });
        log('DESTRYONG REQUESTIN:', req);
  ...
  async function uploadFromBlob(blob) {
    ...
    log('Uploading blob:', JSON.stringify({ md5, sha, length })); // eslint-disable-line no-console
  ...
  return {
    enabled: true,
    deleteObjFor:   guarded(deleteObjFor),
    getContentFor:  guarded(getContentFor),
    uploadFromBlob: guarded(uploadFromBlob),
    urlForBlob:     guarded(urlForBlob),
