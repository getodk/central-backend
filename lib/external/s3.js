...
const init = (config) => {
  ...
  const wrappedOrUnhandled = (err, operation, blobOrBlobs) => {
    ...
    console.log('s3.wrappedOrUnhandled()', 'err:', err);
    console.log('s3.wrappedOrUnhandled()', 'err.name:', err.name);
    console.log('s3.wrappedOrUnhandled()', 'err.code:', err.code);
    console.log('s3.wrappedOrUnhandled()', 'err.message:', err.message);
    console.log('s3.wrappedOrUnhandled()', 'err.props:', Object.keys(err));
  ...
  const getRespHeaders = (filename, contentType, disposition = 'attachment') => ({
    'response-content-disposition': contentDisposition(filename, disposition),
  ...
  async function urlForBlob(filename, blob, disposition = 'attachment') {
    ...
    const respHeaders = getRespHeaders(filename, blob.contentType, disposition);
  ...
  async function uploadFromBlob(blob) {
    ...
    // Uploading a zero-length blob is:
    //
    // 1. pointless at best and maybe counter-productive, and
    // 2. breaks minio-js
    //
    // See: https://github.com/getodk/central/issues/1496
    // See: https://github.com/minio/minio-js/issues/1436
    if (!length) return;
