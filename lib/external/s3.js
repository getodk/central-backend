...
const init = (config) => {
  ...
  const wrappedOrUnhandled = (err, operation, blob) => {
    if (isErrUpstream(err)) return Problem.internal.s3upstreamError({ amzRequestId: err.requestid, operation, blobId: blob.id });
    else if (isErrAccess(err)) return Problem.internal.s3accessDenied({ amzRequestId: err.requestid, operation, reason: err.message });
    else return err;
  };
  ...
  async function deleteObjFor(blob) {
    try {
      return await minioClient.removeObject(bucketName, objectNameFor(blob));
    } catch (err) {
      throw wrappedOrUnhandled(err, 'removeObject', blob);
    }
  }
