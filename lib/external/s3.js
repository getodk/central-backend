...
const init = (config) => {
  ...
  async function uploadFromBlob(blob) {
    ...
    // FIXME this check is not reliable: per s3 documentation:
    ...
    // FIXME this might completely change how etags should be handled - odk-central-backend
    // should probably store the etag response from s3, and support that as a blob's etag
    // IN ADDITION to the md5 hash - depending on how s3 requests are served from central,
    // a client may have seen either or both etags.
