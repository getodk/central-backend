...
const init = (config) => {
  ...
  const { pipethroughAndBuffer, PartialPipe } = require('../util/stream');
  ...
  async function pipeContent(blob) {
    try {
      return PartialPipe.of(await minioClient.getObject(bucketName, objectNameFor(blob)));
    } catch (err) {
      throw wrappedOrUnhandled(err, 'getObject', blob);
    }
  }
  ...
  return {
    ...
    pipeContent:    guarded(pipeContent),   // eslint-disable-line key-spacing, no-multi-spaces
