...
const init = (config) => {
  ...
  function destroy() {
    ...
    return new Promise(resolve => {
      ...
      for (const req of inflight) {
        ...
        req.once('error', err => {
          if (err !== destroyError) console.log('Ignoring unexpected error:', err);
          process.exit(1); // FIXME this is just to check in CI if there are unexpected errors being thrown here...
        });
  ...
  const minioClient = (() => {
    ...
    const request = (_options, callback) => {
      console.log(`[pid:${process.pid}] [minioClient.request()]`, 'ENTRY');
