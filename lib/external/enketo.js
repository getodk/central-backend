...
const enketo = (hostname, pathname, port, protocol, apiKey) => {
  ...
  const _edit = (apiPath, responseField) => (openRosaUrl, domain, form, logicalId, submissionDef, attachments, token) => {
    ...
    for (const att of attachments)
      if (att.blobId != null)
        attsMap[url`instance_attachments[${att.name}]`] = domain + url`/projects/${form.projectId}/forms/${form.xmlFormId}/submissions/${logicalId}/versions/${submissionDef.instanceId}/attachments/${att.name}`;
