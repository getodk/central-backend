...
const filterXmlFormIdFromUrl = (transaction, url) => {
  if (transaction.match(/\/forms\/:xmlFormId(\/.*)?$/)) {
    return url.replace(/\/forms\/[^/]*/, '/forms/:xmlFormId');
  }

  if (transaction.match(/\/forms\/:xmlFormId\.(\w+)(\/.*)?$/)) {
    return url.replace(/\/forms\/[^/]*\.(\w+)/, '/forms/:xmlFormId.$1');
  }
  ...
  if (event.request) {
    ...
    if (event.request.url) {
      ...
      if (event.transaction) {
        event.request.url = filterXmlFormIdFromUrl(event.transaction, event.request.url);
      }
    }
...
const _init = (config) => { // eslint-disable-line no-shadow
  if (!config || !config.key || !config.project || !config.orgSubdomain) {
    ...
    return {
      ...
      flush() {
        process.stderr.write('Sentry.flush()\n');
      },
  ...
  // Avoid irrelevant default integrations, while allowing for new, useful ones.
  ...
  const disabledIntegrations = [
    ...
    'Mysql2',
    'OpenAI',
    'PostgresJs', // The porsager module.  Sentry's "Postgres" Integration SHOULD be enabled.
    'Prisma',
    'Redis',
    'VercelAI',
  ];

  console.error('initialising sentry...');
  Sentry.init({
  ...
  //console.error('sentry initialised.');
  //Sentry.captureException(new Error('testing in setup'));
  //console.error('captured test error');
...
module.exports = { Sentry, _init, sanitizeEventRequest, isSensitiveEndpoint, filterXmlFormIdFromUrl, filterTokenFromUrl };
