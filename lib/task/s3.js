...
const uploadPending = withContainer(({ s3, Blobs }) => async () => {
  ...
  const shutdownListener = async (signal, ...args) => {
    console.log('shutdownListener()', args.length, signal, args);
    ...
    console.log('shutdownListener()', 're-sending kill signal', signal, '...');
