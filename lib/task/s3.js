...
const getCount = withContainer(({ Blobs }) => status => Blobs.s3CountByStatus(status)
  .then(count => console.log(count)));

const setFailedToPending = withContainer(({ Blobs }) => () => Blobs.s3SetFailedToPending()
  .then(count => console.log(`${count} blobs marked for re-uploading.`)));

// TODO rewrite the procedurally with more logging(?)
const uploadPending = withContainer((container) => (isTesting) => container.Blobs.s3CountByStatus('pending')
