...
const getCount = withContainer('s3.getCount', ({ s3, Blobs }) => async status => {
  assertEnabled(s3);
  try {
...
const setFailedToPending = withContainer('s3.setFailedToPending', ({ s3, Blobs }) => async () => {
  assertEnabled(s3);
  const count = await Blobs.s3SetFailedToPending();
  console.log(`${count} blobs marked for re-uploading.`);
});

const uploadPending = withContainer('s3.uploadPending', ({ s3, Blobs }) => async (limit) => {
  assertEnabled(s3);
