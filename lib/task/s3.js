...
const setInProgressToPending = withContainer(({ s3, Blobs }) => async () => {
  assertEnabled(s3);
  const count = await Blobs.s3SetInProgressToPending();
  console.log(`${count} blobs marked for re-uploading.`);
});
...
module.exports = { getCount, setFailedToPending, setInProgressToPending, uploadPending };
