...
const uploadPending = withContainer(({ s3, Blobs }) => async (limit) => {
  ...
  const shutdownListener = async signal => {
    ...
    // TODO wait properly for the s3 upload process to return, or at least give it a fighting chance
    // proper handling may be: with a reasonable timeout, block until s3UploadPending() promise has actually resolved
