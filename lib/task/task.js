...
const { merge, compose, identity } = require('ramda');
const config = require('config');
const pkg = require('../model/container');
const Problem = require('../util/problem');
const Option = require('../util/option');
const { slonikPool } = require('../util/slonik');
const { serialize } = require('../util/http');


// initialize modules we need to put in the container:
const password = require('../util/crypto').password(require('bcrypt'));
...
const task = {
  ...
  withContainer: (taskdef) => (...args) => {
    ...
    if (needsContainer) task._container = pkg.withDefaults({ pool: slonikPool(config.get('default.database')), password, env, mail });
...
const email = task.withContainer(({ mail, env }) => (messageId) =>
