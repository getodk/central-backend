...
const task = {
  ...
  withContainer: (name, taskdef) => (...args) => {
    ...
    return Sentry.startSpan(
      { op: 'task', name }, // see: https://github.com/getsentry/sentry-javascript/issues/8075#issuecomment-1538719331
      async () => {
        try {
...
const auditLog = task.withContainer('auditLog', ({ Audits }) => (action, success, details) =>
...
const email = task.withContainer('email', ({ mail, env }) => (messageId) => // eslint-disable-line no-shadow
