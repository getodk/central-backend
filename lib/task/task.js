...
const task = {
  withContainer: (taskdef) => (...args) => {
    ...
    const _container = task._testContainer ?? container.withDefaults({ db: slonikPool(config.get('default.database')), env, mail, task: true, odkAnalytics, s3, Sentry });

    console.log('Sentry:', Sentry);
    console.log('container:', container);
    console.log('container.Sentry:', container.Sentry);
    console.log('taskdef:', taskdef.name, taskdef);
