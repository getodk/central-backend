...
const odkAnalyticsReporter = require('../external/odk-reporter').init(config.get('default.external.analytics'));
...
const task = {
  ...
  withContainer: (taskdef) => (...args) => {
    ...
    if (needsContainer) task._container = container.withDefaults({ db: slonikPool(config.get('default.database')), env, mail, task: true, odkAnalyticsReporter, s3 });
