...
let dbPool;
slonikPool(config.get('default.database')).then(pool => { dbPool = pool; });
...
const task = {
  ...
  withContainer: (taskdef) => (...args) => {
    ...
    if (needsContainer) {
      if (!dbPool) {
        console.error(new Error('db pool not yet resolved!')); // eslint-disable-line no-console
        process.exit(9);
      }
      task._container = container.withDefaults({ db: dbPool, bcrypt, env, mail, task: true, odkAnalytics });
    }
