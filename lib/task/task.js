...
const initContainer = () => {
  ...
  // init a container and return it.
  return pkg.withDefaults({ db: connect(config.get('default.database')), crypto, mail });
};
...
const task = {
  ...
  withContainer: (taskdef) => (...args) => {
    ...
    if (needsContainer) task._container = initContainer();
