...
const task = {
  ...
  withContainer: (taskdef) => (...args) => {
    console.log('withContainer()', 'taskdef:', taskdef);

    const needsContainer = (task._container == null);
    if (needsContainer) task._container = container.withDefaults({ db: slonikPool(config.get('default.database')), env, mail, task: true, analyticsReporter, s3 });

    const [, scriptPath] = process.argv;
    const { name } = Path.parse(scriptPath);

    Sentry.setTag('odk-task', name);
    return Sentry.startSpan(
      { op: 'task', name },
      async () => {
        try {
          console.log('calling taskdef...');
          return await taskdef(task._container)(...args);
          console.log('taskdef returned.');
        } catch (err) {
          console.log(`
            @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
            @
          ...
          console.log('Flusing sentry...');
          await Sentry.flush(2000);
          console.log('Flusing returned.');
          await Sentry.close();
        }
      },
    );
