...
const task = {
  ...
  withContainer: (taskdef) => (...args) => {
    ...
    if (needsContainer) task._container = container.withDefaults({ db: slonikPool(config.get('default.database')), bcrypt, env, mail, task: true });
...
const auditLog = task.withContainer(({ Audits }) => (action, success, details) =>
  Audits.log(null, action, null, merge({ success }, details)));
