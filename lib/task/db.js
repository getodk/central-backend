...
const pgdump = (directory, namespace = 'default') => {
...
const pgrestore = async (directory, namespace = 'default') => {
  ...
  const pgrestore = `pg_restore -e -j 4 -F d -C -c -h ${dbConfig.host} -U ${dbConfig.user} -d template1 ${directory}`;
  await task.promisify(exec)(pgrestore, { env });
