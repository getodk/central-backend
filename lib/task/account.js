...
const createUser = task.withContainer('account.createUser', (container) => (email, password) => container.transacting(({ Users }) =>
...
const promoteUser = task.withContainer('account.promoteUser', (container) => (email) => container.transacting(({ Assignments, Users }) =>
...
// to the given one.
const setUserPassword = task.withContainer('account.setUserPassword', (container) => (email, password) => container.transacting(({ Sessions, Users }) =>
  Users.getByEmail(email)
    .then(getOrNotFound)
