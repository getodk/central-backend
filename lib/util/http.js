...
const isAtomType = (x) => /^(application\/(atom(svc)?\+)?xml|atom|xml)$/i.test(x);

// these are functions we support on our OData api.
const supportedParams = [ '$format', '$count', '$skip', '$top' ];

// the second json parameter indicates whether this endpoint should force a json
// response format.
const odataEndpoint = (f) => {
  ...
  return (request, response, next) => {
    ...
    if (isJsonType(formatParam) || isJsonType(accept))
      request.odataFormat = 'json';
    else if (isAtomType(formatParam) || isAtomType(accept))
      request.odataFormat = 'atom';
    else if (isBlank(formatParam) && (isBlank(accept) || (accept === '*/*')))
      request.odataFormat = 'atom';
    else
      return next(Problem.user.unacceptableFormat({ allowed: [ 'application/json', 'json', 'application/xml', 'atom' ], got: (request.query['$format'] || request.headers['accept']) }));
...
module.exports = {
  serialize, finalize,
  endpoint, openRosaEndpoint, odataEndpoint,
  sendError,
  versionParser, sessionParser, fieldKeyParser, headerOptionsParser,
  getOrElse, getOrReject, getOrNotFound, rejectIf,
  isTrue,
  success, contentType, xml, atom
};
