...
function blobOrS3Redirect(filename, blob) {
  ...
  if (arguments.length === 1) {
    return _blob => blobOrS3Redirect(filename, _blob);
  }

  if (blob.s3) {
    // TODO move s3 URL creation to a utility function?
    // TODO generate proper s3 URL from config
    // TODO standardise s3 object name
  ...
  return withEtag(`${blob.md5}`, () => binary(blob.contentType, filename, blob.content));
}
