...
// Otherwise executes given function 'fn'.  In some cases (e.g. redirects), it's
// inappropriate to set the ETag header.  Setting `always` to false prevents the
// etag header being set in these cases.
const _withEtag = (serverEtag, fn, always=true) => (request, response) => {
...
const withEtag = (serverEtag, fn) => _withEtag(serverEtag, fn, true);
const withEtagOnMatch = (serverEtag, fn) => _withEtag(serverEtag, fn, false);
