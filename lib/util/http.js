...
const contentDisposition = (filename, type = 'attachment') => {
  ...
  return `${type}; filename="${sanitizeToAscii(sanitized)}"; filename*=UTF-8''${encodeRFC5987ValueChars(sanitized)}`;
};
...
const withEtag = (serverEtag, fn, weak=false) => (request, response) => {
...
const convertToSubmissionVersion = (urlString, instanceId, rootId) => {
  const urlObject = new URL(urlString, 'https://example.com');
  const parts = urlObject.pathname.split('/');
  // Remove the API version, project ID, and xmlFormId.
  const redirectParts = parts.splice(0, 6);
  while (parts.length > 1) {
    const part = parts.shift();
    const nextPart = parts[0].replace(/(.)\.xml$/, '$1');
    if (part === 'submissions' && decodeURIComponent(nextPart) === instanceId) {
      redirectParts.push('submissions', encodeURIComponent(rootId), 'versions', ...parts);
      return redirectParts.join('/') + urlObject.search;
    } else {
      redirectParts.push(part);
    }
  }
  throw new Error('unable to redirect');
};

const setOrRemoveHeader = (res, header, value) => {
  if (value === undefined) res.removeHeader(header);
  else res.set(header, value);
}

const setCachePolicy = (res, policy) => {
  if (!policy) throw new Error('Supplied cache policy was undefined');

  setOrRemoveHeader(res, 'Cache-Control', policy.cacheControl);
  setOrRemoveHeader(res, 'Vary',          policy.vary); // eslint-disable-line no-multi-spaces
};
