...
const fieldKeyParser = ({ Session, Auth }) => (request, response, next) => {
  ...
  if (request.auth.session.isDefined()) return next(Problem.user.authenticationFailed());
...
const headerOptionsParser = (request, response, next) => {
  ...
  if (!isBlank(extendedMeta) && (extendedMeta.toLowerCase() === 'true'))
...
module.exports = { serialize, finalize, endpoint, openRosaEndpoint, sendError, versionParser, sessionParser, fieldKeyParser, headerOptionsParser, getOrElse, getOrReject, getOrNotFound, success };
