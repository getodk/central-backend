...
const openRosaEndpoint = (f) => (request, response, rawNext) => {
  const next = (x) => rawNext(((x != null) && (x.isProblem === true)) ? x.forOpenRosa() : x);

  // Even if we fail we'll want these headers so just do it here.
  response.append('Content-Language', 'en');
  response.append('X-OpenRosa-Version', '1.0');
  response.append('X-OpenRosa-Accept-Content-Length', '20' + '000' + '000');
  response.append('Date', DateTime.local().toHTTP());
  ...
  const success = (result) => { response.status(200).send(result); };
  finalize(success, next)(f(request, response));
};
...
const sendError = (error, request, response) => {
  if (error.isOpenRosaProblem === true) {
    const problem = error.inner;
    // TODO: include more of the error detail.
    response.status(problem.httpCode).send(openRosaMessage({ nature: 'error', message: error.message }));
...
module.exports = { serialize, finalize, endpoint, openRosaEndpoint, sendError, sessionParser, getOrElse, getOrReject, getOrNotFound };
