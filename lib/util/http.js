...
function blobResponse(filename, blob) {
  console.log('blobResponse', filename, blob);
  if (arguments.length === 1) {
    return _blob => blobResponse(filename, _blob);
  }

  if (blob.s3_status === 'uploaded') {
    return withEtag(blob.md5, async () => {
      const s3url = await s3.urlForBlob({ ...blob, filename });
      console.log('blobResponse()', 'returning redirect to:', s3url);
