...
const withEtag = (serverEtag, fn, weak=false) => (request, response) => {
  ...
  console.log('withEtag()', { weak, serverEtag, clientEtag });
...
const convertToSubmissionVersion = (urlString, instanceId, rootId) => {
  ...
  while (parts.length > 1) {
    ...
    const nextPart = parts[0].replace(/(.)\.xml$/i, '$1');
    if (part.toLowerCase() === 'submissions' && decodeURIComponent(nextPart) === instanceId) {
