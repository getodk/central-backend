...
// Otherwise executes given function 'fn'.  In some cases (e.g. when fn()
// returns a non-304 redirect), it's inappropriate to set the ETag header.
// Setting `onlyOnMatch` prevents the etag header being set in these cases.
const _withEtag = (serverEtag, fn, onlyOnMatch) => (request, response) => {
  if (!onlyOnMatch) response.set('ETag', `"${serverEtag}"`);
...
const withEtag = (serverEtag, fn) => _withEtag(serverEtag, fn, false);
const withEtagOnMatch = (serverEtag, fn) => _withEtag(serverEtag, fn, true);

module.exports = {
  isTrue, isFalse, urlPathname,
  success, contentType, xml, atom, json, contentDisposition, redirect,
  urlWithQueryParams, url,
  withEtag, withEtagOnMatch,
};
