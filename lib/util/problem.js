...
const problems = {
  user: {
    ...
    passwordTooWeak: problem(400.44, ({ warning, suggestions }) => {
      const externalMessage = warning || suggestions[0];
      const baseMessage = 'The supplied password is too weak';
      if (!externalMessage) return `${baseMessage}.`;
      if (externalMessage.includes('.')) return `${baseMessage}. ${externalMessage}`;
