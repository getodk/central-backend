...
async function uploadBlobIfAvailable(db) {
  ...
  if (!rows.length) return console.log('uploadBlobIfAvailable() :: no blobs left');

  const blob = rows[0];
  try {
    // eslint-disable-next-line no-console
    console.log('uploadBlobIfAvailable() :: uploading:', blob);
    await uploadFromBlob(blob);
    // eslint-disable-next-line no-console
    console.log('uploadBlobIfAvailable() :: blob uploaded; updating status...');
    await db.query(`UPDATE blobs SET s3_status='uploaded', content=NULL WHERE id=$1`, [blob.id]);
    return true;
  } catch (err) {
    await db.query(`UPDATE blobs SET s3_status='failed' WHERE id=$1`, [blob.id]);
