...
const minioClient = (() => {
  ...
  // request override from: https://github.com/minio/minio-js/issues/722#issuecomment-1594401449
  const http = require('node:http');
  const https = require('node:https');
  const request = (options, callback) => {
    options.timeout = 1;
    return (useSSL ? https : http).request(options, callback);
  }

  const clientConfig = { endPoint, port, useSSL, accessKey, secretKey, transport: { request }, sessionTimeout: 1, timeout: 1 };

  const client = new Minio.Client(clientConfig);
  client.setRequestOptions({ sessionTimeout: 1, timeout: 1 }); // low - let's see what fails.  FIXME remove this or set to sensible value
  return client;
})();
