...
const init = (config) => {
  ...
  async function uploadBlobIfAvailable({ Blobs }) {
    const maybeBlob = await Blobs.getOnePending();
    if (isEmpty(maybeBlob)) return;

    const blob = maybeBlob.get();
    try {
      await uploadFromBlob(blob);
      await Blobs.markAsUploaded(blob);
      return true;
    } catch (err) {
      await Blobs.markAsFailed(blob);
  ...
  return {
    isEnabled: () => true,
    // FIXME these are all blob methods - move to query/blobs.js; minioClient can move to lib/external/s3.js
