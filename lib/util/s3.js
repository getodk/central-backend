...
const minioClient = (() => {
  ...
  const REQUEST_TIMEOUT = isNaN(requestTimeout) ? 120000 : requestTimeout; // ms;
  ...
  const request = (options, callback) => {
    ...
    // setTimeout(): "Milliseconds before a request times out" - https://nodejs.org/api/http.html#requestsettimeouttimeout-callback
    // options.timeout: "the timeout before the socket is connected" - https://nodejs.org/api/http.html#httprequestoptions-callback

    if (isNaN(options.timeout)) options.timeout = REQUEST_TIMEOUT;
    ...
    setTimeout(       () => { if (!closed) req.destroy(new Error('Request exceeded total timeout.')); }, REQUEST_TIMEOUT);
