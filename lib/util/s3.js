...
// FIXME simplify this mess of promises
async function getContentFor(blob) {
  console.log('s3.getContentFor()', blob);
  return new Promise((resolve, reject) => {
    try {
      minioClient.getObject(bucketName, objectNameFor(blob), (err, stream) => {
        console.log('s3.getContentFor().getObject() returned', { err, stream });
        if (err) return reject(err);
        try {
          return pipethroughAndBuffer(stream)
            .then(([ buf ]) => {
              console.log('pipethroughAndBuffer() returned:', buf);
              resolve(buf);
            })
            .catch(err => {
              console.log('pipethroughAndBuffer() threw:', err);
              reject(err);
            });
        } catch(err) {
          reject(err);
