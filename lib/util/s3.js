...
function objectNameForBlob(blob) {
  ...
  const { md5, sha, filename } = blob;

  if(!md5 || !sha || !filename) throw new Error(`blob missing required prop(s) from: md5, sha, filename: ${JSON.stringify(blob)}`);

  // TODO need to sanitise the filename?
  return `blob_md5_${md5}_sha_${sha}_${filename}`;
}
