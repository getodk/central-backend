...
const minioClient = (() => {
  ...
  const SMALL_REQ_TIMEOUT = Math.max(1, MAX_REQUEST_TIMEOUT - 1000);
  ...
  const request = (options, callback) => {
    ...
    setTimeout(       () => { if (!closed) req.destroy(new Error('Request exceeded total timeout.')); }, MAX_REQUEST_TIMEOUT);
