...
//
// in some cases, the user will need to be careful not to call finalize() until the
// underlying database stream is complete, or else multiple streams can try to open
// at the same time.
const zipPart = () => {
...
const zipStreamFromParts = (...zipPartFns) => {
  ...
  const next = () => {
    if (zipPartFns.length === 0) return resultStream.finalize();
    const part = zipPartFns.shift()();
