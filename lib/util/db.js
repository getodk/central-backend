...
const { clone, mergeRight, pick, always, without, remove, indexOf } = require('ramda');
...
const withPgSslConfig = (_cfg) => {
  const cfg = clone(_cfg);

  const { PGSSLMODE, PGSSLROOTCERT } = process.env;

  if (PGSSLROOTCERT) {
    // If a root certificate is supplied, make sure it's used to verify the server's cert.
