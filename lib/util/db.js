...
const applyPaging = (offset, limit) => (db) => {
  let chain = db;
  if (offset != null) chain = chain.offset(offset);
  if (limit != null) chain = chain.limit(limit);
  return chain;
};
...
module.exports = {
  fieldsForJoin, joinRowToInstance, applyPaging,
