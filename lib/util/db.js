...
const { merge, identity, pick, always } = require('ramda');
...
const unjoiner = (...frames) => {
  ...
  for (const MaybeInstance of frames) {
    ...
    for (const field of Instance.fields) {
      const sqlname = `${Instance.from}!${field}`;
      fields.push(`${Instance.from}."${field}" as "${sqlname}"`);
      unmap[sqlname] = Instance.to;
      unprefix[sqlname] = field;
      constructors[Instance.to] = isOption ? maybeConstruct(Instance.construct) : Instance.construct;
    }
  }
...
const extender = (...standard) => (...extended) => (sqlFunc) => {
  ...
  return (exec, options) => {
    const unjoiner = (options.extended === true) ? extUnjoiner : stdUnjoiner;
    const extend = (options.extended === true) ? null : nothing;
    return exec(sqlFunc(unjoiner.fields, options, extend)).then(exec.map(unjoiner));
  };
};

// generic insert utility
const _insert = (table, keys, data) => sql`
insert into ${sql.identifier(table)} (${sql.raw(keys.map((k) => `"${k}"`).join(','))})
values (${sql.join(keys.map((k) => obj[k]), sql`,`)}) returning *`;
const insert = (obj) => _insert(obj.constructor.from, Object.keys(obj), obj);
const insertAll = (obj) => _insert(obj.constructor.from, obj.constructor.fields, obj);

// generic update utility
const _update = (table, keys, data, wv, wk) => sql`
update ${sql.identifier(table)}
set ${sql.join(keys.map((k) => sql`${sql.identifier(k)}=${data[k]}`), sql`,`)}
where ${sql.identifier(wk)}=${wv}`;
const update = (obj, whereValue, whereKey = 'id') =>
  _update(obj.constructor.from, Object.keys(obj), obj, whereValue, whereKey);
const updateAll = (obj, whereValue, whereKey = 'id') =>
  _update(obj.constructor.from, obj.constructor.fields, obj, whereValue, whereKey);
...
const page = (options) => {
  const parts = [];
  if (options.offset != null) parts.push(sql`offset ${option.offset}`);
...
const queryFuncs = (q) => {
  const run = (s) => q.query(s).then(always(true));
  run.map = () => (x) => x;
  const one = (s) => q.one(s);
  one.map = (f) => (x) => f(x);
  const maybeOne = (s) => q.maybeOne(s).then(Option.of);
  maybeOne.map = (f) => (x) => x.map(f);
  const oneFirst = (s) => q.oneFirst(s);
  oneFirst.map = (f) => (x) => f(x);
  const maybeOneFirst = (s) => q.maybeOneFirst(s).then(Option.of); // not sure we need this tbh
  maybeOneFirst.map = (f) => (x) => x.map(f);
  const all = (s) => q.any(s);
  all.map = (f) => (xs) => {
    const result = new Array(x.length);
    for (let i = 0; i < x.length; i++) { result.push(f(x)); } // TODO/SL: ughhh i hate this func invoc every time
    return result;
  };
  const stream = (s) => q.stream(s);
  stream.map = (f) => (strm) => PartialPipe.of(strm, mapStream(f));

  return { run, one, maybeOne, oneFirst, maybeOneFirst, all, stream };
};
...
class QueryOptions {
  constructor(options) {
    Object.assign(this, { extended: false, condition: {}, fragment: nothing }, options);
  }
  ...
  withFragment(fragment) {
    return new QueryOptions(merge(this, { fragment }));
  }
  ...
  ifArg(arg, f) {
    if (options.args == null) return;
    if (options.args[arg] == null) return;
    return f(options.args[arg]);
  }
