...
const _assign = (obj) => (k) => (k === 'createdAt') ? sql`now()` : obj[k];
const _insert = (table, fieldlist, keys, data) => sql`
insert into ${sql.raw(table)} (${fieldlist})
...
const insertMany = (objs) => {
  ...
  return sql`
insert into ${sql.raw(Type.table)} (${Type.fieldlist})
values ${sql.join(
  objs.map((obj) => sql`(${sql.join(Type.fields.map(_assign(data)), sql`,`)})`),
  sql`,`)}`;
};

// generic update utility
const _update = (Type, keys, data, wv, wk) => sql`
update ${sql.raw(Type.table)}
