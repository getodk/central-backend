...
class QueryStream extends Readable {
  constructor(q) {
    // pg-query-stream uses default 100 hwm, just bake it here until we want
    // otherwise. meanwhile cursor batch size could be a separate number but they
    // default to the same in pg-q-s so again just take it for now..
    super({ objectMode: true, autoDestroy: true, highWaterMark: 100 });
    this.q = q;
    this.iterator = q.cursor(100)[Symbol.asyncIterator]();
  }
  // https://nodejs.org/api/stream.html#stream_readable_read_size_1
  _read(size) {
    this.q.cursorRows = size; // CRCRCR: this is internals
    const next = this.iterator.next();
    if (next.done === true) return this.push(null);

    next.then(({ value, done }) => {
      // TODO: need to handle .push retval
      if (value != null) for (let i = 0; i < value.length; i += 1) this.push(value[i]);
      if (done) this.push(null);
    }).catch(this.destroy.bind(this));
  }

  //_final() { this.iterator.return(); }
  _destroy(err, cb) {
...
const queryFuncs = (db, obj) => {
  ...
  obj.one = (s) => db`${s}`.then((xs) => xs[0]); // CRCRCR: slonik used to Enforce this, do we care?
  obj.one.map = (f) => (x) => f(x);
  obj.maybeOne = (s) => db`${s}`.then((xs) => Option.of(xs[0]));
  obj.maybeOne.map = (f) => (x) => x.map(f);
  obj.oneFirst = (s) => db`${s}`.values().then((xs) => xs[0][0]);
