...
const validateConfig = (config) => {
  ...
  if (!(ssl == null || ssl === true || ssl === false))
    throw Problem.internal.invalidDatabaseConfig({ reason: 'ssl can only be true or false.' });

  const unsupportedKeys = Object.keys(unsupported);
  if (unsupportedKeys.length !== 0)
    throw Problem.internal.invalidDatabaseConfig({
      reason: `'${unsupportedKeys[0]}' is not supported.`
    });
};
