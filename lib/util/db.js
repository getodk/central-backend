...
insert into ${sql.identifier(table)} (${fieldlist})
values (${sql.join(keys.map((k) => (k === 'createdBy') ? sql`now()` : obj[k]), sql`,`)})
returning *`;
const insert = (obj) => {
  const keys = Object.keys(obj);
  if (obj.constructor.hasCreatedAt && (obj.createdAt == null)) keys.push('createdAt');
  _insert(obj.constructor.table, sql.join(keys.join((k) => `"${k}"`), sql`,`), keys, obj);
};
const insertAll = (obj) =>
...
update ${sql.identifier(Type.table)}
...
const del = (obj) => ({ run }) =>
  run(sql`delete from ${sql.raw(obj.constructor.table)} where id=${obj.id}`);
const markDeleted = (obj) => ({ run }) =>
  run(sql`update ${sql.raw(obj.constructor.table)} set "deletedAt"=now() where id=${obj.id}`);
...
const queryFuncs = (db, obj) => {
  obj.run = (s) => q.query(s).then(always(true));
  obj.run.map = () => () => true;

  obj.one = (s) => q.one(s);
  obj.one.map = (f) => (x) => f(x);
  obj.maybeOne = (s) => q.maybeOne(s).then(Option.of);
  obj.maybeOne.map = (f) => (x) => x.map(f);
  obj.oneFirst = (s) => q.oneFirst(s);
  obj.oneFirst.map = (f) => (x) => f(x);
  obj.maybeOneFirst = (s) => q.maybeOneFirst(s).then(Option.of); // not sure we need this tbh
  obj.maybeOneFirst.map = (f) => (x) => x.map(f);

  obj.all = (s) => q.any(s);
  obj.all.map = (f) => (xs) => {
    const result = new Array(x.length);
    for (let i = 0; i < x.length; i++) result[i] = f(x); // TODO/SL: ughhh i hate this repeated func invoc
    return result;
  };

  obj.stream = (s) => q.stream(s);
...
module.exports = {
  ...
  insert, insertAll, update, updateWritable, updateAll, del, markDeleted
  QueryOptions,
