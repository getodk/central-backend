...
const queryFuncs = (db, obj) => {
  ...
  obj.stream = (s) => new Promise((resolve, reject1) => db.stream(s, innerStream => {
    // eslint-disable-next-line no-plusplus
    const streamId = `stream:${++streamNumber}`;
    // eslint-disable-next-line no-console
    const log = (...args) => console.log(new Date().toISOString(), '[db.stream]', streamId, ...args);
    log('Started.');
    let dataCount = 0;
    try {
      const streamTimeout = setTimeout(() => {
        // eslint-disable-next-line no-use-before-define
        log('Stream timed out:', `received:${dataCount}`, `(innerStream.destroyed:${innerStream.destroyed}; wrappedStream.destroyed:${wrappedStream.destroyed})`);
        // TODO should probably be innerStream.destroy(new Error('Stream timed out')), but
        // innerStream.destroy(error) may currently throw other issues:
      ...
      const wrappedStream = innerStream.pipe(new Transform({
        objectMode: true,
      ...
      wrappedStream.on('error', error => {
        log('wrappedStream.on.error()', `received:${dataCount}`, `(innerStream.destroyed:${innerStream.destroyed}; wrappedStream.destroyed:${wrappedStream.destroyed})`, error);
        clearTimeout(streamTimeout);
        if (!innerStream.destroyed) innerStream.destroy(error);
      });

      innerStream.on('error', error => {
        log('innerStream.on.error()', `received:${dataCount}`, `(innerStream.destroyed:${innerStream.destroyed}; wrappedStream.destroyed:${wrappedStream.destroyed})`, error);
        clearTimeout(streamTimeout);
        if (!wrappedStream.destroyed) wrappedStream.destroy(error);
      });

      wrappedStream.on('close', () => {
        log('wrappedStream.on.close()', `received:${dataCount}`, `(innerStream.destroyed:${innerStream.destroyed}; wrappedStream.destroyed:${wrappedStream.destroyed})`);
        clearTimeout(streamTimeout);
        if (!innerStream.destroyed) innerStream.destroy();
      });

      innerStream.on('close', () => {
        log('innerStream.on.close()', `received:${dataCount}`, `(innerStream.destroyed:${innerStream.destroyed}; wrappedStream.destroyed:${wrappedStream.destroyed})`);
        clearTimeout(streamTimeout);
...
const postgresErrorToProblem = (x) => {
  ...
  } else if (error.code === '22P02' || error.toString().match(/invalid input syntax/)) { // N.B. https://github.com/gajus/slonik/issues/286 means we no longer receive originalError here
