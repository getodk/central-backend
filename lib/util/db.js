...
const slonikIsTrue = slonikSqlToken => {
  if (slonikSqlToken.type !== Symbol.for('SLONIK_TOKEN_SQL')) return false;

  const normalised = slonikSqlToken.sql.trim().toLowerCase();
  if (!normalised || normalised === 'true') return true;

  if (normalised === '$1' &&
      slonikSqlToken.values.length === 1 &&
      slonikSqlToken.values[0] === true) return true;
};

const sqlAndJoin = tokOrArr => {
  if (Array.isArray(tokOrArr)) {
    const arr = tokOrArr.filter(it => !slonikIsTrue(it));
    if (!arr.length) return sql``;
    if (arr.length === 1) return arr[0];
    return sql.join(arr, sql` AND `);
  }

  const slonikSqlToken = tokOrArr;
  if (slonikSqlToken.type === Symbol.for('SLONIK_TOKEN_SQL')) {
    if (slonikIsTrue(slonikSqlToken)) return sql``;
    return slonikSqlToken;
  } else if (slonikSqlToken.type === Symbol.for('SLONIK_TOKEN_LIST') && slonikSqlToken.glue.sql.match(/^\s+AND\s+$/i)) {
    return sqlAndJoin(slonikSqlToken.members);
  } else {
    return sql`(${slonikSqlToken})`;
  }
};

const sqlAnd = tokOrArr => {
  const joined = sqlAndJoin(tokOrArr);
  if (joined.type === Symbol.for('SLONIK_TOKEN_SQL')  && joined.sql.length)     return sql`AND ${joined}`; // eslint-disable-line no-multi-spaces
  if (joined.type === Symbol.for('SLONIK_TOKEN_LIST') && joined.members.length) return sql`AND ${joined}`;
  else return joined;
};
...
const guarded = s => {
  if (s.sql.split('\n').join(' ').match(/AND\s+TRUE/ig)) {
    console.log(new Error(`AND TRUE found in: ${s.sql}`)); // eslint-disable-line no-console
    process.exit(1);
  }
  return s;
};
const queryFuncs = (db, obj) => {
  ...
  obj.run = (s) => db.query(guarded(s)).then(always(true));
  obj.run.map = () => () => true;

  obj.one = (s) => db.one(guarded(s));
  obj.one.map = (f) => (x) => f(x);
  obj.maybeOne = (s) => db.maybeOne(guarded(s)).then(Option.of);
  obj.maybeOne.map = (f) => (x) => x.map(f);
  obj.oneFirst = (s) => db.oneFirst(guarded(s));
  obj.oneFirst.map = (f) => (x) => f(x);
  obj.maybeOneFirst = (s) => db.maybeOneFirst(guarded(s)).then(Option.of);

  obj.all = (s) => db.any(guarded(s));
  obj.all.map = (f) => (xs) => {
    const result = new Array(xs.length);
    for (let i = 0; i < xs.length; i += 1) result[i] = f(xs[i]);
    return result;
  };
  obj.allFirst = (s) => db.anyFirst(guarded(s));

  obj.stream = (s) => new Promise((resolve, reject1) => {
    db.stream(guarded(s), stream => {
      try {
...
module.exports = {
  unjoiner, extender, sqlAnd, sqlAndJoin, sqlEquals, page, queryFuncs,
