...
const _assign = (obj) => (k) => {
  ...
  return (v === null) ? null :
    (v === undefined) ? null :
    (v instanceof Buffer) ? sql.binary(v) : // eslint-disable-line indent
...
const _update = (Type, keys, data, wk, wv) => {
...
${Type.hasUpdatedAt ? sql`, "updatedAt"=clock_timestamp()` : nothing}
where ${sql.identifier([ wk ])}=${wv}
returning *`;
};
const updater = (obj, data, whereKey = 'id') =>
  _update(obj.constructor, Object.keys(data), data, whereKey, obj[whereKey]);
const updateAll = (obj, data, whereKey = 'id') =>
  _update(obj.constructor, obj.constructor.fields, data, whereKey, obj[whereKey]);

// generic del utilities
const del = (obj) =>
  sql`delete from ${raw(obj.constructor.table)} where id=${obj.id}`;
...
const equals = (obj) => {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    const k = keys[i];
    const v = obj[k];
    if (v === null) parts.push(sql(`${sql.identifier(k.split('.'))} is null`));
    else parts.push(sql`${sql.identifier(k.split('.'))}=${obj[k]}`);
  }
