...
const withJoin = (primary, map, query) => {
  ...
  for (const prefix of Object.keys(map)) {
    ...
    const select = Instance.fields.select || Instance.fields.all;
...
const unjoiner = (...frames) => {
  ...
  for (const Instance of frames) {
    for (const field of Instance.fields) {
      const sqlname = `${Instance.table}!${field}`;
      fields.push(`${Instance.table}."${field}" as "${sqlname}"`);
      unmap[sqlname] = Instance.propname;
  ...
  const unjoin = (row) => {
    ...
    const primary = bag[frames[0].propname];
    delete bag[frames[0].propname];
...
const queryFuncs = (q) => {
  ...
  return { one, maybeOne, oneFirst, maybeOneFirst, all };
};
