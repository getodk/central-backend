...
const extender = (...standard) => (...extended) => (sqlFunc) => {
  ...
  return (exec, options, x, y, z) => {
    ...
    return exec(sqlFunc(unjoiner.fields, extend, options, x, y, z)).then(exec.map(unjoiner));
  };
};

// generic insert utility
const _insert = (table, fieldlist, keys, data) => sql`
insert into ${sql.identifier(Type.from)} (${fieldlist})
values (${sql.join(keys.map((k) => obj[k]), sql`,`)}) returning *`;
const insert = (obj) => {
  const keys = Object.keys(obj);
  _insert(obj.constructor.from, sql.join(keys.join((k) => `"${k}"`), sql`,`), keys, obj);
};
const insertAll = (obj) =>
  _insert(obj.constructor.from, obj.constructor.fieldlist, obj.constructor.fields, obj);
