...
const { encodeNonAsciiHTML } = require('entities');
...
class Html {
  ...
  toString() { return this.content.map(c => c.toString()).join(''); }
}

const html = ([ head, ...tail ], ...vars) => {
  const processed = vars.map((v, idx) => {
    if (v == null) throw new Error(`Cannot embed value '${v}' at position ${idx} in HTML!`);
    const safeV = v instanceof Html ? v : encodeNonAsciiHTML(v.toString());
    return [ safeV, tail[idx] ];
  });
  return new Html(head, ...processed.flat());
};
...
module.exports = { frontendPage, html, safeNextPathFrom, Html };
