...
const getPrivate = (keys, passphrase = '') =>
  promisify(pbkdf2)(passphrase, b64ToBuffer(keys.salt), 128000, 32, 'sha256').then((privkeyKey) => {
    try {
      ...
      // For debugging https://github.com/getodk/central-backend/issues/1212
      // N.B. this is logging secret material so MUST NOT run outside of test environments.
      if(process.env.CI) {
        throw new Error(`#1212 Error thrown for ${JSON.stringify({ keys, passphrase })}`, { cause: ex });
      }
