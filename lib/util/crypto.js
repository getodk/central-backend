...
// TODO/CR: will be removed in the next commit, will not test.
// TODO/CR: will be removed in the next commit, will not test.
// TODO/CR: will be removed in the next commit, will not test.
// TODO/CR: will be removed in the next commit, will not test.
const getSubmissionIv = (instanceId, key, idx) => {
  const iv = createHash('md5').update(instanceId).update(key).digest();
...
module.exports = {
  ...
  getPrivate, generateKeypair, generateVersionSuffix,
  stripPemEnvelope,
  generateLocalCipherer, getLocalDecipherer,
  getSubmissionKey, getSubmissionIvs, getSubmissionIv, getSubmissionCleartext, streamSubmissionCleartext
};
