...
const { createHash, randomBytes, generateKeyPair, pbkdf2, createPrivateKey, createPublicKey, createCipheriv, createDecipheriv, publicEncrypt, privateDecrypt } = require('node:crypto');
...
const hashPassword = (plain) => {
  ...
  const { score, feedback: { warning, suggestions } } = zxcvbn(plain);
  if (score <= 2) return reject(Problem.user.passwordTooWeak({ warning, suggestions }));
