...
const submissionDecryptor = (keys, passphrases) =>
  Promise.all(keys.map((k) => getPrivate(k.private, passphrases[k.id])))
    ...
    .then((privs) => {
      ...
      return (data, keyId, localKey, instanceId, blobIndex, blobCount) => {
        ...
        // get the iv; generate all ivs if necessary.
        if (ivs[instanceId] == null) ivs[instanceId] = getSubmissionIvs(instanceId, pub, blobCount);
