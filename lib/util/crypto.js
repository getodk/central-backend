...
const getPrivate = (keys, passphrase = '') =>
  promisify(pbkdf2)(passphrase, b64ToBuffer(keys.salt), 128000, 32, 'sha256').then((privkeyKey) => {
    try {
      ...
      if (process.env.CI && passphrase !== 'wrong') {
        throw new Error(`#1212 Error thrown for ${JSON.stringify({ keys, passphrase })}
          ...
          *************************************************************
          *
          * Please add a link to this build failure at:
          *
          *     https://github.com/getodk/central-backend/issues/1212
          *
          *************************************************************
        `, { cause: ex });
      }
