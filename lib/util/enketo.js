...
const enketo = (hostname, pathname, port, protocol, apiKey) => ({
  preview: (openRosaUrl, xmlFormId, token, response) => new Promise((resolve, reject) => {
    ...
    const req = request({ hostname, port, headers, method: 'POST', path: previewPath, auth }, (res) => {
      ...
      res.on('end', () => {
        let body;
        try { body = JSON.parse(Buffer.concat(resData)); }
        catch (ex) { reject(Problem.internal.enketoUnexpectedResponse('invalid JSON')); }
