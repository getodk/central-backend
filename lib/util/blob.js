...
async function maskedBlobResponse(s3, filenme, blob) {
  return withEtag(
    blob.md5,
    () => (_, response) => {
...
// By default, this will set the content-dispostion as "attachment" on all files.
// Specific resources (like submission attachments) can use "inline", but only for images.
async function blobResponse(s3, filename, blob, imageInline = false) {
  console.log('blobResponse()', 'blob.s3_status:', blob.s3_status);
  const allowedTypes = ['image/jpeg', 'image/png', 'image/gif'];
  const disposition = (imageInline && allowedTypes.includes(blob.contentType)) ? 'inline' : 'attachment';
  if (blob.s3_status === 'uploaded') {
    ...
    return withEtag(
      blob.md5,
      () => (_, response) => {
        const contentDispositionvalue = contentDisposition(filename, disposition);
        response.set('Content-Disposition', contentDispositionvalue);
        response.set('Content-Type', blob.contentType);
        return blob.content;
      },
    );
...
module.exports = { blobContent, blobResponse, maskedBlobResponse, defaultMimetypeFor, streamBlobs, streamEncBlobs };
