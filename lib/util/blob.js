...
function streamEncBlobs(s3, inStream) {
  return PartialPipe.of(inStream, new Transform({
    ...
    transform({ row: { encS3Status, encMd5, encSha, ...row } }, _, done) {
      ...
      return s3.getContentFor({ md5: encMd5, sha: encSha })
...
function streamBlobs(s3, inStream) {
  return PartialPipe.of(inStream, new Transform({
    ...
    transform({ row }, _, done) {
      const { blobId, content, sha, s3_status } = row;
      const blob = { id: blobId, content, sha, s3_status };

      if (s3_status !== 'uploaded') return done(null, { row: blob }); // eslint-disable-line camelcase

      return s3.getContentFor(blob)
        .then(content => done(null, { row: { ...blob, content } })) // eslint-disable-line no-shadow
        .catch(done);
    },
  }));
}
