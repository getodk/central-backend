...
const traverseXml = (input, initTraversers) => new Promise((resolve) => {
...
const applyTraversal = (filters, inner, traversals, isRoot, e, x, y) => {
  ...
  if (e === 'open')
    ...
    walked = traversals.concat([ new Traversal(0, 0, inner) ]).map((traversal) => {
      ...
      if (traversal.hasMisses) return traversal.miss();
      ...
      return (filters[traversal.re](x, y, isRoot) === true) ? traversal.enter() : traversal.miss();
    });
  else if (e === 'close')
...
// usage: text()
const _text = (e, nodeText) => ((e === 'text') ? Option.of(nodeText) : _text);
const text = () => _text; // so that extern text() API has a paren like the others.
