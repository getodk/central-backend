...
//
// This is, by the way, a form of an Iteratee. We call it a "traverser" because
// the terminology physically describes what is happening better.
//
...
const applyTraversal = (filters, inner, traversals, isRoot, e, x, y) => {
  ...
  if (e === 'open')
    ...
    walked = traversals.concat([ new Traversal(0, 0, inner) ]).map((traversal) => {
  ...
  for (const traversal of walked) {
    ...
    if (traversal.invalid)
      continue;
    else if (traversal.isReal && justOpened) // always start anew:
      result.push(traversal.withData(inner(e, x, y)));
    else if (traversal.isReal && (justClosed || (traversal.re >= filters.length))) // use extant data:
...
const findOne = (...filters) => fix((recurse) => (inner, inTraversals = [], first = true) => (e, x, y) => {
  ...
  for (const traversal of traversals)
    if (traversal.isReal && (typeof traversal.data !== 'function'))
...
const tree = (data = {}, stack) => (e, tagname) => {
  if (e === 'open') {
    if (stack == null) { // bootstrap data once at the start.
      data.name = tagname;
      stack = [ data ];
    } else {
