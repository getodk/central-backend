...
const { exhaustBlobs } = require('../util/s3'); // FIXME need to create a container here...
...
function countFailed() {
  withDb(async (db) => console.log('Failed uploads:', await getCount(db, 'failed')));
}
...
function uploadPending() {
  withDb(async (db) => {
    console.log('Uploading', await getFailedCount(db, 'pending'), 'blobs...');
