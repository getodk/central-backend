...
const queryModuleBuilder = (definition, container) => {
  ...
  for (const methodName of Object.keys(definition)) {
    module[methodName] = (...args) => {
      ...
      if ((fn.audit != null) && (container != null) && (container.context != null) &&
        (container.context.auth != null) && container.context.auth.isAuthenticated) {
        const actor = container.context.auth.actor.orNull();
        const logger = (action, actee, details) => container.Audits.log(actor, action, actee, details);
        return (fn.audit.onResult === true)
          ? wrapped.then(ignoringResult((x) => fn.audit(x)(logger)))
