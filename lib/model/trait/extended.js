...
//
// the Parent fields.all is always used, it is not possible to override this field
// (and nor should it be).
const ExtendedInstance = ({ fields, forApi }) => (Parent) => {
  class Extended extends Parent {}
  if (fields != null) {
    Extended.fields = fields;
    Extended.fields.all = Parent.fields.all;
  }
...
const HasExtended = (partial, propName = 'Extended') => (_, Instance) => {
  const Extended = partial(Instance);
  class HasExtendedTrait {}
  HasExtendedTrait[propName] = Extended;
