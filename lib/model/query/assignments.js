...
values (${actorId}, ${roleId}, ${acteeId})
returning *`)
    .then(construct(Assignment));
};

grant.audit = (actor, role, actee) => (log) => log('assignment.create', actor, { roleId: role.id, grantedActeeId: actee.acteeId });
...
const revoke = (actor, role, actee) => ({ db }) => {
  const actorId = (actor.id == null) ? actor : actor.id;
  const roleId = (role.id == null) ? role : role.id;
  const acteeId = (actee.acteeId == null) ? actee : actee.acteeId;

  return db.query(sql`delete from assignments where ${equals({ actorId, roleId, acteeId })}`)
    .then(({ rowCount }) => Number(rowCount) > 0);
};

revoke.audit = (actor, role, actee) => (log) => log('assignment.delete', actor, { roleId: role.id, revokedActeeId: actee.acteeId });
