...
with
  vals (public, "createdAt") as (values (${key.public}, now())),
...
inner join
  (select "keyId" from form_defs
    inner join
      (select "formDefId" from submission_defs
        where current=true and "localKey" is not null) as submission_defs
        inner join
          (select submissions.id from submissions where draft=true) as submissions
...
module.exports = { ensure, getActiveByFormId, getManagedByIds, getDecryptor };
