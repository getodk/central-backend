...
module.exports = {
  ...
  get: (options = QueryOptions.none) => ({ db, Actor, Audit, Form, Project }) => ((options.extended === false)
    ? db.select('*').from('audits')
      ...
      .modify(ifArg('action', options, (action, db) => db.where({ action })))
      .modify(ifArg('start', options, (start, db) => db.where('loggedAt', '>=', start)))
      .modify(ifArg('end', options, (end, db) => db.where('loggedAt', '<=', end)))
      .orderBy('loggedAt', 'desc')
