...
SELECT ${fields} FROM audits
  ...
  LEFT JOIN (
    ...
    JOIN submission_defs ON submissions.id = submission_defs."submissionId" AND root -- just for the sake of userAgent which we don't need, we should remove it
  ) submissions ON (audits.details->'submissionId')::INTEGER = submissions.id AND action IN ('entity.create', 'entity.update.version')
  ...
  LEFT JOIN audits approval ON (approval.details->'submissionId')::INTEGER = submissions.id AND approval.action = 'submission.update' AND (approval.details->>'reviewState')::TEXT = 'approved'
  LEFT JOIN actors approval_actor ON approval_actor.id = approval."actorId"
...
const getByEntityId = (entityId, options) => ({ all }) => {
  ...
  const _unjoiner = unjoiner(
    ...
    Option.of(Audit.alias('approval', 'approval')), Option.of(Actor.alias('approval_actor', 'approvalActor'))
  );
  ...
  return all(_getByEntityId(_unjoiner.fields, options, entityId))
    ...
    .then(map(audit => {
      ...
      const approval = audit.aux.approval
        .map(a => a.withAux('actor', audit.aux.approvalActor.orNull()))
        .map(a => a.forApi());
