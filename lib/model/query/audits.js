...
const log = (actor, action, actee, details) => ({ run }) => {
  ...
  const processed = actionableEvents.includes(action) ? null : sql`clock_timestamp()`;

  return run(sql`
insert into audits ("actorId", action, "acteeId", details, "loggedAt", processed)
values (${actorId}, ${action}, ${acteeId}, ${(details == null) ? null : JSON.stringify(details)}, clock_timestamp(), ${processed})`);
};
