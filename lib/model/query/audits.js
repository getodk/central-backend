...
const { Actor, Audit, Form, FormDef, Project } = require('../frames');
...
values (${actorId}, ${action}, ${acteeId}, ${sql.json(detail)}, ${processed})`)
    .then(Audit.construct);
};
...
const create = (ca) => ({ one }) => one(insert(ca));
...
const _get = extender
  (Audit)
  (Actor.alias('actor_actor', 'actorActor'), Option.of(Actor), Option.of(Form), Option.of(FormDef), Option.of(Project))
  ((fields, extend, options) => sql`
select * from audits
  ...
  where ${equals(options.condition)} and ${auditFilter(options)}
