...
const { map, mergeLeft, mergeAll } = require('ramda');
...
const actionCondition = (action) => {
  if (action === 'nonverbose')
    ...
    return sql`action in ('form.create', 'form.update', 'form.delete', 'form.restore', 'form.purge', 'form.attachment.update', 'form.submission.export', 'form.update.draft.set', 'form.update.draft.delete', 'form.update.draft.replace', 'form.update.publish', 'form.update.draft.update')`;
    ...
    return sql`action in ('dataset.create', 'dataset.update', 'entity.bulk.delete', 'entity.bulk.restore', 'dataset.delete', 'dataset.update.property.delete')`;
    ...
    return (sql`action in ('config.set')`);
...
${extend|| sql`
  ...
  -- join with datasets to see if the dataset has been soft-deleted
  LEFT JOIN datasets ON datasets.id = entities."datasetId" and datasets."deletedAt" IS NULL
`}
...
const getBySubmissionId = (submissionId, options) => ({ all }) => _getBySubmissionId(all, options, submissionId)
  .then(map(audit => {
    ...
    const entity = mergeAll([
      { entityAvailable: fullEntity.isDefined() && audit.aux.dataset.isDefined() && !audit.aux.dataset.get().deletedAt },
      fullEntity.orElse(undefined),
