const { rowToInstance, rowsToInstances } = require('../../util/db');
...
module.exports = {
  ...
  getById: (table, id, klass) => ({ db }) =>
    db.select('*').from(table).where({ id }).limit(1).then(rowToInstance(klass)),

  getWhere: (table, condition, klass) => ({ db }) =>
    db.select('*').from(table).where(condition).then(rowsToInstances(klass))
};
