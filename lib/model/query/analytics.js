  ...
  FROM datasets ds
    LEFT JOIN (
      SELECT "datasetId"
           ...
           , COUNT(CASE WHEN "createdAt" >= ${_cutoffDate} THEN 1 END) as num_entities_recent
           , COUNT(CASE WHEN "updatedAt" IS NOT NULL          THEN 1 END) AS num_entities_updated_total
           , COUNT(CASE WHEN "updatedAt" >= ${_cutoffDate} THEN 1 END) AS num_entities_updated_recent
        FROM entities
        GROUP BY "datasetId"
    ...
    LEFT JOIN (
      SELECT
        ...
        SUM (CASE WHEN a."details"->'submissionDefId' IS NOT NULL AND a."loggedAt" >= ${_cutoffDate} THEN 1 ELSE 0 END) update_sub_recent,
        SUM (CASE WHEN a."details"->'submissionDefId' IS NULL THEN 1 ELSE 0 END) update_api_total,
        SUM (CASE WHEN a."details"->'submissionDefId' IS NULL AND a."loggedAt" >= ${_cutoffDate} THEN 1 ELSE 0 END) update_api_recent,
        e."datasetId"
      FROM audits a
