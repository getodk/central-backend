...
const checkLoginCustomization = () => ({ all }) =>
  all(sql`SELECT * FROM config WHERE key in ('login-appearance', 'logo', 'hero-image')`)
    .then((rows) => Object.fromEntries(rows.map(r => [r.key, r])))
    .then((configs) => ({
      login_customization_title_set: configs['login-appearance']?.value.title != null ? 1 : 0,
      login_customization_description_set: configs['login-appearance']?.value.description != null ? 1 : 0,
      login_customization_logo_set: configs.logo != null ? 1 : 0,
      login_customization_hero_image_set: configs['hero-image'] != null ? 1 : 0,
    }));
...
const countFormsWebformsEnabled = () => ({ all }) => all(sql`
  ...
  from forms as f
  join form_defs as fd on fd.id = f."currentDefId"
  where fd."webformsEnabled" is true
  group by f."projectId"`);
...
const countMultiEntityErrors = () => ({ oneFirst }) => oneFirst(sql`
SELECT
  COUNT(*)
FROM
  audits
...
const previewMetrics = () => (({ Analytics }) => runSequentially([
  ...
  // 2025.4 number of errors from processing multi entities / entities from repeats
  metrics.system.num_multi_entity_errors = multiEntityErrors || 0;
