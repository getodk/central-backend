...
const maxGeoPerForm = () => ({ oneFirst }) => oneFirst(sql`
  ...
  FROM (
    SELECT fieldhash, count(*) as geo_count
    FROM submission_field_extract_geo_cache 
    GROUP BY "fieldhash"
  ) as geo_counts`);
...
SELECT
  ...
  ds.id
FROM entities AS e
  JOIN ds_properties AS dp ON dp."datasetId" = e."datasetId"
  JOIN datasets AS ds ON ds.id = e."datasetId"
WHERE dp.name = 'geometry'
...
const projectMetrics = () => (({ Analytics }) => runSequentially([
  ...
  for (const row of datasets) {
    ...
    const entitiesWithGeometryRow = entitiesWithGeometry.find(d => (d.projectId === row.projectId && d.id === row.id)) ||
    { total: 0, recent: 0 };
