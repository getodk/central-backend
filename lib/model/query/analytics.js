...
const getDatasets = () => ({ all }) => all(sql`
  SELECT ds.id
       ...
       , COUNT(DISTINCT CASE WHEN entz."createdAt" >= current_date - cast(${DAY_RANGE} as int) THEN e.id END) num_entities_recent
       , COUNT(DISTINCT CASE WHEN entz."updatedAt" IS NOT NULL THEN e.id END) num_entities_updated_total
       , COUNT(DISTINCT CASE WHEN entz."updatedAt" >= current_date - cast(${DAY_RANGE} as int) THEN e.id END) num_entities_updated_recent
  ...
  FROM datasets ds
    LEFT JOIN entities AS entz ON e."datasetId" = ds.id
