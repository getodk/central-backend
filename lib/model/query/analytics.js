...
const getDatasets = () => ({ all }) => all(sql`
  ...
  FROM datasets AS ds
    LEFT JOIN (
      SELECT COUNT(id) AS num_entities_total
    ...
    LEFT JOIN (
      SELECT COUNT(*) AS total
           ...
           , COUNT(a.details->'submissionDefId' IS NOT NULL AND a."loggedAt" >= ${_cutoffDate} OR NULL) update_sub_recent
           , COUNT(a.details->'submissionDefId' IS NULL OR NULL) update_api_total
           , COUNT(a.details->'submissionDefId' IS NULL     AND a."loggedAt" >= ${_cutoffDate} OR NULL) update_api_recent
           , e."datasetId"
        FROM audits AS a
    ...
    LEFT JOIN (
      SELECT COUNT(*) AS total
           ...
           , e."datasetId"
        FROM audits AS a
          JOIN entity_def_sources AS eds ON (a.details->'sourceId')::INT = eds."id"
          JOIN entity_defs        AS ed  ON ed."sourceId" = eds.id AND root=true
          JOIN entities           AS e   ON ed."entityId" = e.id
