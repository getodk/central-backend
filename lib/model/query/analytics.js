...
const countFailedAudits = () => ({ one }) => one(sql`
SELECT
  ...
  -- Unexpectedly unprocessed. It'd be interesting if this differed from failed5.
  count(*) FILTER (WHERE processed IS NULL AND failures = 0 AND "loggedAt" < now() - INTERVAL '1 day') AS unprocessed
FROM audits`);
...
const previewMetrics = () => (({ Analytics }) => Promise.all([
  ...
  metrics.system.num_audits_failed = failedAudits.any_failure;
  metrics.system.num_audits_failed5 = failedAudits.failed5;
