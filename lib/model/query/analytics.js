...
const getDatasets = () => ({ all }) => all(sql`
  SELECT ds.id
       ...
       , COUNT(DISTINCT CASE WHEN entz."createdAt" >= current_date - 45 THEN entz.id END) num_entities_recent
       , COUNT(DISTINCT CASE WHEN entz."updatedAt" IS NOT NULL          THEN entz.id END) num_entities_updated_total
       , COUNT(DISTINCT CASE WHEN entz."updatedAt" >= current_date - 45 THEN entz.id END) num_entities_updated_recent
       , COUNT(DISTINCT fd."formId") num_creation_forms
       , COUNT(DISTINCT fa."formId") num_followup_forms
    ...
    LEFT JOIN (
      SELECT fa."formId"
        ...
        FROM form_attachments AS fa
          JOIN forms f ON f.id = fa."formId" AND f."currentDefId" IS NOT NULL
