...
const getDatasets = () => ({ all }) => all(sql`
  SELECT ds.id
       , ds."projectId"
       , COALESCE(SUM(entz.num_entities_total),          0) AS num_entities_total
       , COALESCE(SUM(entz.num_entities_recent),         0) AS num_entities_recent
       , COALESCE(SUM(entz.num_entities_updated_total),  0) AS num_entities_updated_total
       , COALESCE(SUM(entz.num_entities_updated_recent), 0) AS num_entities_updated_recent
       , MAX(COALESCE(num_creation_forms, 0))        AS num_creation_forms
       , MAX(COALESCE(num_followup_forms, 0))        AS num_followup_forms
       , MAX(COALESCE(errors.total, 0))              AS num_failed_entities_total
       , MAX(COALESCE(errors.recent, 0))             AS num_failed_entities_recent
       , MAX(COALESCE(updates.total, 0))             AS num_entity_updates_total
       , MAX(COALESCE(updates.recent, 0))            AS num_entity_updates_recent
       , MAX(COALESCE(updates.update_sub_total, 0))  AS num_entity_updates_sub_total
       , MAX(COALESCE(updates.update_sub_recent, 0)) AS num_entity_updates_sub_recent
       , MAX(COALESCE(updates.update_api_total, 0))  AS num_entity_updates_api_total
       , MAX(COALESCE(updates.update_api_recent, 0)) AS num_entity_updates_api_recent
       , MAX(COALESCE(conflict_stats.conflicts, 0))  AS num_entity_conflicts
       , MAX(COALESCE(conflict_stats.resolved, 0))   AS num_entity_conflicts_resolved
       , MAX(COALESCE(creates.create_sub_total, 0))  AS num_entity_create_sub_total
       , MAX(COALESCE(creates.create_sub_recent, 0)) AS num_entity_create_sub_recent
       , MAX(COALESCE(creates.create_api_total, 0))  AS num_entity_create_api_total
       , MAX(COALESCE(creates.create_api_recent, 0)) AS num_entity_create_api_recent
       , MAX(COALESCE(bulk_creates.total, 0))        AS num_entity_create_bulk_total
       , MAX(COALESCE(bulk_creates.recent, 0))       AS num_entity_create_bulk_recent
  FROM datasets ds
    LEFT JOIN (
      SELECT "datasetId"
           , COUNT(id) AS num_entities_total
           , COUNT(CASE WHEN "createdAt" >= current_date - 45 THEN 1 END) as num_entities_recent
           , COUNT(CASE WHEN "updatedAt" IS NOT NULL          THEN 1 END) AS num_entities_updated_total
           , COUNT(CASE WHEN "updatedAt" >= current_date - 45 THEN 1 END) AS num_entities_updated_recent
        FROM entities
        GROUP BY "datasetId"
