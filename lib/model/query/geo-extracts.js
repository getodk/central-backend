...
const getSubmissionFeatureGeoNeededCTE = (formPK, fieldPaths, submitterIds, TSTZRange, reviewStates, deleted, limit) => {
...
const getSubmissionFeatureCollectionGeoJsonCount = (formPK, fieldPaths, submitterIds, TSTZRange, reviewStates, deleted, limit) => ({ db }) => {
  const geoNeededCTE = getSubmissionFeatureGeoNeededCTE(formPK, fieldPaths, submitterIds, TSTZRange, reviewStates, deleted, limit);
  return db.one(sql`
    WITH ${geoNeededCTE}
    SELECT
      (SELECT COUNT(*) FROM geo_needed WHERE is_cached AND geovalue IS NOT NULL) as ready,
      (SELECT COUNT(*) FROM geo_needed WHERE NOT is_cached) as todo
    `);
};
...
module.exports = {
  ...
  getSubmissionFeatureCollectionGeoJsonCount,
