...
const getSubmissionFeatureCollectionGeoJson = (formPK, IDs, fieldPaths, submitterIds, TSTZRange, reviewStates, deleted, limit, onlyCurrent=true) => ({ db }) => {

  const idFilter = arrayHasElements(IDs) ? sql`AND sdef."instanceId" = ANY(${sql.array(IDs, 'text')})` : sql``;
  ...
  const reviewStateFilter = arrayHasElements(reviewStates) ? stateFilterToQueryFragments(['sub', 'reviewState'], reviewStates) : sql``;
...
const getEntityFeatureCollectionGeoJson = (datasetPK, IDs, creatorIds, TSTZRange, conflictStates, deleted, limit) => ({ db }) => {
  ...
  const conflictStatusFilter = arrayHasElements(conflictStates) ? stateFilterToQueryFragments(['e', 'conflict'], conflictStates, 'conflictType') : sql``;
  const deletedFilter = deleted ? sql`IS NOT NULL` : sql`IS NULL`;
  const queryLimit = limit ? sql`LIMIT ${limit}` : sql``;
