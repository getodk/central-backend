...
const submissionFilterableFields = [
  ['__id', 'sub.instanceId'],
  ['__versionId', 'sdef.instanceId'],
  ['__system/submissionDate', 'sub.createdAt'],
  ['__system/updatedAt', 'sub.updatedAt'],
  ['__system/submitterId', 'sub.submitterId'],
  ['__system/reviewState', 'sub.reviewState'],
  ['__system/deletedAt', 'sub.deletedAt'],
  ['path', 'ffgeo.path'],
];
const odataSubmissionsToColumnMap = new Map(submissionFilterableFields.concat(submissionFilterableFields.map(f => ([`$root/Submissions/${f[0]}`, f[1]]))));

const odataEntitiesToColumnMap = new Map([
  ['__id', 'e.uuid'],
  ['__system/createdAt', 'e.createdAt'],
  ['__system/updatedAt', 'e.updatedAt'],
  ['__system/deletedAt', 'e.deletedAt'],
  ['__system/creatorId', 'e.creatorId'],
  ['__system/conflict', 'e.conflict']
]);
...
const getSubmissionSelectionEtag = (formPK, odataQuery) => ({ db }) =>
  db.oneFirst(sql`
    ...
    WHERE
        sub."formId" = ${formPK}
        AND
        ${odataFilter(odataQuery, odataSubmissionsToColumnMap)}
        AND
        ${odataExcludeDeleted(odataQuery, odataSubmissionsToColumnMap)}
  `);


const getSubmissionFeatureCollectionGeoJson = (formPK, odataQuery, fieldPaths, limit, onlyCurrent=true, assumeRootSubmissionId=true) => ({ db }) => {
...
const getEntityFeatureCollectionGeoJson = (datasetPK, odataQuery, limit, search) => ({ db }) => {
  ...
  return db.oneFirst(sql`
      WITH extracted AS (
          ...
          WHERE
            ${searchFilter}
            AND
            ${odataFilter(odataQuery, odataEntitiesToColumnMap)}
            AND
            ${odataExcludeDeleted(odataQuery, odataEntitiesToColumnMap)}
          ${queryLimit}
          ),
          featuregeojsoned AS (
              SELECT
