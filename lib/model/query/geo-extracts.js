...
const getSubmissionFeatureCollectionGeoJson = (formPK, fieldPaths, submitterIds, createdAtStart, createdAtEnd, reviewStates, deleted, limit) => ({ db }) => {
  ...
  const createdAtEndFilter = createdAtEnd ? sql`AND sdef."createdAt" < ${createdAtEnd}` : sql``;
...
const getEntityFeatureCollectionGeoJson = (datasetPK, conflicted) => ({ db }) => {

  const conflictStatusFilter = conflicted === undefined ? sql`` : sql`AND e.conflict IS ${conflicted ? sql`NOT` : sql``} NULL`;
