...
// Given Any x, returns true only if x is a FutureQuery of some kind.
const isFutureQuery = (x) => (x != null) && (x.isFutureQuery === true);
...
const maybeEnd = (container, x) =>
  (isFutureQuery(x) ? maybeEnd(container, x.end(container)) : x);
...
module.exports = { isFutureQuery, mappedFutureQuery, futureQueryList, futureQuery };
