...
module.exports = {
  ...
  _get: (condition = []) => ({ db, FieldKey, Actor, Session }) =>
    db.select(fieldsForJoin({
      fieldKey: { table: 'field_keys', fields: FieldKey.fields.all },
      actor: { table: 'actors', fields: Actor.fields.all },
      session: { table: 'sessions', fields: Session.fields.all }
    }))
      .from('field_keys')
      .where(condition)
  ...
  _getExtended: (condition = []) => ({ db, FieldKey, Actor, Session }) =>
    db.select(merge(fieldsForJoin({
      fieldKey: { table: 'field_keys', fields: FieldKey.fields.all },
      actor: { table: 'actors', fields: Actor.fields.all },
      createdBy: { table: 'created_by', fields: Actor.fields.all },
      session: { table: 'sessions', fields: Session.fields.all }
