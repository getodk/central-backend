...
module.exports = {
  create: (form) => ({ db, actees, forms }) =>
    actees.provision('form')
      .then((actee) => db.raw(`
with def as (insert into form_defs ("formId", xml, hash, sha, sha256, version, "keyId", "xlsBlobId", "createdAt")
...
form as (insert into forms (id, name, "xmlFormId", state, "projectId", "currentDefId", "acteeId", "createdAt")
  ...
  returning forms.*)
select id from form;`,
      [ form.def.xml, form.def.hash, form.def.sha, form.def.sha256, form.def.version, form.def.keyId, form.def.xlsBlobId || null,
  ...
  getWhere: (condition, options = QueryOptions.none) => ({ forms, FormDef }) =>
    forms._get(FormDef, options.withCondition(condition)),
  ...
  getByProjectAndXmlFormId: (projectId, xmlFormId, options = QueryOptions.none) => ({ forms, FormDef }) =>
    forms._get(FormDef, options.withCondition({ projectId, xmlFormId })).then(maybeFirst),

  getWithXmlByProjectAndXmlFormId: (projectId, xmlFormId, options = QueryOptions.none) => ({ forms, FormDef }) =>
    forms._get(FormDef.WithXml, options.withCondition({ projectId, xmlFormId })).then(maybeFirst),
  ...
  _get: (DefType, options = QueryOptions.none) => ({ db, Form, Actor }) => ((options.extended === false)
