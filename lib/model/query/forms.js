...
with sch as
  (insert into form_schemas (id)
    ...
    returning *),
def as
  (insert into form_defs ("formId", "schemaId", xml, name, hash, sha, sha256, version, "keyId", "xlsBlobId", "draftToken", "enketoId", "createdAt", "webformsEnabled")
  select nextval(pg_get_serial_sequence('forms', 'id')), sch.id, ${form.xml}, ${def.name}, ${def.hash}, ${def.sha}, ${def.sha256}, ${def.version}, ${def.keyId}, ${form.xls.xlsBlobId || null}, ${def.draftToken || null}, ${def.enketoId || null}, clock_timestamp(), ${def.webformsEnabled}
    from sch
  returning *),
form as
  (insert into forms (id, "xmlFormId", state, "projectId", "draftDefId", "acteeId", "enketoId", "enketoOnceId", "createdAt") -- webformsEnable is set to null implicitly for new draft in forms table
...
const createNew = (partial, project) => async ({ Actees, Datasets, FormAttachments, Forms, Keys }) => {
  ...
  // parsedDatasets also includes warnings and entities-spec version for later validation
  ...
  const savedForm = await Forms._createNew(
    ...
    partial.def.with({ keyId, draftToken, enketoId, webformsEnabled: true }),
    project
  );
  ...
  if (parsedDatasets.isDefined()) {
    ...
    validateEntityRepeatVersion(parsedDatasets.get().version, datasetsAndFields);
    await Promise.all(
...
const createVersion = (partial, form, publish, duplicating = false) => async ({ Datasets, FormAttachments, Forms, Keys }) => {
  ...
  // If there is an existing draft then use its webformsEnabled, otherwise published version.
  // In case of publish is true, then we set webformsEnabled to NULL because its value is already on
  // `forms` table
  const webformsEnabled = publish ? null : (form.def?.webformsEnabled ?? form.webformsEnabled);

  // Save draft def
  const savedDef = await Forms._createNewDef(partial, form, publish, { draftToken, enketoId, schemaId, keyId, webformsEnabled });
...
const publish = (form, concurrentWithCreate = false) => async ({ Forms, Datasets, FormAttachments }) => {
  ...
  const [f, fd] = await Promise.all([
    Forms._update(form, { currentDefId: form.draftDefId, draftDefId: null, webformsEnabled: form.def.webformsEnabled, ...(concurrentWithCreate ? { updatedAt: null } : {}), ...enketoIds }),
    Forms._updateDef(form.def, { draftToken: null, enketoId: null, webformsEnabled: null, publishedAt }),
  ])
  ...
  const dsResult = await Datasets.publishIfExists(form.def.id, publishedAt);
  const datasets = dsResult.filter(ds => ds.action === 'datasetCreated');
  if (datasets.length > 0) {
    await Promise.all(datasets.map(ds => FormAttachments.autoLinkWithDataset(form, ds.id)));
  }
...
const update = (form, data) => async ({ Forms }) => {
  // we don't have currentDefId, it means Form is never published, so we set webformEnabled in the
  // `form_defs` table and other data (only `state` atm) in the `forms` table
  if (!form.currentDefId) {
    const { webformsEnabled, ...otherData } = data;
    if (webformsEnabled !== undefined) {
      await Forms._updateDef(form.def, { webformsEnabled });
    }
    return Forms._update(form, otherData);
  } else {
    return Forms._update(form, data);
  }
};
update.audit = (form, data) => (log) => log('form.update', form, { data });

const _updateDef = (formDef, data) => ({ one }) => one(updater(formDef, data));
const updateDef = (form, formDef, data) => async ({ Forms, run }) => {
  await Forms._updateDef(formDef, data);
  await run(sql`UPDATE forms SET "updatedAt" = clock_timestamp() WHERE id = ${form.id}`);
};
updateDef.audit = (form, formDef, data) => (log) => log('form.update.draft.update', form, { data });

const del = (form) => ({ run }) =>
  run(markDeleted(form));
del.audit = (form, relatedDatasets) => (log) => log('form.delete', form, relatedDatasets);
...
const checkStructuralChange = (existingFields, fields) => ({ context }) => {
  ...
  const removedFields = existingFields.filter(f => !newFieldSet.has(f.path))
    .filter(f => (
      ...
      !f.path.endsWith('/meta/entity') &&
      !f.path.endsWith('/meta/entity/label')
    )); // Don't warn about changed meta/entity blocks
...
module.exports = {
  ...
  publish, clearDraft,
  _update, update, _updateDef, updateDef, replaceDef, del, restore, purge,
  clearUnneededDrafts,
  setManagedKey,
