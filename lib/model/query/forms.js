...
select id from form;`,
  ...
  getVersions: (formId, options = QueryOptions.none) => ({ db, Actor, Form, FormDef }) => ((options.extended === false)
    ...
    : withJoin('form', { form: Form.ExtendedVersion, def: FormDef, publishedBy: Option.of(Actor) }, (fields, unjoin) =>
      db.select(fields)
        ...
        .leftOuterJoin(
          db.select({ id: 'id', excelContentType: 'contentType' }).from('blobs').as('xls'),
          'form_defs.xlsBlobId', 'xls.id'
        )
        .orderBy('publishedAt', 'desc')
        .then(map(unjoin)))),

  getByAuthForOpenRosa: (projectId, auth, options = QueryOptions.none) => ({ db, Form, FormDef }) =>
    withJoin('form', { form: Form, def: FormDef }, (fields, unjoin) =>
      db.select(merge(fields, { 'form!hasAttachments': 'hasAttachments' }))
        ...
        .modify(ifArg('formID', options, (xmlFormId, chain) => chain.where({ xmlFormId })))
  ...
  // TODO: temporarily copy+pasta from _get below pending better resolution
  // of this transaction nightmare. (also, vestigial options)
  getByActeeIdForUpdate: (acteeId, options, version) => ({ db, Form, FormDef }) =>
    withJoin('form', { form: Form, def: FormDef }, (fields, unjoin) =>
      db.select(fields)
        .from('forms')
        .forUpdate()
        .where({ acteeId, deletedAt: null })
        .join(
          db.select('*').from('form_defs').as('form_defs'),
  ...
  getAllSubmitters: (formId) => ({ db, Actor }) =>
    db.select('actors.*')
      .from('actors')
      .innerJoin(
        db.select('submitterId').from('submissions')
          .where({ deletedAt: null, formId })
          .groupBy('submitterId')
          .as('submitters'),
        'submitters.submitterId', 'actors.id'
      )
      .orderBy('actors.displayName', 'asc')
      .then(rowsToInstances(Actor))
