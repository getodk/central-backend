...
const _createNew = (form, publish) => ({ oneFirst, Actees, Forms }) =>
...
with def as
  ...
  values (nextval(pg_get_serial_sequence('forms', 'id')), ${form.def.xml}, ${form.def.hash}, ${form.def.sha}, ${form.def.sha256}, ${form.def.version}, ${form.def.keyId}, ${form.xls.xlsBlobId || null}, ${form.def.draftToken || null}, now(), ${Option.of(form.def.publishedAt).map((d) => d.toISOString()).orNull()})
  returning *),
form as
  ...
  returning forms.*)
select id from form`))
    .then(() => Forms.getByProjectAndXmlFormId(form.projectId, form.xmlFormId, false,
...
const createNew = (partial, publish = false) => ({ run, Blobs, Forms, Keys }) => {
  ...
  return Promise.all([
    ...
    .then(([ keyId, fields ]) => Promise.all([
      Forms._createNew(partial.auxWith('def', Object.assign({ keyId }, additional)), publish),
