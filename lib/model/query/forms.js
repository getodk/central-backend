...
const createVersion = (partial, form, publish, duplicating = false) => async ({ Datasets, FormAttachments, Forms, Keys }) => {
  ...
  // Save draft def
  // In production, form.associated should never be null but in testing code we call Form.createVersion directly
  const webformsEnabled = keyId ? false : (form.associated?.webformsEnabled ?? DEFAULT_WEBFORMS_ENABLED);
...
const _getSql = ((fields, extend, options, defVersion, deleted = false, actorId) => {
  ...
  return sql`
    ...
    LEFT JOIN (
      SELECT forms.id, form_defs."webformsEnabled" FROM forms
      JOIN form_defs ON form_defs.id = ${defVersion === Form.DraftVersion ? sql`forms."draftDefId"` : sql`coalesce(forms."currentDefId", forms."draftDefId")`}
    ) AS "formAssociated" ON "formAssociated".id = forms.id
${extend|| sql`
  left outer join
...
const _get = (exec, options, defVersion, xml, deleted, actorId) => {
  ...
  const frames = [ Form, Form.Associated ];
