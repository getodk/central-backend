...
const _getDeletedAt = (projectId, xmlFormName) => ({ maybeOne }) => maybeOne(sql`
  SELECT * FROM forms WHERE "projectId"=${projectId} AND "xmlFormId"=${xmlFormName}
`);
const assertNotDeleted = (projectId, xmlFormName) => async ({ Forms }) => {
  const maybeForm = await Forms._getDeletedAt(projectId, xmlFormName);
...
const assertSoftDeleted = (projectId, xmlFormName) => async ({ Forms }) => {
  const maybeForm = await Forms._getDeletedAt(projectId, xmlFormName);
  assert.equal(isEmpty(maybeForm), false, 'Form has been hard-deleted or never existed.');
  const { deletedAt } = maybeForm.get();
  assert.ok(deletedAt, 'Form exists but has not been marked as deleted.');
};
const assertHardDeleted = (projectId, xmlFormName) => async ({ Forms }) => {
  const maybeForm = await Forms._getDeletedAt(projectId, xmlFormName);
...
module.exports = {
  ...
  _getDeletedAt, assertHardDeleted, assertNotDeleted, assertSoftDeleted,
};
