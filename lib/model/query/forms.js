...
const versionJoinCondition = (version, enketoId) => {
  ...
  else if (version === Form.NoDefRequired) throw new Error('No JOIN should be performed if NoDefRequired');
...
const _getSql = ((fields, extend, options, version, deleted = false, actorId) => {
  ...
  if (actorId != null) {
    if (version === Form.NoDefRequired) throw new Error('Must request def when actorId is provided');
  }
  ...
  return sql`
    SELECT ${
      ...
      version === Form.NoDefRequired ? sql`${fields} FROM forms` :
...
const _get = (exec, options, xml, version, deleted, actorId) => {
  if (version === Form.NoDefRequired && xml === true) throw new Error('Cannot request xml without a form def');
