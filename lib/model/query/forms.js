...
const createNew = (partial, project, publish = false) => async ({ run, Datasets, FormAttachments, Forms, Keys }) => {
  ...
  const fields = await getFormFields(partial.xml);
  const dataset = await (partial.aux.key.isDefined() ? resolve(Option.none()) : getDataset(partial.xml)); // Don't parse dataset schema if Form has encryption key
  ...
  await run(insertMany(fields.map((field) => new Form.Field(Object.assign(field, ids)))));
...
const createVersion = (partial, form, publish = false) => async ({ run, one, Datasets, FormAttachments, Forms, Keys }) => {
  ...
  const fields = await getFormFields(partial.xml);
  const dataset = await (partial.aux.key.isDefined() ? resolve(Option.none()) : getDataset(partial.xml));
  ...
  const savedDef = await partial.aux.key.map(Keys.ensure).orElse(resolve(null))
