...
const { PURGE_DAY_RANGE, DEFAULT_WEBFORMS_ENABLED } = require('../../util/constants');
...
const createNew = (partial, project) => async ({ Actees, Datasets, FormAttachments, Forms, Keys }) => {
  ...
  const webformsEnabled = keyId ? false : DEFAULT_WEBFORMS_ENABLED;

  // Save draft form
  const savedForm = await Forms._createNew(
    partial.with({ acteeId }),
    partial.def.with({ keyId, draftToken, enketoId, webformsEnabled }),
    project
  );
...
const updateDef = (form, formDef, data) => ({ Forms }) => Forms._updateDef(formDef, data);
