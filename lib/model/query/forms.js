...
form as
  ...
  select def."formId", ${form.xmlFormId}, ${form.state || 'open'}, ${project.id}, def.id, ${form.acteeId}, ${form.enketoId || null}, ${form.enketoOnceId || null}, def."createdAt", ${form.webformsEnabled || DEFAULT_WEBFORMS_ENABLED} from def
  returning forms.*)
select id from form`)
...
const versionJoinCondition = (defVersion, enketoId) => {
  ...
  else if (defVersion === Form.EmptyVersion) return versionJoinCondition('');
