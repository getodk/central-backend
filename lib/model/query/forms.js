...
const createNew = (partial, project, publish = false) => ({ run, FormAttachments, Forms, Keys }) =>
  Promise.all([
    ...
    .then(([ keyId, fields ]) => Forms._createNew(partial, partial.def.with({ keyId }), project, publish)
      .then((savedForm) => {
        const keys = { formId: savedForm.id, formDefId: savedForm.def.id };
        return Promise.all([
          FormAttachments.createNew(partial.xml, savedForm, partial.xls.itemsets),
          run(insertMany(fields.map((field) => new Form.Field(Object.assign(field, keys)))))
        ])
          .then(always(savedForm));
      }));

createNew.audit = (form, partial, _, publish) => async (log) => {
  await log('form.create', form);
  // if we are asked to publish right away, log that too.
  if (publish === true)
    await log('form.update.publish', form, { newDefId: form.currentDefId });
};
