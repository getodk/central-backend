...
with def as
  ...
  values (nextval(pg_get_serial_sequence('forms', 'id')), ${form.xml}, ${def.hash}, ${def.sha}, ${def.sha256}, ${def.version}, ${def.keyId}, ${form.xls.xlsBlobId || null}, ${(publish !== true) ? generateToken() : null}, now(), ${(publish === true) ? sql`now()` : null})
  returning *),
form as
  ...
  return Promise.all([
    ...
    partial.aux.key.map(Keys.ensure).orElse(resolve(null))
      ...
      .then(ignoringResult((savedDef) => ((publish === true)
        ? Forms.update(form, { currentDefId: savedDef.id })
        : Forms.update(form, { draftDefId: savedDef.id })))),
...
createVersion.audit = (partial, form, _, publish) => (log) => ((publish === true)
  ? log('form.update.publish', form, { newDefId: form.currentDefId })
  : log('form.update.draft.set', form));
...
const update = (form, data) => ({ one }) => one(updater(form, data));
update.audit = (form, data) => (log) => log('form.update', form, { data });

const updateDef = (form, data) => ({ one }) => one(updater(form.def, data));
