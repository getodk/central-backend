...
const setManagedKey = (form, key, suffix) => ({ Forms }) => {
  ...
  if (form.draftDefId != null)
    work = work.then(() =>
      Forms.getByProjectAndXmlFormId(form.projectId, form.xmlFormId, Form.DraftVersion, true)
...
const versionJoinCondition = (version, enketoId) => {
  if (enketoId) {
    if (version != null) throw new Error('Should not provide both enketoId and version; got:', { enketoId, version });
    return sql`
  ...
  else if (version === Form.WithoutDef) throw new Error('No JOIN should be performed if WithoutDef');
...
const _getByActeeId = ({ forUpdate, maybeOne, acteeId, version }) => {
  const selectables = version === Form.WithoutDef ?
    sql`* FROM forms` :
  ...
  if (version === Form.WithoutDef) return maybeOne(query).then(map(construct(Form)));
  else return maybeOne(query).then(map(_unjoiner));
};
const getByActeeIdForUpdate = (acteeId, version) => ({ maybeOne }) => _getByActeeId({ forUpdate: true, maybeOne, acteeId, version });
const getByActeeId = (acteeId, version) => ({ maybeOne }) => _getByActeeId({ maybeOne, acteeId, version });
...
const _getSql = ((fields, extend, options, version, deleted = false, actorId) => {
  ...
  if (actorId != null) {
    if (version === Form.WithoutDef) throw new Error('Must request def when actorId is provided');
  }
  ...
  return sql`
    SELECT ${
      ...
      version === Form.WithoutDef ? sql`${fields} FROM forms` :
...
ORDER BY ${version === Form.WithoutDef ? sql`"xmlFormId"` : sql`coalesce(form_defs.name, "xmlFormId")`} ASC`;
});
/* eslint-enable indent */

const _get = (exec, options, version, xml, deleted, actorId) => {
  if (version === Form.WithoutDef && xml === Form.IncludeXml) throw new Error('Cannot request xml without a form def');
  if (xml !== Form.IncludeXml && xml !== Form.WithoutXml) throw new Error('Must use Form.WithoutXml or Form.IncludeXml');

  const frames = [ Form ];
  if (version !== Form.WithoutDef) frames.push(Form.Def);
...
const getByProjectId = (auth, projectId, version, xml = Form.WithoutXml, options = QueryOptions.none, deleted = false) => ({ all }) =>
  _get(all, options.withCondition({ projectId }), version, xml = Form.WithoutXml, deleted = false, auth.actor.map((actor) => actor.id).orElse(-1));
const getByProjectAndXmlFormId = (projectId, xmlFormId, version, xml = Form.WithoutXml, options = QueryOptions.none, deleted = false) => ({ maybeOne }) =>
  _get(maybeOne, options.withCondition({ projectId, xmlFormId }), version, xml, deleted);
const getByEnketoId = (enketoId, xml, options = QueryOptions.none, deleted = false) =>
  ({ maybeOne }) => _get(maybeOne, options.withCondition({ enketoId }), xml, null, deleted);

const getByProjectAndNumericId = (projectId, id, version, xml = Form.WithoutXml, options = QueryOptions.none, deleted = false) => ({ maybeOne }) =>
  _get(maybeOne, options.withCondition({ projectId, 'forms.id': id }), version, xml, deleted);
const getAllByAuth = (auth, options = QueryOptions.none) => ({ all }) =>
  _get(all, options, false, Form.AnyVersion, false, auth.actor.map((actor) => actor.id).orElse(-1));
