...
const versionJoinCondition = (defVersion, enketoId) => {
  ...
  else if (defVersion === Form.DraftAndPublishedVersion)
                                                 return sql`form_defs.id=forms."draftDefId" OR form_defs.id=forms."currentDefId"`; // eslint-disable-line no-multi-spaces
...
const getDraftAndPublishedVersions = (formId) => ({ all }) => {
  const _unjoiner = unjoiner(Form, Form.Def, Form.Xml);

  const query = sql`
    SELECT ${_unjoiner.fields}
    FROM forms
    JOIN form_defs ON ${versionJoinCondition(Form.DraftAndPublishedVersion)}
    WHERE "formId" = ${formId} AND forms."deletedAt" IS NULL
    ORDER BY "publishedAt" DESC NULLS LAST
  `;

  return all(query).then(map(_unjoiner));
};
...
const createAndPublishDatasetsOnRestore = (form) => async ({ Forms, Datasets }) => {
  const draftAndPublishedForms = await Forms.getDraftAndPublishedVersions(form.id);


  /* eslint-disable no-await-in-loop */
  // There are two forms only and we really want to run this code sequentially for the sake of simplicity
  for (const formVersion of draftAndPublishedForms) {
    const [fields, parsedDatasets] = await _parseFormXml(formVersion.withAux('key', Option.of(formVersion.aux.def.keyId)));

    // We don't need to do any validation because the Form was published in the past and must be valid
    if (parsedDatasets.isDefined()) {
      const datasetsAndFields = matchFieldsWithDatasets(parsedDatasets.get().datasets, fields);
      await Promise.all(
        datasetsAndFields.map(ds =>
          Datasets.createOrMerge(ds.dataset, formVersion, ds.fields)
        )
      );
      if (formVersion.aux.def.publishedAt) {
        await Datasets.publishIfExists(formVersion.def.id);
      }
    }
  }
...
const getUnpublishedDatasetsAndProps = (form) => async ({ Datasets }) => {
  const [fields, parsedDatasets] = await _parseFormXml(form.withAux('key', Option.of(form.aux.def.keyId)));
  if (!parsedDatasets.isDefined()) return [];

  const datasetsAndFields = matchFieldsWithDatasets(parsedDatasets.get().datasets, fields);
  const existingDatasets = await Datasets.getListWithProperties(form.projectId, datasetsAndFields.map(d => d.dataset.name));

  const existingDatasetMap = new Map(existingDatasets.map(ds => [ds.name, { ...ds, properties: new Set(ds.properties.map(p => p.name)) }]));

  // Diff: find new datasets and new properties
  const datasetAndPropStatus = datasetsAndFields.map(({ dataset: datasetInForm, fields: associatedFields }) => {
    const existingDataset = existingDatasetMap.get(datasetInForm.name);

    return {
      name: datasetInForm.name,
      isNew: !existingDataset,
      properties: associatedFields.map(field => ({ name: field.propertyName, isNew: !existingDataset ? true : !existingDataset.properties.has(field.propertyName) }))
    };
  });

  return datasetAndPropStatus.filter(ds => ds.isNew || ds.properties.some(p => p.isNew));
};
...
module.exports = {
  ...
  getDraftAndPublishedVersions, createAndPublishDatasetsOnRestore, getUnpublishedDatasetsAndProps
};
