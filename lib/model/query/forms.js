...
const { merge, map, compose, identity, always, construct } = require('ramda');
const { Frame, into, readable } = require('../frame');
    ...
    .then(([ { xml, itemsets, warnings }, blob ]) =>
      (((warnings.length > 0) && !ignoreWarnings)
        ...
        : Promise.all([ Form.fromXml(xml), Blobs.ensure(blob) ])
          .then(([ partial, savedBlob ]) => partial.withAux('xls', { xlsBlobId: savedBlob.id, itemsets }))));
...
const _createNew = (form, publish) => ({ one, oneFirst, Actees }) =>
...
with def as
  ...
  values (nextval(pg_get_serial_sequence('forms', 'id')), ${form.def.xml}, ${form.def.hash}, ${form.def.sha}, ${form.def.sha256}, ${form.def.version}, ${form.def.keyId}, ${form.xls.xlsBlobId}, ${form.def.draftToken}, now(), ${form.def.publishedAt})
  returning *),
form as
  ...
  returning forms.*)
select id from form`))
    .then((formId) => _get(one, QueryOptions.condition({ 'forms.id': formId }), false, // eslint-disable-line no-use-before-define
      (publish === true) ? undefined : Form.DraftVersion()));

const createNew = (partial, publish = false) => ({ run, Blobs, Forms, Keys }) => {
  const additional = (publish === true) ? { publishedAt: new Date() } : { draftToken: generateToken() };

  return Promise.all([
    partial.def.key.map(Keys.ensure).orElse(resolve(null)),
    getFormFields(partial.def.xml)
  ])
    .then(([ keyId, fields ]) => Promise.all([
      Forms._createNew(partial.append('def', partial.def.with(Object.assign({ keyId }, additional))), publish),
      expectedFormAttachments(partial.def.xml)
    ])
      .then(itemsetsHack(Blobs, partial.aux.xls.itemsets, 1))
...
const createVersion = (partial, form, publish = false) => ({ run, one, Blobs, Forms, FormAttachments, Keys }) => {
  ...
  return Promise.all([
    ...
    .then(itemsetsHack(Blobs, partial.aux.xls.itemsets, 1, 2))
...
const publish = (form) => ({ Forms, FormDefs }) => {
...
const _openRosaJoiner = unjoiner(Form, Form.Def, Frame.define(into('openRosa', 'hasAttachments', readable)));
...
const _getVersions = extender(Form, Form.Def)(Form.Extended)((fields, extend, options, formId) => sql`
...
const _getExtendedWithoutXml = unjoiner(Form, Form.Extended, Form.Def);
const _getWithXml = unjoiner(Form, Form.Def, Form.Xml);
const _getExtendedWithXml = unjoiner(Form, Form.Extended, Form.Def, Form.Xml);
