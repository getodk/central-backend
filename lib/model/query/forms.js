...
const versionJoinCondition = (version, enketoId) => {
  if (enketoId) {
    if (version != null) console.log(new Error(`Should not provide both enketoId and version; got: ${JSON.stringify({ enketoId, version })}`)) || process.exit(1); // eslint-disable-line no-console, no-unused-expressions
    return sql`
  ...
  if (version == null) console.log(new Error('Must request a specific version or version class')) || process.exit(1); // eslint-disable-line no-console, no-unused-expressions
  else if (version === Form.NoDefRequired) console.log(new Error('No JOIN should be performed if NoDefRequired')) || process.exit(1); // eslint-disable-line no-console, no-unused-expressions
...
const _getByActeeId = ({ forUpdate, maybeOne, acteeId, version }) => {
  ...
  if (version === Form.NoDefRequired) return maybeOne(query);
