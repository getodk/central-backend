...
with def as
  ...
  values (nextval(pg_get_serial_sequence('forms', 'id')), ${def.xml}, ${def.hash}, ${def.sha}, ${def.sha256}, ${def.version}, ${def.keyId}, ${form.xls.xlsBlobId || null}, ${(publish !== true) ? generateToken() : null}, now(), ${(publish === true) ? sql`now()` : null})
  returning *),
form as
