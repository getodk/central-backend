...
const versionJoinCondition = (version, enketoId) => {
  if (enketoId) {
    if (version != null) console.log(`Should not provide both enketoId and version; got: ${JSON.stringify({ enketoId, version })}`) || process.exit(1);
    return sql`
  ...
  if (version == null) console.log('Must request a specific version or version class') || process.exit(1);
  else if (version === Form.NoDefRequired) console.log('No JOIN should be performed if NoDefRequired') || process.exit(1);
...
const _getSql = ((fields, extend, options, version, deleted = false, actorId) => {
  console.log('_getSql()', { fields, extend, options, version, deleted, actorId });
...
ORDER BY ${version === Form.NoDefRequired ? sql`"xmlFormId"` : sql`coalesce(form_defs.name, "xmlFormId")`} ASC`;
  console.log('_getSql() query:', query);
  return query;
});
