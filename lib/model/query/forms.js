...
module.exports = {
  create: (form) => ({ db, actees, forms }) =>
    actees.provision('form')
      .then((actee) => db.raw(`
with def as (insert into form_defs ("formId", xml, hash, sha, sha256, version, "createdAt")
...
form as (insert into forms (id, name, "xmlFormId", state, "projectId", "currentDefId", "acteeId", "createdAt")
  ...
  returning forms.*)
select id from form;`,
      [ form.def.xml, form.def.hash, form.def.sha, form.def.sha256, form.def.version,
  ...
  getForOpenRosa: (condition = {}) => ({ db, Form, FormDef }) =>
    db.select(merge(
      fieldsForJoin({ form: Form, def: FormDef }),
  ...
  _get: (options = QueryOptions.none) => ({ db, Form, Actor, FormDef }) => ((options.extended === false)
    ? db.select(fieldsForJoin({ form: Form, def: FormDef }))
      .from('forms')
    ...
    : db.select(fieldsForJoin({ form: Form.Extended, def: FormDef, createdBy: Actor }))
      .from('forms')
      .where(options.condition)
      .leftOuterJoin(
        db.select('*').from('form_defs').as('form_defs'),
