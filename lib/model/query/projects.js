...
const { withJoin, rowsToInstances, maybeFirst, QueryOptions } = require('../../util/db');
...
module.exports = {
  ...
  _get: (options = QueryOptions.none) => ({ db, Project }) => ((options.extended === false)
    ...
    : db.select('*')
      ...
      .leftOuterJoin(
        db.select(db.raw('"acteeId" as "fkActeeId", count(*)::integer as "appUsers"')).from('assignments')
          .innerJoin(
            db.select('*').from('field_keys')
              .innerJoin(
                db.select('id').from('actors').where({ deletedAt: null }).as('actors'),
                'actors.id', 'field_keys.actorId'
              )
              .as('field_keys'),
            'field_keys.actorId', 'assignments.actorId'
          )
          .groupBy('acteeId')
          .as('assignments'),
        'assignments.fkActeeId', 'projects.acteeId'
      )
      .orderBy(db.raw('coalesce(archived, false)'), 'asc')
      .orderBy('name')
