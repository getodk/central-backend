...
const { sql } = require('ramda');
const { extender, equals, insert, update, markDeleted, QueryOptions } = require('../../util/db');
...
const update = (project) => ({ one }) => one(update(project));

const del = (project) => ({ run }) => run(markDeleted(project));

const setManagedEncryption = (project, passphrase, hint) => ({ FormDefs }) => {
...
const _get = extender(Project)(Project.Extended)((fields, extend, options) => sql`
