...
const { rowsToInstances, maybeFirst, QueryOptions } = require('../../util/db');
...
module.exports = {
  ...
  // TODO: this query pattern exists only in one other place (query/forms), and
  // the two are just different enough to evade refactoring for now.
  ...
  getAllByAuth: (auth, options) => ({ db, projects }) => projects._get(options.withModify((query) =>
    query.innerJoin( // self-join against projects to group/dedupe ids before joining up
      db.select('id').from('projects') // match assignment acteeIds to projects
        .innerJoin(
          db.select('acteeId').from('assignments') // get all assignments against those roles
            ...
            //.groupBy('acteeId') // TODO: this seemed superfluous so i commented it out.
            .as('assignment'),
  ...
  getFormsWithXml: (projectId, options) => ({ forms }) =>
    forms.getWithXmlWhere({ projectId }, options),

  getFormByXmlFormId: (projectId, xmlFormId, options = QueryOptions.none) => ({ forms }) =>
    forms.getWhere({ projectId, xmlFormId }, options).then(maybeFirst),
  ...
  _get: (options = QueryOptions.none) => ({ db, Project }) => ((options.extended === false)
    ...
    : db.select('*')
      ...
      .leftOuterJoin(
        db.select(db.raw('"projectId", count("actorId")::integer as "appUsers"'))
          .from('field_keys')
          .innerJoin(db.select('id').from('actors').where({ deletedAt: null }).as('actors'),
            'actors.id', 'field_keys.actorId')
          .groupBy('projectId')
          .as('field_keys'),
        'field_keys.projectId', 'projects.id'
      )
      .orderBy(db.raw('coalesce(archived, false)'), 'asc')
      .orderBy('name')
      .then(rowsToInstances(Project.Extended)))
};
