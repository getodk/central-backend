...
const DEFAULT_SESSION_LIFETIME = 24 * 60 * 60;

const create = (actor, expiresAt) => ({ one }) => {
  const SESSION_LIFETIME = parseInt(process.env.ODKCENTRAL_SESSION_LIFETIME, 10) || config.sessionLifetime || DEFAULT_SESSION_LIFETIME;
  const expirySqlFragment = (expiresAt instanceof Date ? sql`${expiresAt.toISOString()}` : sql`statement_timestamp() + ${SESSION_LIFETIME + ' s'}::interval`);
  return one(sql`
