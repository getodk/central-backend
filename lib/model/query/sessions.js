...
module.exports = {
  ...
  getByActorId: (actorId) => ({ simply, Session }) =>
    simply.getWhere('sessions', [ { actorId }, [ 'expiresAt', '>', 'now()' ] ], Session),

  deleteByActorId: (actorId) => ({ db }) =>
    db('sessions').delete().where({ actorId: actorId }).then(wasUpdateSuccessful)
};
