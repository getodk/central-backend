...
const getByBearerToken = (token) => async ({ maybeOne }) => {
  if (!isValidToken(token)) return Option.none();

  const maybeRes = await maybeOne(sql`
  ...
  if (maybeRes.isEmpty()) return Option.none();

  const { csrf, ...actorProps } = maybeRes.get();

  const actor = new Actor(actorProps);
  const session = new Session({ csrf }, { actor });
