...
values (${actor.id}, ${generateToken()}, ${generateToken()}, now(), ${expiresAt.toISOString()})
returning *`)
...
terminate.audit = (session) => (log) => {
  ...
  if ((session.actor != null) && (session.actor.type === 'user')) return;
  return log('session.end', session.actor);
};

const reap = () => ({ run }) =>
  run(sql`delete from sessions where "expiresAt" < now()`);

module.exports = { create, getByBearerToken, deleteByActorId, terminate, reap };
