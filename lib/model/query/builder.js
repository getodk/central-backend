...
const { merge, identity, reject } = require('../../util/util');


// The general premise behind this entire query system is as follows:
//
// Query definitions are given in modules containing simple named functions of
// the signature: (x, y, z) => (container) => Promise
//
// The queryBuilder takes these modules and wraps each simple function such that
// when called with the first set of arguments (x, y, z), a FutureQuery is returned
// wrapping that operation. The query is not performed, but FutureQuery is a valid
// Thenable, allowing .then() chaining.
//
...
// queryBuilder is the only exposed interface; it performs the shuffling necessary
// to expose an interface matching the definition modules but returning wrapped
// FutureQuerys instead of plain Promises.
const queryBuilder = (module, container) => {
  // By default, no transaction is assumed.
  const result = { _transacting: false };

  // First wrap and decorate all the provided named functions.
  for (const methodName in module) {
    result[methodName] = function(...args) {
      return futureQuery(module[methodName](...args), container, this._transacting);
    };
  }

  // Now allow for .transacting to be chained in, and set up the context for it.
  result.transacting = merge(result, { _transacting: true });
