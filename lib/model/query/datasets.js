...
LEFT JOIN (
  entity_defs ed JOIN entities e ON e.id =  ed."entityId" AND e."datasetId" = ${datasetId}
) ON ed."submissionDefId" = sd.id
...
const getUnprocessedSubmissions = (datasetId) => ({ all }) =>
  all(_unprocessedSubmissions(datasetId, sql`sd.id "submissionDefId", s.id "submissionId", s."reviewState", f."acteeId" "formActeeId"`));
