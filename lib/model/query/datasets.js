...
// Returns a Map of datasetId -> { approvalRequired, filteredByAuth, lastAudit, lastEntityUpdate, entities, filterRules }
...
// filterRules is a JSON array of { datasetProperty, actorProperty, value } objects for property-filtered datasets,
//   or null if no property filter rules exist. The value is the actor's current value for that actor property.
const _fetchActivityStats = (all, datasetIds, filterActorId = null) => {
  ...
  if (filterActorId != null) fields.push('lastEntityUpdate', 'entities', 'filterRules');
  ...
  return all(sql`
    ...
    LEFT OUTER JOIN (
      SELECT datasets.id, MAX("loggedAt") AS "lastAudit",
        ${filterActorId == null ? sql`FALSE` : sql`("ownerOnly" OR EXISTS (SELECT 1 FROM dataset_access_filters WHERE "datasetId" = datasets.id))`} AS "filteredByAuth"
      FROM audits
      JOIN datasets ON audits."acteeId" = datasets."acteeId"
      ${filterActorId == null ? sql`` : sql`
        WHERE
          (NOT datasets."ownerOnly" AND NOT EXISTS (SELECT 1 FROM dataset_access_filters WHERE "datasetId" = datasets.id)) OR
          audits.action IN ('dataset.update', 'dataset.update.property.delete')
      `}
    ...
    ${filterActorId == null ? sql`` : sql`
      LEFT OUTER JOIN (
        ...
        WHERE
          ...
          AND (
            -- ownerOnly: only consider entities created by this actor
            (datasets."ownerOnly" AND entities."creatorId" = ${filterActorId})
            OR
            -- property filters: only consider entities matching this actor's property values
            EXISTS (
              ...
              JOIN entity_defs ef ON ef."entityId" = entities.id AND ef.current = true
              WHERE daf."datasetId" = entities."datasetId"
                AND apv."actorId" = ${filterActorId}
                AND ef.data ->> dp."name" = apv."value"
            )
          )
        GROUP BY datasets.id
...
const _computeHashResult = (stats, filterActorId) => {
  ...
  const hashInput = JSON.stringify({
    ...
    filterRules: stats.filteredByAuth ? (stats.filterRules ?? null) : undefined,
