...
const { extender, insert, QueryOptions, sqlAnd, sqlEquals, unjoiner, updater, markDeleted } = require('../../util/db');
...
const _trashedFilter = (force, projectId, datasetName) => {
  ...
  return (force
    ? sql`datasets."deletedAt" IS NOT NULL ${sqlAnd(idFilters)}`
    : sql`datasets."deletedAt" < CURRENT_DATE - CAST(${PURGE_DAY_RANGE} AS INT) ${sqlAnd(idFilters)}`);
};
