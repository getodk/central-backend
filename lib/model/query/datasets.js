...
const getMetadata = (dataset) => async ({ all, Datasets }) => {
  ...
  const properties = await Datasets.getProperties(dataset.id, true, true);
  ...
  for (const property of properties) {
    // Special handling for system properties starting with '__'
    if (property.name.startsWith('__')) {
      if (property.name === '__repeat') {
        // If there is a __repeat property, we want to augment the source form
        // of that property with the name of the repeat group.

        // Find the source form with matching xmlFormId
        const sourceForm = sourceForms.find(f => f.xmlFormId === property.aux.xmlFormId);
        if (sourceForm) {
          // Set repeat group to the last segment of the property path, which is the field name
          sourceForm.repeatGroup = property.path.split('/').pop();
        }
      }
    } else {
...
const getFieldsByDataset = (formDefId) => async ({ Datasets }) => {
  const fields = await Datasets.getFieldsByFormDefId(formDefId);
  // Group fields by datasetId
  return fields.reduce((acc, field) => {
    if (!field.datasetId) return acc;
    if (!acc[field.datasetId]) acc[field.datasetId] = [];
    acc[field.datasetId].push(field);
    return acc;
  }, {});
};
...
module.exports = {
  ...
  getProperties, getFieldsByFormDefId, getFieldsByDataset,
