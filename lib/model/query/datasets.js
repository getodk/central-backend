...
const _insertDatasetDef = (dataset, acteeId, metadata) => sql`
  ...
  ds_defs AS (
    INSERT INTO dataset_form_defs ("datasetId", "formDefId", "actions", "rootPath", "isRepeat")
    SELECT ds.id, ${dataset.aux.formDefId}, ${JSON.stringify(metadata.actions)}, ${metadata.rootPath}, ${metadata.isRepeat} FROM ds
...
WHERE form_defs."id" = ${formDefId}
  AND (ds_properties."id" IS NOT NULL 
       OR form_fields."path" = dataset_form_defs."rootPath" || 'meta/entity'
       OR form_fields."path" = dataset_form_defs."rootPath" || 'meta/entity/label')
