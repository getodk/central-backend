...
const createFromApi = (project, name) => ({ one, Actees }) =>
  Actees.provision('dataset', project)
    .then((actee) => one(sql`
INSERT INTO datasets (name, "projectId", "createdAt", "publishedAt", "acteeId")
VALUES (${name}, ${project.id}, clock_timestamp(), clock_timestamp(), ${actee.id})
RETURNING *`).then((ds) => new Dataset(ds)));
