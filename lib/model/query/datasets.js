...
const _trashedFilter = (force, projectId, datasetName) => {
  ...
  const idFilter = idFilters.length ? sql`AND ${sql.join(idFilters, sql` AND `)}` : sql``;

  return (force
    ? sql`datasets."deletedAt" IS NOT NULL ${idFilter}`
    : sql`datasets."deletedAt" < CURRENT_DATE - CAST(${PURGE_DAY_RANGE} AS INT) ${idFilter}`);
};
