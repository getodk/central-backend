...
const _getLinkedForms = (datasetName, projectId, publishedOnly = true) => sql`
...
JOIN form_defs fd ON fd.id = fa."formDefId" ${publishedOnly ? sql` AND fd."publishedAt" IS NOT NULL` : sql``}
JOIN forms f ON (f.id = fd."formId"
  AND fd.id = f."currentDefId" ${publishedOnly ? sql`` : sql`OR fd.id = f."draftDefId"`}
  AND f."deletedAt" IS NULL
)
JOIN datasets ds ON ds.id = fa."datasetId"
WHERE ds.name = ${datasetName}
  AND ds."projectId" = ${projectId}
  ${publishedOnly ? sql`AND ds."publishedAt" IS NOT NULL` : sql``}
`;
...
const getMetadata = (dataset, publishedOnly = true) => async ({ all, Datasets }) => {
  ...
  const _getSourceForms = (datasetName, projectId) => sql`
    ...
    JOIN forms f ON fd.id = f."currentDefId" ${publishedOnly ? sql`` : sql`OR fd.id = f."draftDefId"`}
    WHERE ds.name = ${datasetName}
      AND ds."projectId" = ${projectId}
      AND f."deletedAt" IS NULL
      ${publishedOnly ? sql`AND ds."publishedAt" IS NOT NULL` : sql``}
  ...
  const linkedForms = await all(_getLinkedForms(dataset.name, dataset.projectId, publishedOnly))
    .then(rows => rows.map(construct(Dataset.LinkedForm)).map(f => f.forApi(!publishedOnly)));
...
const getListWithProperties = (projectId, ids) => async ({ all }) => {
  ...
  const rows = await all(sql`
    ...
    LEFT JOIN ds_properties ON ds_properties."datasetId" = datasets.id
      AND ds_properties."deletedAt" IS NOT NULL
