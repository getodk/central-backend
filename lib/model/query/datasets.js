...
insert_property_fields AS (
  ...
  ON CONFLICT ("dsPropertyId", "formDefId") DO NOTHING
)
`;
...
const cloneDatasetForDraftForms = (dataset) => sql`
WITH draft_form_defs AS (
  SELECT dfd.*
  FROM dataset_form_defs dfd
  JOIN form_defs fd ON fd.id = dfd."formDefId"
  JOIN forms f ON f.id = fd."formId"
  WHERE dfd."datasetId" = ${dataset.id}
    AND fd."publishedAt" IS NULL
    AND f."deletedAt" IS NULL
),
draft_properties AS (
  SELECT dp.name, dpf."formDefId", dpf.path
  FROM ds_properties dp
  JOIN ds_property_fields dpf ON dpf."dsPropertyId" = dp.id
  WHERE dp."datasetId" = ${dataset.id}
    AND dpf."formDefId" IN (SELECT "formDefId" FROM draft_form_defs)
),
new_actee AS (
  INSERT INTO actees (id, species, parent)
  SELECT gen_random_uuid(), 'dataset', "acteeId" FROM projects
  WHERE id = ${dataset.projectId}
  AND EXISTS (SELECT 1 FROM draft_form_defs)
  RETURNING *
),
new_dataset AS (
  INSERT INTO datasets (name, "projectId", "createdAt", "acteeId")
  SELECT name, "projectId", clock_timestamp(), new_actee.id 
  FROM datasets, new_actee
...
new_properties AS (
  INSERT INTO ds_properties (name, "datasetId")
  SELECT DISTINCT dp.name, nd.id
  FROM draft_properties dp, new_dataset nd
  RETURNING *
),
new_property_fields AS (
  INSERT INTO ds_property_fields ("dsPropertyId", "formDefId", path)
  SELECT np.id, dp."formDefId", dp.path
  FROM draft_properties dp
  JOIN new_properties np ON np.name = dp.name
),
new_ds_form_defs AS (
  INSERT INTO dataset_form_defs ("datasetId", "formDefId", actions, path, "isRepeat")
  SELECT nd.id, dfd."formDefId", dfd.actions, dfd.path, dfd."isRepeat"
  FROM draft_form_defs dfd, new_dataset nd
)
SELECT * FROM new_dataset
`;

const del = (dataset) => async ({ run }) => {
  await run(markDeleted(dataset));
  await run(cloneDatasetForDraftForms(dataset));
};
...
const clonePropertiesForDraftForms = (property, dataset) => sql`
WITH draft_property_fields AS (
  SELECT dpf."formDefId", dpf.path
  FROM ds_property_fields dpf
  JOIN form_defs fd ON fd.id = dpf."formDefId"
  JOIN forms f ON f.id = fd."formId"
  WHERE dpf."dsPropertyId" = ${property.id}
    AND fd."publishedAt" IS NULL
    AND f."deletedAt" IS NULL
),
new_property AS (
  INSERT INTO ds_properties (name, "datasetId")
  SELECT ${property.name}, ${dataset.id}
  WHERE EXISTS (SELECT 1 FROM draft_property_fields)
  RETURNING *
)
INSERT INTO ds_property_fields ("dsPropertyId", "formDefId", path)
SELECT np.id, dpf."formDefId", dpf.path
FROM draft_property_fields dpf, new_property np
`;

const deleteProperty = (property, dataset) => async ({ run }) => {
  await run(markDeleted(property));
  await run(clonePropertiesForDraftForms(property, dataset));
};
...
const publishIfExists = (formDefId, publishedAt = sql`clock_timestamp()`) => async ({ all, context, maybeOne }) => {
...
/** Returns a list of datasets with their properties for the given projectId and dataset names. */
const getListWithProperties = (projectId, names) => async ({ all }) => {
  if (names.length === 0) return [];

  const _unjoiner = unjoiner(Dataset, Dataset.Property);
  ...
  const rows = await all(sql`
    ...
    WHERE datasets."projectId" = ${projectId}
      AND datasets.name IN (${sql.join(names.map(n => sql`${n}`), sql`,`)})
  ...
  // Group properties by dataset (1:many relationship)
  const r = Array.from(rows.reduce((resultMap, row) => {
    if (!resultMap.has(row.id)) {
      resultMap.set(row.id, Object.assign(row.forApi(), { properties: [] }));
    }
    if (row.aux.properties.id != null) {
      resultMap.get(row.id).properties.push(row.aux.properties.forApi());
    }
    return resultMap;
  }, new Map()).values());
