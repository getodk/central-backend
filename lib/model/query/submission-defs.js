...
module.exports = {
  ...
  streamForExport: (formId, encryptedBlobs, options) => ({ submissionDefs }) =>
    submissionDefs.helper.forExport(formId, encryptedBlobs, options)((query, unjoin) =>
  ...
  helper: {
    forExport: (formId, encryptedBlobs = false, options = QueryOptions.none) => ({ db, Actor, Submission, SubmissionDef }) => (observe) =>
      withJoin('def', { def: SubmissionDef, submission: Submission, submitter: Actor }, (fields, unjoin) => observe(
        db.select(fields)
          ...
          .modify((db) => (encryptedBlobs === false)
            ? db
            : db.select({
              'def!encData': 'blobs.content',
              // only return the actual xml if we don't have encData instead:
              'def!xml': db.raw('case when "def!encData" is null then submission_defs.xml else null')
            })
