...
const _get = (submissionId, name, draft) => sql`
...
const getCurrentForSubmissionId = (submissionId, draft) => ({ all }) =>
  all(_get(submissionId, null, draft)).then(map(construct(Submission.Attachment)));

const getCurrentBlobByIds = (submissionId, name, draft) => ({ maybeOne }) => maybeOne(sql`
with sa as (${_get(submissionId, name, draft)})
...
join submissions on draft=${draft} and "formId"=${formId} and
