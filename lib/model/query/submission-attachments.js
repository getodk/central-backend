...
module.exports = {
  ...
  createAll: (attachments) => ({ simply }) =>
    simply.insert('submission_attachments', attachments),
  ...
  streamForExport: (formId, draft, keyIds = [], options = QueryOptions.none) => ({ db }) =>
    db
      ...
      .where({
        'submission_defs.current': true,
        'submissions.formId': formId,
        deletedAt: null
      })
      .innerJoin(
        db.select('*').from('submissions').where({ draft }).as('submissions'),
        'submissions.id', 'submission_defs.submissionId'
      )
      .innerJoin('form_defs', 'submission_defs.formDefId', 'form_defs.id')
      .innerJoin('submission_attachments', 'submission_attachments.submissionDefId', 'submission_defs.id')
