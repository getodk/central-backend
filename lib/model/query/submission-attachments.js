...
module.exports = {
  ...
  getAllBySubVersionId: (subVersionId) => ({ db, SubmissionAttachment }) =>
    db.select('*')
  ...
  update: (sa) => ({ db }) =>
    db.update({ blobId: sa.blobId }).into('submission_attachments')
      .where({ subVersionId: sa.subVersionId, name: sa.name })
    ...
    db
      ...
      .join(
        db.select(db.raw('max(id) as "latestId"'))
          ...
          .as('versions'),
        'versions.latestId', 'submissions.id'
      )
      .join('submission_attachments', 'submission_attachments.subVersionId', 'versions.latestId')
