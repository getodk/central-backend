...
const { clone, map, pick, without } = require('ramda');
...
const ExportSubmission = { ...pick(['from', 'to'], Submission), fields: without(['userAgent', 'localKey'], Submission.fields) };
const ExportSubmissionDef = { ...pick(['from', 'to'], Submission.Def), fields: clone(Submission.Def.fields) };
ExportSubmissionDef.fields[ExportSubmissionDef.fields.indexOf('xml')] = {
  name: 'xml',
  selectSql: tableAlias => {
    if (!tableAlias) {
      return sql.identfier(['xml']);
    } else if (tableAlias === 'submission_defs') {
      return sql`(case when ${sql.identifier([tableAlias, 'localKey'])} is null then ${sql.identifier([tableAlias, 'xml'])} end)`;
    } else {
      return sql.identifier([tableAlias, 'xml']);
    }
  },
};
