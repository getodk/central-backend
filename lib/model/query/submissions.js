...
const createNew = (partial, form, maybeActor, deviceIdIn = null) => ({ one }) => {
...
with def as (insert into submission_defs ("submissionId", xml, "formDefId", "submitterId", "localKey", "encDataAttachmentName", "signature", "createdAt", current)
  values (nextval(pg_get_serial_sequence('submissions', 'id')), ${sql.binary(partial.def.xml)}, ${form.def.id}, ${actorId}, ${partial.def.localKey}, ${partial.def.encDataAttachmentName}, ${partial.def.signature}, now(), true)
  returning *),
...
select ins.*, def.id as "submissionDefId" from ins, def;`)
    .then(({ submissionDefId, ...submissionData }) => ({ // TODO/SL why are these separate?
      submission: new Submission(submissionData),
      submissionDef: new Submission.Def({ // TODO/HACK: reassembling this from bits and bobs.
        id: submissionDefId,
...
const clearDraftSubmissions = (formId) => ({ run }) => run(sql`
...
const _get = extender(Submission)(Actor)((fields, extend, options) => sql`
...
select count(*) from submissions
  where ${equals({ formId, draft })} and "deletedAt" is null ${odataFilter(sql`and`, options.filter)}`);
...
order by "createdAt" desc limit 1`)
  .then(map(Submission.Def.construct));
...
const _exportUnjoiner = unjoiner(Submission, Submission.Def, Submission.Xml, Actor.to('submitter'),
