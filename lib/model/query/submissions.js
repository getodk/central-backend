...
const ExportSubmissionDef = { ...pick(['from', 'to', 'fields'], Submission.Def) };
ExportSubmissionDef.fields[ExportSubmissionDef.fields.indexOf('xml')] = {
  name: 'xml',
  selectSql: tableAlias => {
    if (!tableAlias) return sql.identfier(['xml']);
    else if (tableAlias === 'submission_defs') return sql`(case when ${sql.identifier(tableAlias)}."localKey" is null then ${sql.identifier(tableAlias)}.xml end)`;
    else return sql.identifier([tableAlias, 'xml']);
  },
};
