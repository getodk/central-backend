  ...
  maybeOne(
    sql`
      SELECT "submissions"."id" AS "submissions!id"
           ...
           , "encHasData" AS "encHasData"
           , "blobId" AS "blobId"
           , "encData" AS "encData"
           , "encSha" AS "encSha"
           , "encIndex" AS "encIndex"
           , "encKeyId" AS "encKeyId"
           , "encS3Status" AS "encS3Status"
        ...
        LEFT OUTER JOIN (
          SELECT "submissionDefId"
               , COUNT("blobId")::integer AS present
               , COUNT(name)::integer AS expected
            FROM submission_attachments
          GROUP BY "submissionDefId"
        ) AS attachments ON attachments."submissionDefId"=submission_defs.id
        INNER JOIN (
          SELECT "formDefId", "submissionId"
            FROM submission_defs
            WHERE root is true
        ...
        JOIN (
          SELECT null AS "encKeyId", null AS "encData", null AS "encSha", null AS "encS3Status"
        ) AS enc ON true
        INNER JOIN (
