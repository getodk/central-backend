...
const _defInsert = (id, partial, formDefId, actorId) => sql`insert into submission_defs ("submissionId", xml, "formDefId", "instanceId", "submitterId", "localKey", "encDataAttachmentName", "signature", "createdAt", current)
  values (${id}, ${sql.binary(partial.xml)}, ${formDefId}, ${partial.instanceId}, ${actorId}, ${partial.def.localKey}, ${partial.def.encDataAttachmentName}, ${partial.def.signature}, clock_timestamp(), true)
  returning *`;
...
const createVersion = (partial, deprecated, form) => ({ one, context }) => {
  ...
  return one(sql`
with upd as (update submission_defs set current=false where "submissionId"=${deprecated.submissionId})
${_defInsert(deprecated.submissionId, partial, form.def.id, actorId)}`);
};

createVersion.audit = (partial, submission, form) => (log) =>
  log('submission.update.version', form, { submissionId: submission.id, instanceId: partial.instanceId });
...
where "submissionId"=${submissionId} and current=true
limit 1`)
...
const getAnyDefByFormAndInstanceId = (formId, instanceId, draft = false) => ({ maybeOne }) => maybeOne(sql`
select submission_defs.* from submission_defs
inner join
  (select id from submissions where "formId"=${formId} and draft=${draft} and "deletedAt" is null)
  as submissions on submissions.id=submission_defs."submissionId"
where submission_defs."instanceId"=${instanceId}`)
  .then(map(construct(Submission.Def)));
...
module.exports = {
  ...
  getCurrentDefBySubmissionId, getCurrentDefByIds, getAnyDefByFormAndInstanceId,
