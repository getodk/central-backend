...
const base = (options, { db, Submission, Actor }) => ((options.extended === false)
  ? db.select('*')
    .from('submissions')
...
module.exports = {
  ...
  streamRowsByFormId: (formId, options = QueryOptions.none) => (container) =>
    resolve(base(options.withCondition({ formId }), container)
      .pipe(mapStream(deserializer(options.extended, container)))),
  ...
  _get: (options) => (container) =>
    base(options, container).then(map(deserializer(options.extended, container))),
};
