...
const getAllForFormByIds = (projectId, xmlFormId, draft, options) => async ({ all }) => {
  ...
  console.log('getAllForFormByIds()', 'got:', got, auxes(got)); // eslint-disable-line no-console
  const mapped = await map(assignCurrentVersionSubmitter)(got);
  console.log('getAllForFormByIds()', 'mapped:', mapped, auxes(mapped)); // eslint-disable-line no-console

  console.log('options:', options); // eslint-disable-line no-console
  console.log('options.extend:', options.extend); // eslint-disable-line no-console
