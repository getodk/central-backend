...
const getAllForFormByIds = (projectId, xmlFormId, draft, options) => async ({ all }) => {
  const auxes = rows => rows.map(row => row.aux);
  const got = await _get(all, options, projectId, xmlFormId, draft);
  console.log('getAllForFormByIds()', 'got:', got, auxes(got));
  const mapped = await map(assignCurrentVersionSubmitter)(got);
  console.log('getAllForFormByIds()', 'mapped:', mapped, auxes(mapped));

	if (options.extend) {
		const maybeExtendedUnjoiner = options.extend ?
			unjoiner(Submission, Submission.Def.into('currentVersion')) :
			unjoiner(Submission, Submission.Def.into('currentVersion'), Actor.into('submitter'), Actor.alias('current_version_actors', 'currentVersionSubmitter')) ;
		...
		return all(sql`
			SELECT "submissions"."id" AS "submissions!id"
					 ...
					 , "submission_defs"."id" AS "submission_defs!id"
					 , "submission_defs"."submissionId" AS "submission_defs!submissionId"
					 ...
					 , "submission_defs"."encDataAttachmentName" AS "submission_defs!encDataAttachmentName"
					 , "submission_defs"."signature" AS "submission_defs!signature"
					 ...
					 , "submission_defs"."current" AS "submission_defs!current"
					 , "submission_defs"."xml" AS "submission_defs!xml"
					 , "submission_defs"."root" AS "submission_defs!root"
					 ...
					 , "current_version_actors"."acteeId" AS "current_version_actors!acteeId"
					 , "current_version_actors"."displayName" AS "current_version_actors!displayName"
					 , "current_version_actors"."meta" AS "current_version_actors!meta"
					 ...
					 , "current_version_actors"."deletedAt" AS "current_version_actors!deletedAt"
				FROM (
