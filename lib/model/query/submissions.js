...
const { fieldsForJoin, joinRowToInstance, rowsToInstances, maybeFirst, translateProblem, ifPresent } = require('../../util/db');
...
module.exports = {
  ...
  streamRowsByFormId: (formId) => ({ db }) =>
    resolve(db.select('*').from('submissions').where({ formId }).orderBy('createdAt', 'asc').stream()),

  streamRowsForOdata: (formId, params) => ({ db }) =>
    resolve(db.select('*')
      .from('submissions')
      .where({ formId })
      .modify(ifPresent, params['$skip'], (db, skip) => db.offset(parseInt(skip)))
      .modify(ifPresent, params['$top'], (db, top) => db.limit(parseInt(top)))
