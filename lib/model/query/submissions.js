...
const getSelectMultipleValuesForExport = (formId, draft, options) => ({ all }) => all(sql`
  -- Submissions.getSelectMultipleValuesForExport()
...
const _export = (caller, formId, draft, keyIds = [], options) => {
  const encrypted = keyIds.length !== 0;
  return sql`
    -- ${sql.identifier(['Submissions', caller])}()
...
const streamForExport = (formId, draft, keyIds, options = QueryOptions.none) => ({ s3, stream }) =>
  stream(_export('streamForExport', formId, draft, keyIds, options))
...
const getForExport = (formId, instanceId, draft, options = QueryOptions.none) => ({ maybeOne }) =>
  maybeOne(_export('getForExport', formId, draft, [], options.withCondition({ 'submissions.instanceId': instanceId })))
...
const getODataSelectionEtag = (formId, draft, options = QueryOptions.none) => ({ db }) => db.oneFirst(sql`
    -- Submissios.getODataSelectionEtag()
    WITH selection AS (
      SELECT event
      FROM submissions
      ${joinOnSkiptoken(options.skiptoken, formId, draft)}
