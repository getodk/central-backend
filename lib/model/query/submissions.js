...
const { addBlobContent, streamEncBlobs } = require('../../util/blob');
...
left outer join (select id, content as "encData", sha as "encSha", md5 as "encMd5", s3_status as "encS3Status" from blobs) as blobs on blobs.id=submission_attachments."blobId"`
    : sql`join (select null as "encKeyId", null as "encData", null as "encSha", null as "encMd5", null as "encS3Status") as enc on true`}
inner join
