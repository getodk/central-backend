...
where submission_defs."instanceId"=${instanceId} and current=true
limit 1`)
  .then(map(construct(Submission.Def)));

const getAnyDefByFormAndInstanceId = (formId, instanceId, draft) => ({ maybeOne }) => maybeOne(sql`
select submission_defs.* from submission_defs
inner join
...
where "formId"=${formId} and draft=${draft}`)
...
module.exports = {
  ...
  getByIds, getAllForFormByIds, countByFormId,
  getCurrentDefByIds, getAnyDefByFormAndInstanceId, getRootForInstanceId,
