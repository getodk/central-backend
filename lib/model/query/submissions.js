...
const { unjoiner, extender, equals, joinSqlStr, page, updater, QueryOptions, insertMany } = require('../../util/db');
...
const _defInsert = (id, partial, formDefId, actorId, root, deviceId, userAgent) => sql`insert into submission_defs ("submissionId", xml, "formDefId", "instanceId", "instanceName", "submitterId", "localKey", "encDataAttachmentName", "signature", "createdAt", root, current, "deviceId", "userAgent")
  values (${id}, ${partial.xml.toString('utf8')}, ${formDefId}, ${partial.instanceId}, ${partial.def.instanceName}, ${actorId}, ${partial.def.localKey}, ${partial.def.encDataAttachmentName}, ${partial.def.signature}, clock_timestamp(), ${root}, true, ${deviceId}, ${userAgent})
  returning *`;
...
const _exportFields = sql.unsafe(_exportUnjoiner.fields.strings[0].replace(
...
where
  ${encrypted ? sql`(form_defs."encKeyId" is null or form_defs."encKeyId" in (${joinSqlStr(keyIds, sql`,`)})) and` : sql``}
...
const streamForExport = (formId, draft, keyIds, options = QueryOptions.none) => ({ stream }) =>
  PartialPipe.of(stream(_export(formId, draft, keyIds, options)), mapStream(_exportUnjoiner));
