...
const { unjoiner, extender, sqlAnd, sqlAndJoin, sqlEquals, page, updater, QueryOptions, insertMany, markDeleted, markUndeleted, sqlInArray } = require('../../util/db');
const { blankStringToNull, construct, truncateString, applyPipe, prependString } = require('../../util/util');
...
where
  form_field_values."formId"=${formId}
...
const countByFormId = (formId, draft, options = QueryOptions.none) => ({ one }) => {
  const filter = sqlAndJoin([
    sqlEquals({ formId, draft }),
    odataExcludeDeleted(options.filter, odataToColumnMap),
...
where
  ${sqlAndJoin([
    encrypted ? sql`(form_defs."encKeyId" is null or ${sqlInArray(sql`form_defs."encKeyId"`, keyIds)})` : sql`true`,
    odataFilter(options.filter, options.isSubmissionsTable ? odataToColumnMap : odataSubTableToColumnMap),
    sqlEquals(options.condition),
    sql`submission_defs.current=true`,
    sql`submissions."formId"=${formId}`,
...
////////////////////////////////////////////////////////////////////////////////
// Etag-deriving functions

const getODataSelectionEtag = (formId, draft, options = QueryOptions.none) => ({ db }) => db.oneFirst(sql`
    WITH selection AS (
      SELECT event
      FROM submissions
      ${joinOnSkiptoken(options.skiptoken, formId, draft)}
    ...
    SELECT
      format(${EVENTHASH_FORMAT}, md5_hash_agg(event))
    FROM
      selection
  `);

const getSelectionEtag = (formId, draft, odataQuery=null, limit=null, offset=null, orderby=null) => {
  const queryOptions = QueryOptions.fromODataRequest({ table: 'Submissions' }, { $top: limit, $filter: odataQuery, $skip: offset, $orderby: orderby });
  return getODataSelectionEtag(formId, draft, queryOptions);
};
