...
const { always, identity, ifElse, map, pick, without, ...R } = require('ramda');
...
const ExportSubmission = cloneFrame(Submission, without(['userAgent', 'localKey']));
const ExportSubmissionDef = cloneFrame(Submission.Def, map(ifElse(R.equals('xml'), always(xmlUnlessEncrypted), identity)));
