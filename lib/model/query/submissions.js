...
with def as (insert into submission_defs ("submissionId", xml, "formDefId", "submitterId", "localKey", "encDataAttachmentName", "signature", "createdAt", current)
  values (nextval(pg_get_serial_sequence('submissions', 'id')), ${sql.binary(partial.def.xml)}, ${form.def.id}, ${actorId}, ${partial.def.localKey}, ${partial.def.encDataAttachmentName}, ${partial.def.signature}, clock_timestamp(), true)
  returning *),
...
select ins.*, def.id as "submissionDefId" from ins, def;`)
    .then(({ submissionDefId, ...submissionData }) => // TODO/HACK: reassembling this from bits and bobs.
      new Submission(submissionData, { def: new Submission.Def({
        id: submissionDefId,
...
left outer join
  (select *, (submission_attachments."blobId" is not null) as "encHasData" from submission_attachments)
