...
const getResetPasswordActor = actor => ({ maybeOne }) => maybeOne(sql`
  SELECT id AS "actorId", "acteeId" FROM actors WHERE id = (
    SELECT meta->>'resetPassword' FROM actors WHERE actors.id=${actor.id}
  )::INTEGER
`);
...
module.exports = {
  create, update,
  updatePassword, invalidatePassword, provisionPasswordResetToken, getResetPasswordActor,
