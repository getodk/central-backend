...
update.audit = (user) => (log) => log('user.update', user.actor, { data: user });

const withPassword = (user, cleartext) => ({ bcrypt }) =>
  bcrypt.hash(cleartext).then((password) => user.with({ password }));
...
const invalidatePassword = (user) => ({ run }) =>
...
const emailEverExisted = (email) => ({ oneFirst }) =>
  oneFirst(sql`select count(*) from users where email=${email};`)
    .then((count) => count > 0);

module.exports = {
  create, update,
  withPassword, updatePassword, invalidatePassword, provisionPasswordResetToken,
