...
const getForUser = (userId) => async ({ allFirst }) => {
  const site = Object.fromEntries(await allFirst(sql`
    SELECT JSONB_BUILD_ARRAY("propertyName", "propertyValue")
      FROM user_site_preferences
      WHERE "userId" = ${userId}
  `));

  const projects = Object.fromEntries(await allFirst(sql`
    SELECT JSONB_BUILD_ARRAY("projectId", JSONB_OBJECT_AGG("propertyName", "propertyValue"))
