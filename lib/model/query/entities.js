...
const purge = (force = false, projectId = null, datasetName = null, entityUuid = null) => async ({ all, oneFirst, run }) => {
  ...
  const deletedJson = JSON.stringify(Object.entries(deletedEntities.reduce((acc, { acteeId, uuid }) => { if(!acc[acteeId]) acc[acteeId] = []; acc[acteeId].push(uuid); return acc; }, {})).map(([ acteeId, entityUuids ]) => ({ acteeId, details:{ entitiesDeleted:entityUuids.length, entityUuids } })))
  console.log('deletedJson:', deletedJson);
  await run(sql`
    INSERT INTO audits ("action", "acteeId", "loggedAt", "processed", "details")
      SELECT 'entity.purge'
           ...
           , clock_timestamp()
           , details
        FROM JSONB_TO_RECORDSET(${deletedJson})
