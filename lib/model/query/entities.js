...
const sqlAnd = tok => {
  const normalised = tok.sql.trim().toLowerCase();
  if(!normalised || normalised === 'true') return sql``;

  return sql`AND ${tok}`;
};

const streamForExport = (datasetId, options = QueryOptions.none, actorId) => async ({ stream, run, ...rest }) => {
  await run(sql`SET LOCAL enable_hashjoin = OFF`);
  await run(sql`SET LOCAL enable_mergejoin = OFF`);

  return stream({ batchSize:1000 }, sql`
