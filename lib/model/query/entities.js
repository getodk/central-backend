...
const { odataToColumnMap, getDiffProp, ConflictType, normalizeUuid } = require('../../data/entity');
const { submissionXmlToEntityData } = require('../../data/submission');
...
const _processEntityDataFromSubmission = (entityData, event, parentEvent, form, submissionDef, forceOutOfOrderProcessing) => async ({ Datasets, Entities, oneFirst, run }) => {
  ...
  if (entityData.system.update === '1' || entityData.system.update === 'true')
    try {
      maybeEntity = await Entities._updateEntity(dataset, entityData, submissionDef.submissionId, submissionDef, submissionDef.id, event, forceOutOfOrderProcessing);
    } catch (err) {
      ...
      if ((err.problemCode === 404.8 || (err.problemCode === 400.2 && err.problemDetails.field === 'baseVersion')) && attemptCreate) {
        maybeEntity = await Entities._createEntity(dataset, entityData, submissionDef.submissionId, submissionDef, submissionDef.id, event, parentEvent, forceOutOfOrderProcessing);
      } else {
        throw (err);
      }
    }
    ...
    try {
      ...
      if (err.problemCode === 409.3 && err.problemDetails?.fields[0] === 'uuid') {
        ...
        if (rootDef && rootDef.aux.source.forceProcessed) {
          maybeEntity = await Entities._updateEntity(dataset, entityData, submissionDef.submissionId, submissionDef, submissionDef.id, event, forceOutOfOrderProcessing, true);
        } else {
          throw (err);
        }
      } else {
...
const _processSubmissionEvent = (event, parentEvent) => async ({ Datasets, Entities, Submissions, Forms }) => {
  ...
  for (const entityData of entities) {
    ...
    await Entities._processEntityDataFromSubmission(entityData, event, parentEvent, form, submissionDef, forceOutOfOrderProcessing);
  }
...
module.exports = {
  createNew, _processSubmissionEvent, _processEntityDataFromSubmission,
  createSource,
  createMany,
