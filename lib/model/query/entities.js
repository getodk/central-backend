...
const idFilter = (options, idCol = 'uuid') => {
  const query = options.ifArg('id', ids => sql`${sql.identifier([idCol])} IN (${sql.join(ids.split(',').map(id => sql`${id.trim()}`), sql`, `)})`);
  return query.sql ? query : sql`TRUE`;
};

const _getAllEntitiesState = (datasetId, acteeId, options) => sql`
...
const getEntitiesState = (datasetId, acteeId, options = QueryOptions.none) =>
  ({ all }) => all(_getAllEntitiesState(datasetId, acteeId, options));
