...
const { sqlEquals, extender, unjoiner, page, markDeleted, insertMany, markUndeleted, sqlInArray } = require('../../util/db');
...
// If actorId is specified, the entities will be limited to those created by the
// actor (ownerOnly).
const streamForExport = (datasetId, actorId = undefined, options = QueryOptions.none) => ({ stream }) =>
  stream(sql`
...
WHERE
  ...
  ${actorId ? sql`AND entities."creatorId" = ${actorId}` : sql``}
