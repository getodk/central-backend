...
const _processSubmissionEvent = (event, parentEvent) => async ({ Datasets, Entities, Submissions, Forms }) => {
  ...
  const res = await submissionXmlToEntityData(structurals, fields, submissionDef.xml);
  for (const [key, entities] of Object.entries(res)) {
    console.log(`Key: ${key}`);
    entities.forEach((entity, idx) => {
      console.log(`  Entity ${idx + 1}:`);
      console.log('    system:', JSON.stringify(entity.system, null, 2));
