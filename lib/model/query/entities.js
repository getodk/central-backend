...
const _update = (partial) => ({ one }) => {
  const _unjoiner = unjoiner(Entity, Entity.def);
  const json = JSON.stringify(partial.aux.data);

  return one(sql`
  with def as (${_defInsert(partial.id, null, null, json)}),
  ...
  entities as (update entities set "updatedAt"=clock_timestamp()
    ...
    returning *)
  select ${_unjoiner.fields} from def
  join entities on def."entityId" = entities.id
  `)
    .then(_unjoiner);
};

const createVersion = (data, entity) => async ({ Datasets, Entities }) => {
  // TODO: will eventually use for authoring the def
  //const actorId = context.auth.actor.map((actor) => actor.id).orNull();

  // gets the properties
  const properties = await Datasets.getProperties(entity.datasetId);

  // builds the new partial entity
  ...
  const partial = new Entity(entity, { data: defData });
...
const _get = extender(Entity, Entity.Def.into('currentVersion'))(Actor.into('creator'))((fields, extend, options) => sql`
...
const getByUuidWithDef = (datasetId, uuid, options = QueryOptions.none) => ({ maybeOne }) =>
  _get(maybeOne, options.withCondition({ 'entities.uuid': uuid }), datasetId);

const getByDataset = (datasetId, options = QueryOptions.none) => ({ all }) =>
  _get(all, options.withCondition({ datasetId }));
...
module.exports = {
  ...
  getDefBySubmissionId, getByUuidWithDef,
  countByDatasetId, getByDataset,
