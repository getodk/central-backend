...
// Helper function to assign entity.currentVersionCreator to entity.currentVersion.creator
// Currently there is no way to create such complex object using `extender` and `unjoiner` framework functions
const assignCurrentVersionCreator = (x) => x.withAux('currentVersion', x.aux.currentVersion.withAux('creator', x.aux.currentVersionCreator));

const _get = extender(Entity, Entity.Def.into('currentVersion'))(Actor.into('creator'), Actor.alias('current_version_actors', 'currentVersionCreator'))((fields, extend, options, deleted = false) => sql`
  select ${fields} from entities
  inner join entity_defs
      on entities.id = entity_defs."entityId" and entity_defs.current
  ${extend||sql`
    left outer join actors on actors.id=entities."creatorId"
    left outer join actors current_version_actors on current_version_actors.id=entity_defs."creatorId"
  `} 
...
const getById = (datasetId, uuid, options = QueryOptions.none) => ({ maybeOne }) =>
  _get(maybeOne, options.withCondition({ 'entities.uuid': uuid }), datasetId);

const getAll = (datasetId, options = QueryOptions.none) => ({ all }) => _get(all, options.withCondition({ datasetId }), isTrue(options.argData.deleted))
