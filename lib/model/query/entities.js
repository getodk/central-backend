...
const _trashedFilter = (force, projectId, datasetName, entityUuid) => {
  ...
  return (force
    ? sql`entities."deletedAt" IS NOT NULL ${idFilter}`
    : sql`entities."deletedAt" < CURRENT_DATE - CAST(${PURGE_DAY_RANGE} AS INT) ${idFilter}`);
};
...
const idFilter = (options, idCol = 'uuid') => {
  if (options.args.id) {
    ...
    return idList.length ? sql`AND ${sql.identifier([idCol])} = ANY (${sql.array(idList, 'uuid')})` : sql``;
  } else {
    return sql``;
  }
};
