...
const mapActeeIdsToEntityUuids = deletedEntities => {
  const byActeeId = Object.entries(
    deletedEntities.reduce(
      (acc, { acteeId, uuid }) => {
        if (!acc[acteeId]) acc[acteeId] = [];
        acc[acteeId].push(uuid);
        return acc;
      },
      {},
    ));
  const structured = byActeeId.map(([ acteeId, entityUuids ]) => ({
    acteeId,
    details: { entitiesDeleted: entityUuids.length, entityUuids },
  }));
  ...
  await run(sql`
    INSERT INTO audits ("action", "acteeId", "loggedAt", "processed", "details")
      SELECT 'entity.purge'
           ...
           , clock_timestamp()
           , details
        FROM JSONB_TO_RECORDSET(${mapActeeIdsToEntityUuids(deletedEntities)})
