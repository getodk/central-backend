...
const purge = (force = false, projectId = null, datasetName = null, entityUuid = null) => async ({ all, oneFirst, run }) => {
  console.log('purge()', { entityUuid, projectId, datasetName });
  if (entityUuid && (!projectId || !datasetName)) {
    console.log('purge()', 'throwing...');
  ...
  const deletedEntities = await all(sql`
    DELETE FROM entities
            ${(projectId || datasetName ? sql`USING datasets` : sql``)}
      WHERE ${(projectId || datasetName ? sql`entities."datasetId" = datasets.id AND` : sql``)}
            ${_trashedFilter(force, projectId, datasetName, entityUuid)}
      RETURNING entities.id
              , entities.uuid
  `);
  console.log('deletedEntities:', deletedEntities);

  if(!deletedEntities.length) return 0;

  const sqlEntityIds   = sql.join(deletedEntities.map(e => e.id),   sql`,`);
  const sqlEntityUuids = sql.join(deletedEntities.map(e => e.uuid), sql`,`);
  console.log('sqlEntityUuids:', sqlEntityUuids);

  // TODO Redact existing audits
  //
  ...
  // entities.
  console.log('entity_defs',         await all(sql`SELECT * FROM entity_defs`));
  await run(sql`
    UPDATE audits
      SET notes = NULL
  ...
  // TODO Delete entity sources
  console.log('entity_defs',         await all(sql`SELECT * FROM entity_defs`));
  console.log('entity_def_sources:', await all(sql`SELECT * FROM entity_def_sources`));
  const res = await all(sql`
    DELETE FROM entity_def_sources
      USING entity_defs
      WHERE entity_def_sources.id = entity_defs."sourceId"
        AND entity_defs."entityId" IN (${sqlEntityIds})
        AND ( entity_def_sources.type = 'submission'
          OR (   entity_def_sources.type = 'api'
            AND (entity_def_sources.details IS NULL
              OR entity_def_sources.details = 'null'))) -- don't detail bulk source
      RETURNING *
  `);
  console.log('entity_def_sources: deleted:', res);
  ...
  // TODO Add entity.purge audits
//        INSERT INTO audits ("action", "acteeId", "loggedAt", "processed", "details")
//          SELECT 'entity.purge', datasets."acteeId", clock_timestamp(), clock_timestamp(), jsonb_build_object('entitiesDeleted', COUNT(*), 'entityUuids', ARRAY_AGG(uuid))
//            FROM entities
