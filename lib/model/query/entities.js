...
const { sqlAnd, sqlEquals, extender, unjoiner, page, markDeleted, insertMany } = require('../../util/db');
const { map, mergeRight, pickAll } = require('ramda');
const { blankStringToNull, construct } = require('../../util/util');
const { QueryOptions, specificFields } = require('../../util/db');
const { odataFilter, odataOrderBy, odataExcludeDeleted } = require('../../data/odata-filter');
const { odataToColumnMap, getDiffProp, ConflictType, normalizeUuid, submissionXmlToEntityData } = require('../../data/entity');
...
const _processSubmissionEvent = (event, parentEvent) => async ({ Datasets, Entities, Submissions, Forms }) => {
  ...
  const form = await Forms.getByActeeId(event.acteeId, Form.WithoutDef);
  ...
  const structurals = await Forms.getStructuralFields(submissionDef.formDefId);

  const entities = await submissionXmlToEntityData(structurals, fields, submissionDef.xml);

  const datasets = await Datasets.getList(form.get().projectId, QueryOptions.none.withCondition({
    name: [...new Set(entities.filter(e => e.system.dataset).map(e => e.system.dataset))]
  }).with({ conditionTypes: { name: 'varchar' } }));

  const datasetMap = new Map(datasets.map(ds => [ds.name, ds]));
  ...
  for (const [i, entityData] of entities.entries()) {
    try {
      ...
      if (!datasetMap.has(entityData.system.dataset)) {
        throw Problem.user.datasetNotFound({ datasetName: entityData.system.dataset });
      }

      const dataset = datasetMap.get(entityData.system.dataset);
      // eslint-disable-next-line no-await-in-loop
      await Entities._processEntityData(sourceId, entityData, event, form, submissionDef, forceOutOfOrderProcessing, dataset);
    } catch (err) {
      if (totalEntities === 1)
        throw (err);
      else
...
const _processEntityData = (sourceId, entityData, event, form, submissionDef, forceOutOfOrderProcessing, dataset) => async ({ Entities, oneFirst, run }) => {
...
const getEntitiesWithNonEmptyPropertyValues = (datasetId, propertyName) => async ({ all }) => {
  const entities = await all(sql`
    WITH records AS (
      SELECT uuid, label FROM entities e
      JOIN entity_defs ef ON e.id = ef."entityId" AND ef.current
      WHERE e."datasetId" = ${datasetId} AND e."deletedAt" IS NULL 
      AND NULLIF(TRIM(ef."data"->>${propertyName}), '') IS NOT NULL
    )
    (SELECT NULL AS uuid, 'totalCount' AS label, count(*) AS "totalCount" FROM records)
    UNION ALL
    (SELECT uuid, label, NULL AS totalCount FROM records LIMIT 3)
  `);


  const [meta, ...data] = entities;
...
WHERE
  ...
  ${sqlAnd(sqlEquals(options.condition))}
  ${sqlAnd(odataExcludeDeleted(options.filter, odataToColumnMap))}
  AND entity_defs.current=true
  ${sqlAnd(odataFilter(options.filter, odataToColumnMap))}
  ${sqlAnd(searchClause(options.search))}
  ${sqlAnd(actorId != null ? _actorHasAccess(actorId) : sql``)}
${options.orderby ? sql`
...
const _trashedFilter = (force, projectId, datasetName, entityUuid) => {
  ...
  return (force
    ? sql`entities."deletedAt" IS NOT NULL ${sqlAnd(idFilters)}`
    : sql`entities."deletedAt" < CURRENT_DATE - CAST(${PURGE_DAY_RANGE} AS INT) ${sqlAnd(idFilters)}`);
};

const purge = (force = false, projectId = null, datasetName = null, entityUuid = null) => ({ oneFirst, run }) => {
  if (entityUuid && (!projectId || !datasetName)) {
    throw Problem.internal.unknown({ error: 'Must specify projectId and datasetName to purge a specific entity.' });
  }
...
const getEntitiesState = (dataset, options = QueryOptions.none, actorId) => ({ all }) => all(sql`
  SELECT uuid, (
    entities."deletedAt" IS NOT NULL
    ${actorId != null ? sql`OR NOT (${_actorHasAccess(actorId)})` : sql``}
  ) as deleted 
  FROM entities 
  ${actorId != null ? sql`JOIN entity_defs ON entities.id = entity_defs."entityId" AND entity_defs.current` : sql``}
  WHERE "datasetId" = ${dataset.id}
  ${sqlAnd(idFilter(options))}
  UNION
  SELECT "entityUuid" AS uuid, TRUE AS deleted FROM purged_entities
  WHERE "acteeId" = ${dataset.acteeId}
  ${sqlAnd(idFilter(options, 'entityUuid'))}
...
const deleteByIds = (uuids, dataset) => ({ allFirst }) => allFirst(sql`
  UPDATE entities SET "deletedAt" = now()
  WHERE "deletedAt" IS NULL
    AND uuid = ANY (ARRAY[${sql.array(uuids, 'uuid')}])
    AND "datasetId" = ${dataset.id}
  RETURNING uuid
`);

deleteByIds.audit = (deletedUuids, _, dataset) => (log) => {
  if (deletedUuids.length === 0) return Promise.resolve();
  else if (deletedUuids.length === 1) return del.audit(new Entity({ uuid: deletedUuids[0] }), dataset)(log);
  else return log(
    'entity.bulk.delete',
    { acteeId: dataset.acteeId },
    { entityUuids: deletedUuids }
  );
};

deleteByIds.audit.withResult = true;

const restoreByIds = (uuids, dataset) => ({ allFirst }) => allFirst(sql`
  UPDATE entities SET "deletedAt" = null
...
restoreByIds.audit = (restoredUuids, _, dataset) => (log) => {
  if (restoredUuids.length === 0) return Promise.resolve();
  else if (restoredUuids.length === 1) return restore.audit(new Entity({ uuid: restoredUuids[0] }), dataset)(log);
  else return log(
    'entity.bulk.restore',
    { acteeId: dataset.acteeId },
    { entityUuids: restoredUuids }
  );
};
...
module.exports = {
  ...
  getEntitiesWithNonEmptyPropertyValues
};
