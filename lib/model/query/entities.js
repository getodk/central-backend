...
const deleteByIds = (uuids) => ({ run }) => run(sql`
  UPDATE entities SET "deletedAt" = now() WHERE uuid = ANY (ARRAY[${sql.array(uuids, 'text')}])
  `);

deleteByIds.audit = (uuids, dataset) => (log) =>
  (uuids.length === 1 ?
    del.audit(new Entity({ uuid: uuids[0] }), dataset)(log) :
    log(
      'entity.bulk.delete',
...
const restoreByIds = (uuids) => ({ run }) => run(sql`
  UPDATE entities SET "deletedAt" = null WHERE uuid = ANY (ARRAY[${sql.array(uuids, 'text')}])
  `);

restoreByIds.audit = (uuids, dataset) => (log) =>
  (uuids.length === 1 ?
    restore.audit(new Entity({ uuid: uuids[0] }), dataset)(log) :
    log(
      'entity.bulk.restore',
      { acteeId: dataset.acteeId },
      { entityUuids: uuids }
