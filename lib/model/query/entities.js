...
const _processSubmissionDef = (event, submissionId, submissionDefId, formActeeId) => async ({ Datasets, Entities, Submissions, Forms, Audits }) => {
  ...
  const form = await Forms.getByActeeId(formActeeId);
...
const _processionSubmissionEvent = (event, submissionId, submissionDefId, formActeeId) => (container) =>
  container.db.transaction((trxn) =>
    container.with({ db: trxn }).Entities._processSubmissionDef(event, submissionId, submissionDefId, formActeeId))
    .catch((err) =>
...
const createEntitiesFromPendingSubmissions = (pendingSubmissions, event) => (container) =>
  ...
  runSequentially(pendingSubmissions.map(s =>
  // what should be eventId here? If we pass event.id then we won't be able to retrieve submission InstanceId when submission is deleted
  // and if we pass submission create/edit eventId then we are telling a lie. should we store both? how it will look with new source table
    () => _processionSubmissionEvent(event, s.submissionId, s.submissionDefId, s.formActeeId)(container)));
