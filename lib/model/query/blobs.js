...
const _markAsSkipped = ({ id }) => ({ run }) => run(sql`
  UPDATE blobs
...
const uploadBlobIfAvailable = async container => {
  ...
  const res = await container.transacting(async outerTx => {
    ...
    try {
      return await outerTx.db.transaction(async innerDb => {
        const innerTx = outerTx.with({ db: innerDb });

        if (await innerTx.s3.uploadFromBlob(blob)) {
          await innerTx.Blobs._markAsUploaded(blob);
          return true;
        } else {
          await innerTx.Blobs._markAsSkipped(blob);
          return 'skipped';
        }
      });
    } catch (err) {
...
const s3UploadPending = (limit) => async (container) => {
  ...
  try {
    let status;
    while (status = await uploadBlobIfAvailable(container)) { // eslint-disable-line no-await-in-loop, no-cond-assign
      if (status === 'skipped') {
        ++skipped; // eslint-disable-line no-plusplus
      } else {
    ...
    if (uploaded > 0 || failed > 0 || skipped > 0)
      await container.Audits.log(null, 'blobs.s3.upload', null, { uploaded, failed, skipped, duration: end-start });
  }
};

module.exports = {
  ensure, getById, purgeUnattached,
  _getOnePending, _markAsFailed, _markAsUploaded, _markAsSkipped,
