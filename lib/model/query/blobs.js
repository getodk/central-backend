...
const unattachedClause = (extraWhere) => sql`
  ...
  WHERE ${extraWhere ?? sql``} ca."blobId" IS NULL
...
const _purgeAllUnattached = () => ({ all }) => all(sql`
  DELETE FROM blobs
    USING blobs AS b
    ${unattachedClause(sql`blobs.id = b.id AND`)}
`);

const _getOneUnattached = () => ({ one }) => one(sql`
  SELECT id, md5, sha
    FROM blobs AS b
    ${unattachedClause()}
    FOR UPDATE
...
const purgeUnattached = () => async ({ Blobs }) => {
  ...
  while (await (async () => {
    ...
    const blob = await Blobs._getOneUnattached();
    if (!blob) return;
