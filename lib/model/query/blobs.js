...
const _purgeOneUnattached = (blobId) => ({ maybeOne }) => maybeOne(sql`
  DELETE FROM blobs
    WHERE id IN (
      SELECT id
        FROM blobs AS b
      ...
      AND b.id=${blobId}
    LIMIT 1
    RETURNING blobs.id, blobs.s3_status
`);

const purgeUnattached = () => async ({ s3, Blobs }) => {
  if (!s3.enabled) return Blobs._purgeAllUnattached();

  // TODO optimisation: delete all non-uploaded?

  while (true) { // eslint-disable-line no-constant-condition
    const maybeBlob = await Blobs._purgeOneUnattached(); // eslint-disable-line no-await-in-loop
    if (isEmpty(maybeBlob)) return;

    const blob = maybeBlob.get();
    if (blob.s3_status === 'uploaded') await s3.deleteObjFor(blob); // eslint-disable-line no-await-in-loop
  }
};
...
module.exports = {
  ...
  _purgeAllUnattached, _purgeOneUnattached,
};
