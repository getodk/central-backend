...
const uploadBlobIfAvailable2 = async outerTx => {
  const maybeBlob = await outerTx.Blobs._getOnePending();
  if (isEmpty(maybeBlob)) return;

  const blob = maybeBlob.get();

  // Only takes effect if inner transaction is rolled back and outer transaction
  // is committed.
  await outerTx.Blobs._markAsFailed(blob);
