...
const uploadBlobIfAvailable = async container => {
  ...
  const log = (...args) => console.log(id, 'uploadBlobIfAvailable()', ...args);
  log('ENTRY');

  const shutdownListener = (...args) => {
    log('shutdownListener()', args.length, args);
    // This cancels the normal process, so:
    // TODO:
    // * interrupt the upload,
    // * mark the blob as failed, and
    // * return false from uploadBlobIfAvailable();
  };
      ...
      try {
        await outerTx.db.transaction(async innerDb => {
          try {
            ...
            log('INNER TRANSACTION FINISHING...');
          }
        });
        ...
        log('OUTER TRANSACTION FINISHING...');
      }
    });
