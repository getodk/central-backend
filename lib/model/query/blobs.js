...
const s3SetInProgressToPending = () => ({ oneFirst }) => oneFirst(sql`
  WITH updated AS (
    UPDATE blobs
      SET   s3_status='pending'
      WHERE s3_status='in_progress'
    RETURNING 1
  )
  SELECT COUNT(*) FROM updated
`);
...
const _getOnePending = (in_progress) => ({ maybeOne }) => maybeOne(sql`
  UPDATE blobs
    SET s3_status='failed'
    WHERE id IN (
      SELECT id
...
// eslint-disable-next-line no-unused-vars // FIXME remove
const uploadBlobIfAvailable1 = async ({ s3, Blobs }) => {
...
// FIXME future approach - get it working once new tests are working
// eslint-disable-next-line no-unused-vars
const uploadBlobIfAvailable = container => container.transacting(async outerTx => {
  console.log('outerTx:', outerTx.db.isTransacting, outerTx);
  ...
  try {
    await outerTx.db.transaction(async innerDb => {
      console.log('innerDb:', innerDb.isTransacting, innerDb);
      const innerTx = outerTx.with({ db: innerDb });
      console.log('innerTx:', innerTx.db.isTransacting, innerTx);
...
const s3UploadPending = () => async (container) => {
  // eslint-disable-next-line no-await-in-loop
  let ret;
  while (ret = await uploadBlobIfAvailable(container)) console.log('uploadBlobIfAvailable() returned:', ret);
  console.log('uploadBlobIfAvailable() returned:', ret);
};

module.exports = {
  ensure, getById, purgeUnattached,
  _getOnePending, _markAsFailed, _markAsUploaded,
  s3CountByStatus, s3SetFailedToPending, s3SetInProgressToPending, s3UploadPending,
