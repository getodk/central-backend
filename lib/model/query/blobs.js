...
const _dbQuery = (query) => ({ run }) => run(query);
...
const uploadBlobIfAvailable = async ({ s3, Blobs }) => {
  await Blobs._dbQuery(sql`BEGIN`);
  ...
  try {
    ...
    await Blobs._dbQuery(sql`COMMIT`);

    return true;
  } catch (err) {
    await Blobs._markAsFailed(blob);
    await Blobs._dbQuery(sql`ROLLBACK`);
...
module.exports = {
  ...
  _getOnePending, _markAsFailed, _markAsUploaded, _dbQuery,
