...
const purgeUnattached = () => async ({ s3, Blobs }) => {
  ...
  while (true) { // eslint-disable-line no-constant-condition
    ...
    const maybeBlob = await Blobs._getOneUnattached(); // eslint-disable-line no-await-in-loop
    ...
    const maybeDeleted = await Blobs._purgeOneUnattached(blob.id); // eslint-disable-line no-await-in-loop
