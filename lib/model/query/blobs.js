...
const _log = fnName => (...args) => console.error(`[pid:${process.pid}] [Blobs.${fnName}() | ${new Date()}]`, ...args);
...
const uploadBlobIfAvailable = async container => {
  const log = _log('uploadBlobIfAvailable');

  let innerError;

  log('Starting outer transaction...');
  const res = await container.transacting(async outerTx => {
    log('Outer transaction started.');
    const maybeBlob = await outerTx.Blobs._getOnePending();
    log('Got:', maybeBlob);
    if (isEmpty(maybeBlob)) {
      log('No blob; returning undefined.');
      return;
    }
    ...
    try {
      log('Starting inner transaction...');
      await outerTx.db.transaction(async innerDb => {
        log('Inner transaction started.');
        const innerTx = outerTx.with({ db: innerDb });

        log('Uploading blob...');
        await innerTx.s3.uploadFromBlob(blob);
        log('Marking blob as uploaded...');
        await innerTx.Blobs._markAsUploaded(blob);
        log('Inner loop complete; committing inner transaction...');
      });

      log('Outer transaction returning true.');
      return true;
    } catch (err) {
      log('Inner transaction threw:', err);
      // Allow outer transaction to commit, but bubble err
      innerError = err;
    }
  });

  log('Completed with:', { innerError, res });
...
const s3UploadPending = (limit) => async (container) => {
  const log = _log('s3UploadPending');
  ...
  try {
    while (await uploadBlobIfAvailable(container)) { // eslint-disable-line no-await-in-loop
      ...
      log('Marked another one uploaded.  Total:', uploaded);
      if (limit && !--limit) break; // eslint-disable-line no-param-reassign, no-plusplus
    }
    log('Completed uploading available blobs without issue.');
  } catch (err) {
    log('Caught and rethrowing err:', err);
    ++failed; // eslint-disable-line no-plusplus
    throw err;
  } finally {
    log('Final stats:', { uploaded, failed });
    const end = new Date();
