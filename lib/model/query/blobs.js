...
const uploadBlobIfAvailable = async container => {
  const log = (...args) => console.log(new Date(), 'uploadBlobIfAvailable()', ...args); // eslint-disable-line no-console
  log('ENTRY');
  ...
  const res = await container.transacting(async outerTx => {
    ...
    log('maybeBlob:', maybeBlob);
    if (isEmpty(maybeBlob)) return;

    const blob = maybeBlob.get();
    log('blob:', blob.id, blob.md5);
