...
const del = (actor) => ({ run, Assignments, Sessions }) => Promise.all([
  run(markDeleted(actor)),
  Assignments.revokeByActorId(actor.id),
  Sessions.terminateByActorId(actor.id)
]);
del.audit = (actor) => (log) => log('user.delete', actor);
...
module.exports = { create, createSubtype, update, consume, del, getById };
