...
module.exports = {
  ...
  getAllByDefinitionId: (definitionId, options = QueryOptions.none) => ({ formAttachments }) =>
    formAttachments._get(options.withCondition({ definitionId })),

  // we need to join against blob to get the md5.
  getAllByDefinitionIdForOpenRosa: (definitionId) => ({ FormAttachment, db }) =>
  ...
  update: (fa) => ({ db }) =>
    db.update({ blobId: fa.blobId }).into('form_attachments')
      .where({ definitionId: fa.definitionId, name: fa.name })
