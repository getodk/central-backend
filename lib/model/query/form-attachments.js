...
module.exports = {
  getAllByFormDefId: (formDefId, options = QueryOptions.none) => ({ db, FormAttachment }) => ((options.extended === false)
    ? db.select('*')
    ...
    : db.select(FormAttachment.fields.extended)
      ...
      .where({ formDefId })
      .leftOuterJoin(
        db.select(db.raw(`details->>'name' as "updateName", max("loggedAt") as "updatedAt"`))
          .from('audits')
          .where({ action: 'form.attachment.update' })
          .where(db.raw("details->>'formDefId' = ?", formDefId))
          .groupBy(db.raw("details->>'name'"))
          .as('last_update'),
        'form_attachments.name', 'last_update.updateName'
      )
      .orderBy('name', 'asc')
      .then(rowsToInstances(FormAttachment))),
