    ...
    LEFT OUTER JOIN (SELECT id, md5 FROM blobs) AS blobs ON form_attachments."blobId"=blobs.id
      LEFT OUTER JOIN (
        SELECT
          ...
          ${filterActorId == null ? sql`FALSE` : sql`"ownerOnly"`} AS "ownerOnly"
          FROM datasets
...
const getAllByFormDefIdForOpenRosa = (formDefId, options = QueryOptions.none, filterActorId = null) => ({ all }) =>
  _getWithDatasetMetadata(all, options.withCondition({ formDefId }), filterActorId)
    .then((attachments) => attachments.map(_chooseOpenRosaHash));

const getByFormDefIdAndNameForOpenRosa = (formDefId, name, options = QueryOptions.none, filterActorId = null) => ({ maybeOne }) =>
  _getWithDatasetMetadata(maybeOne, options.withCondition({ formDefId, name }), filterActorId)
