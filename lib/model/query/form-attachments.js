...
module.exports = {
  ...
  getAllByFormId: (formId, options) => ({ FormAttachment, formAttachments, simply }) => ((options.extended === false)
    ? simply.getWhere('form_attachments', { formId }, FormAttachment)
    : formAttachments._getExtended({ formId })),

  // we need to join against blob to get the md5.
  getAllByFormIdForOpenRosa: (formId) => ({ FormAttachment, db }) =>
    db.select(FormAttachment.fields().concat([ 'md5' ]))
  ...
  update: (fa) => ({ db }) =>
    db.update({ blobId: fa.blobId }).into('form_attachments')
      .where({ formId: fa.formId, name: fa.name })
      .then(wasUpdateSuccessful),

  _getExtended: (condition = []) => ({ db, FormAttachment }) =>
    db.select(FormAttachment.extendedFields())
      .from('form_attachments')
      .where(condition)
      .leftOuterJoin(
