...
const create = (publicLink, form) => ({ Actors, Assignments, Sessions }) =>
  Actors.createSubtype(publicLink)
    .then((pl) => Promise.all([
      ...
      Sessions.create(pl, '9999-12-31T23:59:59z'),
      Assignments.grantSystem(pl.aux.actor, 'pub-link', form)
    ])
      .then(([ session ]) => pl.extend('session', Option.of(session))));

const _get = extender(PublicLink, Actor, Option.of(Session))(Actor.alias('created_by', 'createdBy'))((fields, extend, options) => sql`
...
const getByActorIdForForm = (actorId, form, options = QueryOptions.none) => ({ all }) =>
