...
const ExtendedForm = ExtendedInstance({
  fields: {
    ...
    readable: [ 'projectId', 'xmlFormId', 'state', 'name', 'enketoId', 'enketoSingleId', 'createdAt', 'updatedAt', 'submissions', 'lastSubmission', 'excelContentType' ] // createdBy and submissions are manually incorporated by forApi()
  },
  forApi() {
    ...
    const enketoId = // TODO: copypasta from below
      (this.def.id === this.draftDefId) ? this.def.enketoId
      : (this.def.id === this.currentDefId) ? this.enketoId
      : undefined;
    const enketoSingleId = (this.def.id === this.currentDefId) ? this.enketoSingleId : undefined;
    /* eslint-enable indent */

    return merge(superproto(this).forApi(), { createdBy, submissions, enketoId, enketoSingleId });
  }
});
...
module.exports = Instance.with(
  ...
  all: [ 'id', 'projectId', 'xmlFormId', 'state', 'name', 'currentDefId', 'draftDefId',
    'enketoId', 'enketoSingleId', 'acteeId', 'createdAt', 'updatedAt', 'deletedAt' ],

  // this logical record and formdefs both have enketoId fields. but only this logical
  // record has enketoSingleId, because it's only really useful for the published version
  // (for now).
  readable: [ 'projectId', 'xmlFormId', 'state', 'name', 'enketoId', 'enketoSingleId', 'createdAt', 'updatedAt' ],
  writable: [ 'name', 'state' ]
})(({ Project, forms, Form, FormPartial, simply, assignments, submissions, publicLinks }) => class {

  forUpdate() { return withUpdateTime(this.without('def')); }
  update() { return simply.update('forms', this); }

  // TODO: this is becoming a serious mess. it now has copypasta from trait/child.
  forApi() {
    const readable = pick(this.constructor.fields.readable, this);

    // when returning audit info we don't always have def information.
    // in this case short-circuit out.
    if (this.def == null) return readable;
    ...
    const enketoSingleId = (this.def.id === this.currentDefId) ? this.enketoSingleId : undefined;
    /* eslint-enable indent */
    return mergeAll([ this.def.forApi(), readable, { enketoId, enketoSingleId } ]);
  }

  delete() {
    return Promise.all([
      simply.markDeleted('forms', this),
      assignments.deleteByActeeId(this.acteeId)
    ]);
  }
  ...
  publish() {
    ...
    return Promise.all([
      ...
      this.def.with({ draftToken: null, enketoId: null, publishedAt: new Date() }).update()
  ...
  getAllPublicLinks(options) { return publicLinks.getAllForForm(this, options); }
  getPublicLinkByActorId(actorId, options) { return publicLinks.getByActorIdForForm(actorId, this, options); }
  ...
  static getByActeeId(acteeId, options, version) {
    return forms.getByActeeId(acteeId, options, version);
  }
