...
class Form extends Frame.define(
  ...
  static fromXml(xml) {
    ...
    return (typeof xml === 'string')
      ? parse(xml).then((form) =>
        form.auxWith('def', new Form.Def({ hash: md5sum(xml), sha: shasum(xml), sha256: sha256sum(xml), xml })))
      : pipethrough(xml, digestWith('md5'), digestWith('sha1'), digestWith('sha256'),
        ...
        .then(([ hash, sha, sha256, [ partial, buffer ] ]) =>
          partial.auxWith('def', new Form.Def({ hash, sha, sha256, xml: buffer.toString('utf8') })));
  }
