...
const { resolve, ignoringResult, getOrReject } = require('../../util/promise');
...
module.exports = Instance.with(
  ...
  writable: [ 'name', 'state' ]
})(({ simply, Form, forms, FormAttachment, FormDef, Transformation }) => class {
  ...
  createNewVersion(def, makeCurrent = true) {
    return Promise.all([
      // get the identity transformation id (TODO: three queries deep feels bad)
      ...
      simply.getOneWhere('transformations', { system: 'identity' }, Transformation)
        ...
        .then((identityTransformation) =>
          def.with({ formId: this.id, transformationId: identityTransformation.id }).create()
