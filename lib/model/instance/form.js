...
const ExtendedForm = ExtendedInstance({
  fields: {
    ...
    readable: [ 'projectId', 'xmlFormId', 'state', 'name', 'enketoId', 'enketoOnceId', 'createdAt', 'updatedAt', 'submissions', 'lastSubmission', 'excelContentType' ] // createdBy and submissions are manually incorporated by forApi()
  },
  forApi() {
    ...
    return merge(superproto(this).forApi(), { createdBy, submissions, enketoId, enketoOnceId });
  }
});
...
module.exports = Instance.with(
  ...
  all: [ 'id', 'projectId', 'xmlFormId', 'state', 'name', 'currentDefId', 'draftDefId',
    'enketoId', 'enketoOnceId', 'acteeId', 'createdAt', 'updatedAt', 'deletedAt' ],

  // this logical record and formdefs both have enketoId fields. but only this logical
  // record has enketoOnceId, because it's only really useful for the published version
  // (for now).
  readable: [ 'projectId', 'xmlFormId', 'state', 'name', 'enketoId', 'enketoOnceId', 'createdAt', 'updatedAt' ],
  writable: [ 'name', 'state' ]
  ...
  forApi() {
    ...
    const enketoOnceId = (this.def.id === this.currentDefId) ? this.enketoOnceId : undefined;
    /* eslint-enable indent */
    return mergeAll([ this.def.forApi(), readable, { enketoId, enketoOnceId } ]);
  }
  ...
  static getByActeeIdForUpdate(acteeId, options, version) {
    return forms.getByActeeIdForUpdate(acteeId, options, version);
  }

  getAllSubmitters() { return forms.getAllSubmitters(this.id); }
