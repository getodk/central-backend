...
module.exports = Instance.with(
  ...
  writable: [ 'name', 'state' ]
})(({ Project, forms, FormPartial, simply }) => class {
  ...
  setManagedKey(key, suffix) {
    ...
    if (this.def.keyId != null) return resolve();

    return injectPublicKey(this.def.xml, key.public)
      ...
      .then((partial) => partial.createVersion(this));
  }
  ...
  static getByProjectAndXmlFormId(projectId, xmlFormId, options) {
    return forms.getByProjectAndXmlFormId(projectId, xmlFormId, options);
  }
  static getWithXmlByProjectAndXmlFormId(projectId, xmlFormId, options) {
    return forms.getWithXmlByProjectAndXmlFormId(projectId, xmlFormId, options);
  }
});
