...
class Form extends Frame.define(
  ...
  forApi() {
    const inherited = Frame.prototype.forApi.call(this);
    if (this.def == null) return inherited;
    ...
    return Object.assign(inherited, { enketoId, enketoOnceId });
  }
  ...
  static fromXml(xml) {
    ...
    return (typeof xml === 'string')
      ? parse(xml).then((form) =>
        form.auxWith('def', { hash: md5sum(xml), sha: shasum(xml), sha256: sha256sum(xml), xml }))
      : pipethrough(xml, digestWith('md5'), digestWith('sha1'), digestWith('sha256'),
        ...
        .then(([ hash, sha, sha256, [ partial, buffer ] ]) =>
          partial.auxWith('def', { hash, sha, sha256, xml: buffer.toString('utf8') }));
  }
...
Form.Xml = Frame.define(table('form_defs'), 'xml');
