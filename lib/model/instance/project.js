...
module.exports = Instance.with(ActeeTrait('project'), HasExtended(ExtendedProject))('projects', {
...
})(({ simply, fieldKeys, forms, formDefs, projects, Key }) => class {
  ...
  setManagedEncryption(passphrase, hint) {
    ...
    return generateManagedKey(passphrase)
      .then((keys) => formDefs.lock() // lock!
        ...
        .then(([ projectForms, key ]) => Promise.all([
          this.with({ keyId: key.id }).update(),
          Promise.all(projectForms.map((form) => form.setManagedKey(key, suffix)))
        ]))
  ...
  getAllFormsWithXml(options) { return projects.getFormsWithXml(this.id, options); }
  getFormsByAuthForOpenRosa(auth, options) { return forms.getByAuthForOpenRosa(this.id, auth, options); }
