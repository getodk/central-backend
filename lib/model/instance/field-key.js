...
const ExtendedFieldKey = ExtendedInstance({
  fields: {
    joined: [ 'lastUsed' ],
    readable: [ 'createdBy', 'lastUsed', 'createdBy', 'projectId' ] // token is manually incorporated by forApi()
  },
  forApi() {
...
module.exports = Instance.with(
  ActeeSpeciesTrait('field_key'),
  HasExtended(ExtendedFieldKey),
  ChildTrait('Actor')
)('field_keys', {
  all: [ 'actorId', 'createdBy', 'projectId' ],

  // token is manually incorporated by forApi(), and
  // createdBy is only available via extended request.
  readable: [ 'projectId' ],

  writable: []
})(({ Session, fieldKeys }) => class {
