...
module.exports = Instance('form_defs', {
  all: [ 'id', 'formId', 'keyId', 'xml', 'version', 'hash', 'sha', 'sha256', 'createdAt', 'xlsBlobId' ],
  readable: [ 'version', 'hash', 'sha', 'sha256', 'keyId' ],
  writable: []
})(({ simply, FormDef, FormField }) => class {
  ...
  schema() { return getFormSchema(this.xml); }
  tables() { return this.schema().then(getSchemaTables); }

  getBinaryFields() {
    return simply.getWhere('form_fields', { formDefId: this.id, binary: true }, FormField);
  }
});
