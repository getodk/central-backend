...
// system.
const builder = (table, fields) => (traitDefs) => {
  ...
  return [ Instance, (container) => {
    ...
    for (const traitDef of traitDefs) traits.push(traitDef(container));
...
const Instance = (table, fields, fieldOptions) => (def) =>
  builder(table, fields, fieldOptions)([ def ]);
Instance.with = (...traits) => (table, fields, fieldOptions) => (def) =>
  builder(table, fields, fieldOptions)(traits.concat([ def ]));
