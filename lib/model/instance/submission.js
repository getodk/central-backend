  ...
  static fromXml(input) {
    ...
    return process
      .then(([ [ idText, metaInstanceId, metaInstanceName, attrInstanceId, versionText, localKeyText, encDataAttachmentNameText, signatureText ], xml ]) => {
        ...
        return new Submission.Partial({ xmlFormId, instanceId },
          { def: new Submission.Def({ instanceName, version, xml, localKey,
            encDataAttachmentName, signature }) });
      });
  }
}
...
Submission.Def = Frame.define(
  ...
  'instanceName', readable
);

Submission.Xml = Frame.define(table('submission_defs'), 'xml');

Submission.Export = Frame.define('encData', 'encIndex',  'encKeyId');
