...
module.exports = Instance.with(ActeeSpeciesTrait('user'), ChildTrait('Actor'))('users', {
  ...
  readable: [ 'email' ],
  writable: [ 'email' ]
})(({ Actor, Session, User, users, crypto, simply }) => class {
  ...
  provisionPasswordResetToken() {
    ...
    return (new Actor({ type: Actor.types().singleUse, expiresAt, displayName, meta }))
      ...
      .then((actor) => Promise.all([
        actor.assignSystemRole('pwreset', this.actor),
        Session.fromActor(actor).create()
      ]))
  ...
  withHashedPassword(cleartext) {
    return crypto.hashPassword(cleartext).then((password) => this.with({ password }));
  }
  ...
  updatePassword(plain) {
    return crypto.hashPassword(plain).then((hash) => users.updatePassword(this, hash));
  }
  ...
  static getAll(options) { return users.getAll(options); }
  static getByEmail(email) { return users.getOneWhere({ email }); }
  static getByActorId(actorId) { return users.getOneWhere({ actorId }); }
  static emailEverExisted(email) {
    return simply.getOneWhere('users', { email }, User).then((opt) => opt.isDefined());
  }
});
