...
const { getFormSchema, stripNamespacesFromSchema, schemaToFields, expectedFormAttachments, injectPublicKey, addVersionSuffix } = require('../../data/schema');
...
module.exports = Instance()(({ simply, Blob, Form, forms, FormDef, FormAttachment, Key, FormPartial, xlsform }) => class {
  ...
  createNew() {
    ...
    return Promise.all([
      ...
      getFormSchema(this.xml).then(stripNamespacesFromSchema)
    ])
      .then(([ keyId, schema ]) => Promise.all([
        forms.create(form.with({ def: { keyId, schema: JSON.stringify(schema) } })),
        ...
        .then(([ savedForm, expectedAttachments ]) => {
          ...
          return Promise.all([
            ...
            simply.insert('form_fields', schemaToFields(schema).map(withIds))
  ...
  createVersion(form, makeCurrent = true) {
    ...
    return Promise.all([
      ...
      // database, and mark it as current if requested.
      ...
      getFormSchema(this.xml).then(stripNamespacesFromSchema).then(schemaToFields)
    ])
