...
module.exports = Instance('submission_defs', {
...
})(({ Blob, ClientAudit, SubmissionAttachment, submissionAttachments, submissionDefs }) => class {
  ...
  //
  // TODO: right now we create a second readstream from the filesystem in the case where
  // a client audit log must be processed. someday there are gains to be made in reusing
  // one stream for both tasks, and only calculating the SHA once. but this results in a
  // rather labyrinthine controlflow problem as it stands now.
  createExpectedAttachments(formDef, files = []) {
    const fileLookup = {};
    for (const file of files) { fileLookup[file.fieldname] = file; }

    const makeAttachment = (name, index = null) => {
      ...
      return makeBlobId.then((blobId) => submissionAttachments.create(
        new SubmissionAttachment({ submissionDefId: this.id, blobId, name, index })
      ));
    };

    return (this.localKey == null)
      ? submissionXmlToFieldStream(formDef, this.xml)
        .then(mapStreamToPromises((object) => {
          // bail early if we have an irrelevant binding type or an empty xml node.
          if (object.field.type !== 'binary') return Option.none();
          const name = object.text.trim();
          if (isBlank(name)) return Option.none();

          // if we have a client audit log on our hands, we hand off the work to a
          // different path altogether because the branching and converging data needs
          // for that processing become quite complex. otherwise use makeAttachment.
          const isClientAudit = (object.field.name === 'audit') &&
            (object.path.length === 1) && (object.path[0] === 'meta');
          const file = Option.of(fileLookup[name]);

          return Option.of(isClientAudit
            ? ClientAudit.intake(this, name, file.map((f) => f.path).orNull())
            : makeAttachment(name));
        }))
  ...
  upsertAttachments(files) {
    return this.getAttachmentMetadata().then((expecteds) => {
      const work = [];
      const lookup = {};
      for (const expected of expecteds) lookup[expected.name] = expected;
      for (const file of files) {
        const expected = lookup[file.fieldname];
        if (expected != null) {
          const insertBlob = (expected.isClientAudit === true)
            ? ClientAudit.attachFromFile(file.path)
            : Blob.fromFile(file.path, file.mimetype)
              .then((blob) => blob.create())
              .then((blob) => blob.id);
          work.push(insertBlob.then(this.attach(expected.name)));
        }
      }
      return Promise.all(work);
    });
  ...
  // file name. curries.
  attach(name, blobId) {
    if (arguments.length === 1) return (y) => this.attach(name, y);
    return submissionAttachments.update(new SubmissionAttachment({
      submissionDefId: this.id, blobId, name
    }));
  }
