...
module.exports = Instance('submission_defs', {
  ...
  createExpectedAttachments(formDef, files = []) {
    const makeAttachment = (name, index = null) => {
      ...
      return makeBlobId.then((blobId) =>
        submissionAttachments.create(new SubmissionAttachment({ submissionDefId: this.id, blobId, name, index })));
    };
    ...
    return (this.localKey == null)
      ...
      : traverseXml(this.xml, [ findAll(root(), node('media'), node('file'))(text()) ])
        .then(([ maybeNames ]) => {
          ...
          names.push(this.encDataAttachmentName);
          return names;
        })
        .then((names) => Promise.all(names.map((name, idx) => makeAttachment(name.trim(), idx))));
  }
  ...
  static streamForExport(formId, encryptedBlobs, options) {
    return submissionDefs.streamForExport(formId, encryptedBlobs, options);
  }
});
