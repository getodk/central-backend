...
// takes eg 'md5'/'sha1', then a stream and a cb func, then computes the digest
// with that algo and returns a promise of the result.
const digestWith = (algo) => (stream) => new Promise((resolve) => {
  stream.pipe(digest(algo, 'hex', resolve));
});
...
module.exports = Instance(({ Blob, simply }) => class {
  ...
  static fromStream(stream, contentType) {
    return calculateAndBuffer(stream, digestWith('md5'), digestWith('sha1'))
