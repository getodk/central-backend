...
module.exports = Instance(({ all, Blob, simply }) => class {
  ...
  //
  // TODO: find some nice stream-processing md5 npm lib for this. md5 is pretty fast
  // so it's probably okay for now.
  // TODO: perhaps use the new cloneable-readable to read from fs and mux to multiple
  // sinks here.
  static fromFile(path, contentType) {
    ...
    ]).then(([ sha, buffer ]) => new Blob({ sha, md5: md5sum(buffer), contentType, content: buffer }));
  }
