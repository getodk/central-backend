const up = async (db) => {
  ...
  // Set webformsEnabled=false to the rest (orphan drafts)
  await db.raw(`
    UPDATE form_defs
    SET "webformsEnabled" = FALSE
    WHERE "publishedAt" IS NULL AND "webformsEnabled" IS NULL
  `);
  ...
  await db.raw(`
    ALTER TABLE form_defs
      ADD CONSTRAINT check_webforms_enabled_matches_publish_state
      CHECK (
        ("publishedAt" IS NOT NULL AND "webformsEnabled" IS NULL) OR
        ("publishedAt" IS NULL AND "webformsEnabled" IS NOT NULL)
      );
  `);
};
...
const down = async (db) => {
  ...
  await db.raw(`
    ALTER TABLE form_defs
      DROP CONSTRAINT check_webforms_enabled_matches_publish_state;
  `);
