...
const up = (knex) =>
  knex.schema.table('blobs', (blobs) => { blobs.string('md5', 32); })
    .then(() => require('../package').withDefaults({ db: knex }).transacting(({ all, Blob, simply }) => {
      ...
      return simply.getAll('blobs', UpdatableBlob)
        .then((blobs) => all.do(blobs.map((blob) =>
          simply.update('blobs', blob.with({ md5: md5sum(blob.content) })))));
    }))
