...
const up = (db) =>
  ...
  // the project's name will vary.
    .then(() => {
      const { Project } = require('../package').withDefaults({ db });
      return db.count('*').from('forms').where({ deletedAt: null }).limit(1)
        .then(([{ count }]) => {
          const name = (Number(count) > 0)
            ? 'Forms you made before projects existed'
            : 'Default Project';
          return (new Project({ name })).create();
        });
    })
