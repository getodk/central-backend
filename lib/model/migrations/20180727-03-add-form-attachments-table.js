...
const up = (knex) =>
  knex.schema.createTable('form_attachments', (fa) => {
    ...
    fa.index([ 'formId' ]);
  }).then(() => require('../package').withDefaults({ db: knex }).transacting(({ simply, Form }) => {
    ...
    return simply.getAll('forms', Form)
      .then((forms) => Promise.all(forms.map((form) => expectedFormAttachments(form.xml)
        .then((expected) => {
