...
// a project that DOES have a keyId, we throw ODK03 and either the business logic
...
create or replace function check_managed_key() returns trigger as $check_managed_key$
  ...
  begin
    select "keyId" into "projectKeyId" from projects
      inner join forms on forms.id = NEW."formId"
      where projects.id = forms."projectId";
    if "projectKeyId" is not null and NEW."keyId" is null then
      raise exception 'ODK03';
    end if;
    return NEW;
  end;
...
`);
  ...
  await db.raw(`create trigger check_managed_key before insert or update on form_defs
