...
AS
    $BODY$
        WITH deleted AS (
            DELETE FROM eventcounters WHERE "acteeId" = actee_id RETURNING evt_cnt
        )
        INSERT INTO eventcounters ("acteeId", evt_cnt) VALUES (actee_id, (SELECT COALESCE(SUM(evt_cnt), 0) FROM deleted)) RETURNING evt_cnt
    $BODY$
LANGUAGE sql
VOLATILE
STRICT
...
COMMENT ON TRIGGER "eventcounter_bump_trigger" ON "public"."audits" IS '{"dbsamizdat": {"version": 1, "definition_hash": "02c47d256b34248644c5eddb21fecf5a"}}';
