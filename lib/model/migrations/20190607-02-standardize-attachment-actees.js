...
const up = async (db) => {
  ...
  // 4. and create jsonb indices for the formDefId property.
  await db.raw(`create index audits_details_formdefid_gin_index on audits using gin ((details->'formDefId'))`);
  await db.raw(`create index audits_details_name_gin_index on audits using gin ((details->'name'))`);
};
