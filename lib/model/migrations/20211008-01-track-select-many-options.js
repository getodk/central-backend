...
const up = async (db) => {
  ...
  await new Promise((resolve, reject) => {
    ...
    stream.on('data', ({ id, xml }) => {
      work.push(getFormFields(xml).then((fields) => Promise.all(fields
        .filter((field) => field.selectMany === true)
        .map((field) => db
          .update({ selectMany: true })
          .into('form_fields')
  ...
  await new Promise((resolve, reject) => {
    ...
    stream.on('data', ({ formId, id, xml }) => {
      work.push(getSelectManyResponses(fieldsByFormId[formId], xml)
        .then((pairs) => {
          const inserts = [];
