...
const up = (db) =>
  db.schema.table('grants', (grants) =>
    ...
    return Promise.all([
      db.insert(acteeSpecies).into('actees'),
      systemActors.map((actor) =>
        db.insert({ id: uuid(), species: 'system' }).into('actees').returning('id').then(([{ id }]) => id)
          .then((acteeId) => db.insert(Object.assign({ acteeId }, actor)).into('actors')))
    ])
      .then(() => db.insert({ actorId: admins.id, verb: '*', acteeId: '*', system: true }).into('grants'));
  });
/* eslint-enable */
