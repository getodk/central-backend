...
// this class is now deprecated; this shell remains to ensure continued operation
// of this migration.
class Grant {
...
const up = (db) =>
  db.schema.table('grants', (grants) =>
  ...
  )).then(() => require('../package').withDefaults({ db }).transacting(({ Actee, Actor, simply }) => {
    ...
    return Promise.all(acteeSpecies.map((actee) => actee.create()))
      .then(() => Promise.all(systemActors.map((actor) => actor.create())))
