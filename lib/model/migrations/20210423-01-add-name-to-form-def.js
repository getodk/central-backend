...
const up = async (db) => {
  ...
  didn't.) We tried a couple of alternatives to `for await`, using `data` and
  `readable` events. However, in each case, the process was killed after 100
  definitions; so it seems necessary to set highWaterMark. This Knex issue may
  be related: https://github.com/knex/knex/issues/484 */
