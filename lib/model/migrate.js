...
const getPostKnexClient = async config => {
  const log = (...args) => console.log('[getPostKnexClient]', ...args); // eslint-disable-line no-console
  log('ENTRY');
...
const postKnexMigrations = async (config) => {
  ...
  const client = await getPostKnexClient(config);
  ...
  try {
    ...
    const toRun = getPostKnexMigrationsToRun(client);
...
module.exports = { checkKnexMigrations, checkPostKnexMigrations, initKnex, withKnex, knexMigrations, postKnexMigrations };
