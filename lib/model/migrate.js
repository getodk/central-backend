...
const postKnexMigrations = async (config) => {
  const log = (...args) => console.log('[postKnexMigrations]', ...args);
  log('ENTRY');
  ...
  try {
    ...
    for(const { migration, name, path } of toRun) {
      log('Validing migration:', name, '...');

      if(name.length > 255) throw new Error(`Migration name '${name}' is too long - max length is ${maxLen}, but got ${name.length}`);
    ...
    log('Ending client...');
    await client.end(); // TODO needs await or callback?
    log('Client ended.');
  }
};
...
module.exports = { checkMigrations, initKnex, withKnex, knexMigrations, postKnexMigrations };
