...
class Form extends Frame.define(
  ...
  _webformsEnabledForApi() {
    if (this.def == null) return undefined; // in audits general query, we don't request defs
    if (this.isCurrentDraft()) return this.def.webformsEnabled;
    if (this.def.id === this.currentDefId) return this.webformsEnabled;
    return undefined; // For versions API, we don't return `webformsEnabled`
  }
  ...
  forApi() {
    ...
    const enketoOnceId = this.def?.id === this.currentDefId
      ? this.enketoOnceId
      : null;
    const webformsEnabled = this._webformsEnabledForApi();
