...
const { Frame, table, readable, writable, into, embedded, fieldTypes, from } = require('../frame');
...
const AnyVersion = Symbol('any version');
const WithoutDef = Symbol('no def required');
const IncludeXml = Symbol('xml:include');
const WithoutXml = Symbol('xml:exclude');
...
class Form extends Frame.define(
  ...
  get associated() { return this.aux.formAssociated; }

  isCurrentDraft() {
    return this.draftDefId && this.def?.id === this.draftDefId;
  }
  ...
  _enketoIdForApi() {
    ...
    if (this.isCurrentDraft()) return this.def.enketoId;
  ...
  forApi() {
    ...
    // Sometime we don't request def or there is no (draft) def, in those cases we read the latest
    // `webformsEnabled`;
    const webformsEnabled = this.def?.webformsEnabled ?? this.associated?.webformsEnabled;
    ...
    return Object.assign(Frame.prototype.forApi.call(this), { enketoId, enketoOnceId, webformsEnabled }, deletedAtIdFields);
  }
  ...
  /** Published version gets first priority */
  static get AnyVersion() { return AnyVersion; }
  static get WithoutDef() { return WithoutDef; }

  static get IncludeXml() { return IncludeXml; }
  static get WithoutXml() { return WithoutXml; }
  static withXml(withXml) { return withXml ? IncludeXml : WithoutXml; }
}

Form.Partial = class extends Form {};

Form.Associated = Frame.define(
  from('formAssociated'), into('formAssociated'),
...
Form.Def = class extends Frame.define(
  table('form_defs', 'def'),
  'id',                                 'formId',
