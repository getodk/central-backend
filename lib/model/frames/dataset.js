...
class Dataset extends Frame.define(
  table('datasets'),
  'id',                                        'name', readable,
...
Dataset.LinkedForm = class extends Frame.define(
  'name', readable,
  'xmlFormId', readable,
  'isRepeat', readable,
  'path', readable,
  ...
  forApi(includeDraft=false) {
    return {
      name: this.name,
      xmlFormId: this.xmlFormId,
      ...(includeDraft && { draft: this.draft }),
      ...(this.isRepeat && this.path && { repeatPath: this.path })
    };
  }
};
