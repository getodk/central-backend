...
class Blob extends Frame.define(table('blobs'), 'id', 'sha', 'content', 'contentType', 'md5', 's3_status') {
  ...
  static fromStream(stream, contentType) {
    return pipethroughAndBuffer(stream, digestWith('md5'), digestWith('sha1'))
      .then(([ md5, sha, buffer ]) => new Blob({ contentType, md5, sha, content: buffer, s3_status: 'pending' })); // TODO setting s3_status should not be necessary as the column defaults to pending anyway
  }
  ...
  static fromBuffer(buffer, contentType) {
    return new Blob({
      ...
      content: buffer,
      contentType,
      s3_status: 'pending', // TODO setting s3_status should not be necessary as the column defaults to pending anyway
    });
  }
}
