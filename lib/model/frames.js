...
class Audit extends Frame.define(
  ...
  forApi() {
    ...
    return Object.assign(base, {
      actor: this.aux.actorActor.map((x) => x.forApi()).orNull(),
...
class FieldKey extends Frame.define(
  ...
  forApi() {
    const result = Frame.prototype.forApi.call(this.withAux('session', null));
    result.token = Option.of(this.aux.session).map((s) => s.token).orNull();
    return result;
  }
}
