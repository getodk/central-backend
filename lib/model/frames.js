...
const { identity, pick } = require('ramda');
...
// configKeys holds options for each config. It determines whether the config
// can be directly set over the API, and if so, how. It also determines
// whether/how the config value is returned over the API and saved in the audit
// log. (Using Object.create(null) to prevent access to Object.prototype
// properties.)
const configKeys = Object.assign(Object.create(null), {
  analytics: {
    fromApi: (value) => {
      ...
      if (result.enabled) {
        ...
        if ((typeof value.organization === 'string') && (value.organization.length !== 0))
  ...
  'backups.google': {
    forApi: () => 'New or refreshed Google API credentials'
  }
});
...
class Config extends Frame.define(
  ...
  // Returns `true` if the config for `key` can be directly set over the API and
  // `false` if not.
  static settableFromApi(key) {
    return (configKeys[key] != null) && (configKeys[key].fromApi != null);
  }

  static valueFromApi(key, value) {
    return this.settableFromApi(key) ? configKeys[key].fromApi(value) : null;
  }

  forApi() {
    const data = { key: this.key, setAt: this.setAt };
    const forApi = configKeys[this.key] != null ? configKeys[this.key].forApi : null;
    if (forApi != null) data.value = forApi(this.value);
    return data;
  }
}
