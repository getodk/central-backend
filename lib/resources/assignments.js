...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:id/assignments/:roleId', endpoint(({ Assignment, Project, Role }, { auth, params, queryOptions }) =>
    Promise.all([
  ...
  service.post('/projects/:id/assignments/:roleId/:actorId', endpoint(({ Actor, Assignment, Project, Role }, { auth, params }) =>
    Promise.all([
      Project.getById(params.id).then(getOrNotFound),
      getRoleByParam(Role, params.roleId).then(getOrNotFound),
      Actor.getById(params.actorId).then(getOrNotFound)
      ...
      .then(([ project, role, actor ]) => auth.canOrReject('assignment.create', project)
        .then(() => actor.assignRole(role, project))
        .then(success))));

  service.delete('/projects/:id/assignments/:roleId/:actorId', endpoint(({ Actor, Assignment, Project, Role }, { auth, params }) =>
    Promise.all([
      Project.getById(params.id).then(getOrNotFound),
      getRoleByParam(Role, params.roleId).then(getOrNotFound),
      Actor.getById(params.actorId).then(getOrNotFound)
      ...
      .then(([ project, role, actor ]) => auth.canOrReject('assignment.delete', project)
        .then(() => actor.unassignRole(role, project))
        .then((result) => (result === true) ? success() : Problem.user.notFound()))));
