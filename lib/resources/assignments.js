    ...
    Promise.all([
      ...
      auth.canOrReject('assignment.create', Assignment.species()) // sitewide assignment rights
        .then(() => Promise.all([
          ...
          Audit.log(auth.actor(), 'assignment.create', actor, { role: role.id, acteeId: '*' })
        ]))
        .then(success))));
    ...
    Promise.all([
      ...
      auth.canOrReject('assignment.delete', Assignment.species()) // sitewide assignment rights
        .then(() => Promise.all([
          ...
          Audit.log(auth.actor(), 'assignment.delete', actor, { role: role.id, acteeId: '*' })
        ]))
    ...
    Promise.all([
      ...
      .then(([ project, role, actor ]) => auth.canOrReject('assignment.create', project)
        .then(() => Promise.all([
          ...
          Audit.log(auth.actor(), 'assignment.create', actor, { role: role.id, acteeId: project.acteeId })
        ]))
        .then(success))));
    ...
    Promise.all([
      ...
      .then(([ project, role, actor ]) => auth.canOrReject('assignment.delete', project)
        .then(() => Promise.all([
          ...
          Audit.log(auth.actor(), 'assignment.delete', actor, { role: role.id, acteeId: project.acteeId })
        ]))
        .then(([ result ]) => ((result === true) ? success() : Problem.user.notFound())))));
