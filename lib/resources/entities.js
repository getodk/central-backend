...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/datasets/:name/entities', endpoint(async ({ Datasets, Entities }, { params, auth, queryOptions }) => {
    ...
    const entities = await Entities.getAll(dataset.id, queryOptions); //populateEntities();

    // excluding data from the currentVersion
    return entities.map(e => {
      const defMetadata = new Entity.Def.Metadata(e.aux.currentVersion, e.aux.currentVersion.aux);
