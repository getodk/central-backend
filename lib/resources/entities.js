...
const BULK_ENTITY_BATCH_SIZE = 10;
const BULK_ENTITY_BATCH_DELAY = 250;
const acceptsEventStream = (request) => request.headers.accept?.includes('text/event-stream') === true;
const writeEvent = (response, event, data) => {
  response.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`);
  response.flush?.();
};
const wait = (duration) => new Promise(resolve => { setTimeout(resolve, duration); });
const requestCancelled = () => Object.assign(new Error('Request cancelled by client.'), { code: 'ERR_REQUEST_ABORTED' });
const cancellation = (request, response) => {
  ...
  response.once('close', cancelIfUnfinished);
...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/datasets/:name/entities/:uuid', endpoint(async ({ Datasets, Entities }, { params, auth, queryOptions }) => {
    ...
    return entity.withAux('currentVersion', entity.aux.currentVersion.with({ data: pick(properties.map(p => p.name), entity.aux.currentVersion.data) }));
  }));
  ...
  service.get('/projects/:projectId/datasets/:name/entities/:uuid/versions', endpoint(async ({ Datasets, Entities, Audits }, { params, auth, queryOptions, query }) => {
    ...
    // TODO: check the impact of this on frontend
    const currentVersionIndex = defs.findIndex(d => d.current);
    const properties = await Datasets.getProperties(dataset.id);
    defs[currentVersionIndex] = defs[currentVersionIndex].with({ data: pick(properties.map(p => p.name), defs[currentVersionIndex].data) });
  ...
  service.get('/projects/:projectId/datasets/:name/entities/:uuid/geojson', endpoint.plain(async ({ Datasets, Entities, GeoExtracts }, { params, auth }) => {
    ...
    return GeoExtracts.getEntityFeatureCollectionGeoJson(dataset.id, `__id eq '${entityUuid}'`).then(json);
  }));


  // Create a single entity or bulk create multiple entities.
  // In either case, this appends new entities to a dataset.
  service.post('/projects/:id/datasets/:name/entities', endpoint(async ({ Datasets, Entities }, { auth, body, params, userAgent }, request, response) => {
    ...
    if (!entities) {
      ...
      const batchCount = Math.ceil(partials.length / BULK_ENTITY_BATCH_SIZE);
      const eventStream = acceptsEventStream(request);
      ...
      if (eventStream) {
        ...
        writeEvent(response, 'progress', { completed: 0, total: partials.length, batch: 0, batchCount });
      }
      // eslint-disable-next-line no-console
      console.log(`Creating ${partials.length} entities in ${batchCount} batch(es).`);
      try {
        for (let start = 0; start < partials.length; start += BULK_ENTITY_BATCH_SIZE) {
          requestCancellation.throwIfCancelled();
          const batch = partials.slice(start, start + BULK_ENTITY_BATCH_SIZE);
          const batchNumber = (start / BULK_ENTITY_BATCH_SIZE) + 1;
          // eslint-disable-next-line no-console
          console.log(`Creating entity batch ${batchNumber}/${batchCount} (${batch.length} entities).`);
          // eslint-disable-next-line no-await-in-loop
          await Entities.createMany(dataset, batch, sourceId, userAgent);
          requestCancellation.throwIfCancelled();
          // eslint-disable-next-line no-console
          console.log(`Created entity batch ${batchNumber}/${batchCount}.`);
          if (eventStream)
            writeEvent(response, 'progress', { completed: start + batch.length, total: partials.length, batch: batchNumber, batchCount });
          if (batchNumber < batchCount) {
            // eslint-disable-next-line no-await-in-loop
            await wait(BULK_ENTITY_BATCH_DELAY);
            requestCancellation.throwIfCancelled();
          }
        }
        if (eventStream) {
          writeEvent(response, 'complete', { success: true, total: partials.length });
          response.end();
        }
      } finally {
        requestCancellation.stopListening();
      }
      return success();
    }
  ...
  service.post('/projects/:projectId/datasets/:name/entities/bulk-delete', endpoint(async ({ Datasets, Entities }, { auth, body, params }) => {
    const dataset = await Datasets.get(params.projectId, params.name, true).then(getOrNotFound);
    ...
    const deletedUuids = await Entities.deleteByIds(ids, dataset);

    return { count: deletedUuids.length };
  }));

  service.post('/projects/:projectId/datasets/:name/entities/bulk-restore', endpoint(async ({ Datasets, Entities }, { auth, body, params }) => {
    const dataset = await Datasets.get(params.projectId, params.name, true).then(getOrNotFound);
    ...
    const restoredUuids = await Entities.restoreByIds(ids, dataset);

    return { count: restoredUuids.length };
  }));
};
