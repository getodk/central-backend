...
module.exports = (service, endpoint) => {
  ...
  // Use properties on Actors (app users and public links) to filter or segment datasets
  // by setting a filter rule using dataset properties and actor properties.

  // Add or update a single access filter rule for a dataset.
  // Body: { "datasetProperty": "<name>", "actorProperty": "<name>" }
  service.put('/projects/:projectId/datasets/:name/access-filter', endpoint(async ({ Datasets }, { auth, params, body }) => {
    const dataset = await Datasets.get(params.projectId, params.name, true).then(getOrNotFound);
    await auth.canOrReject('dataset.update', dataset);

    const { datasetProperty, actorProperty } = body ?? {};
    if (!datasetProperty || !actorProperty)
      throw Problem.user.unexpectedValue({ field: 'datasetProperty/actorProperty', value: null, reason: 'Both datasetProperty and actorProperty are required.' });
    await Datasets.setAccessFilter(dataset.id, datasetProperty, actorProperty);
    const filters = await Datasets.getAccessFilter(dataset.id);
    return filters.find((f) => f.datasetProperty === datasetProperty) ?? null;
  }));

  // Get all access filter rules for a dataset.
  service.get('/projects/:projectId/datasets/:name/access-filter', endpoint(async ({ Datasets }, { auth, params }) => {
    const dataset = await Datasets.get(params.projectId, params.name, true).then(getOrNotFound);
