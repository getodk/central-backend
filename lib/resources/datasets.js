...
module.exports = (service, endpoint) => {
  ...
  service.post('/projects/:id/datasets', endpoint(async ({ Projects, Datasets }, { auth, body, params }) => {
    ...
    return Datasets.createFromApi(new Dataset({ name, projectId: project.id }), project);
  }));
  ...
  service.post('/projects/:projectId/datasets/:name/properties', endpoint(async ({ Datasets }, { params, body, auth }) => {
    ...
    if (!validatePropertyName(name))
      throw Problem.user.invalidEntityForm({ reason: 'Invalid property name.' });

    return Datasets.createProperty(new Dataset.Property({ name, datasetId: dataset.id }));
  }));
