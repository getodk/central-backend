...
module.exports = (service, endpoint) => {
  ...
  service.post('/projects/:projectId/app-users', endpoint(({ FieldKey, Project }, { auth, body, params }) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('field_key.create', project)
        ...
        .then((fk) => fk.actor.assignSystemRole('app_user', project)
          .then(() => fk)))));

  service.delete('/projects/:projectId/app-users/:id', endpoint(({ FieldKey, Project }, { auth, params }) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('field_key.delete', project)
        .then(() => FieldKey.getByActorId(params.id)) // TODO/CR: need to check that the key is actually part of the project
        .then(getOrNotFound)
        .then((fk) => fk.delete())
        .then(success))));
