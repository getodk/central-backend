...
module.exports = (service, endpoint) => {
  ...
  service.delete('/projects/:projectId/app-users/:id', endpoint(({ Actors, Assignments, FieldKeys, Projects }, { auth, params }) =>
    Projects.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('field_key.delete', project))
      .then((project) => FieldKeys.getByActorIdForProject(params.id, project))
      .then(getOrNotFound)
      .then((fk) => Promise.all([
        Actors.delete(fk.actor),
        Assignments.deleteByActor(fk.actor)
      ]))
      .then(success)));
