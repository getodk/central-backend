...
const streamAttachment = async (container, attachment, filterActorId, response) => {
  ...
  if (attachment.blobId == null && attachment.datasetId == null) {
    ...
    return withEtag(attachment.openRosaHash, async () => {
      const options = filterActorId != null && attachment.aux?.dataset?.ownerOnly
        ? QueryOptions.none.withCondition({ 'entities.creatorId': filterActorId })
        : QueryOptions.none;
      // NOTE from Kathleen: Don't like having to *remind* streamForExport that it should filter by owner only
      // when it will automatically filter by prop filters
      const entities = await Entities.streamForExport(attachment.datasetId, options, filterActorId);
...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  service.delete('/projects/:projectId/forms/:xmlFormId', endpoint(async ({ Forms, Datasets }, { auth, params }) => {
    const form = await Forms.getByProjectAndXmlFormId(params.projectId, params.xmlFormId, Form.WithoutDef).then(getOrNotFound);
    await auth.canOrReject('form.delete', form);
    const relatedDatasets = await Datasets.getRelatedDatasetsOfForm(params.projectId, params.xmlFormId);
    await Forms.del(form, relatedDatasets);
    return success();
  }));
  ...
  service.post('/projects/:projectId/forms/:numericFormId/restore', endpoint(async ({ Forms, Datasets, Audits }, { auth, params }) => {
    const form = await Forms.getByProjectAndNumericId(params.projectId, params.numericFormId, Form.WithoutDef, Form.ExcludeXml, QueryOptions.none, true)
      .then(getOrNotFound);
    await auth.canOrReject('form.restore', form);

    // Get the latest form.delete audit for this form
    const audits = await Audits.get(new QueryOptions({
      condition: { acteeId: form.acteeId },
      args: { action: 'form.delete' },
      limit: 1
    }));

    // skip it if audit log or the information we are looking for is missing
    if (audits.length > 0 && audits[0].details && audits[0].details.targetDatasets && audits[0].details.linkedDatasets) {
      const { targetDatasets: targetDatasetsAtDelete, linkedDatasets: linkedDatasetsAtDelete } = audits[0].details;

      const uniqueDatasetId = new Set([...targetDatasetsAtDelete.map(d => d.id), ...linkedDatasetsAtDelete.map(d => d.id)]);
      const presentDatasets = await Datasets.getListWithProperties(params.projectId, [...uniqueDatasetId]);

      if (uniqueDatasetId.size !== presentDatasets.length) {
        return reject(Problem.user.datasetMissingForFormRestore());
      }

      // all properties of targetDataset should still be there
      for (const ds of targetDatasetsAtDelete) {
        const presentProps = new Set(presentDatasets.find(i => i.id === ds.id).properties.map(p => p.name));
        if (ds.properties.some(p => !presentProps.has(p.name))) {
          return reject(Problem.user.datasetMissingForFormRestore());
        }
      }
    }

    await Forms.restore(form);
    return success();
  }));
