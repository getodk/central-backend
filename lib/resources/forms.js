...
const { sanitizeFieldsForOdata } = require('../data/schema');
...
module.exports = (service, endpoint) => {
  ...
  // the projectId.
  const getPartial = (request, project, FormPartial, Key) =>
    (isExcel(request.headers['content-type'])
      ? FormPartial.fromXls(request, request.headers['content-type'],
        request.headers['x-xlsform-formid-fallback'], isTrue(request.query.ignoreWarnings))
      : FormPartial.fromXml(request.body))
  ...
  // TODO: allow empty POST
  // verify xmlformid
  service.post('/projects/:projectId/forms/:id/draft', endpoint(({ Audit, FormPartial, Key, Project }, { params, auth }, request) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => project.getFormByXmlFormId(params.id)
        .then(getOrNotFound)
        .then((form) => auth.canOrReject('form.update', form))
        .then((form) => getPartial(request, project, FormPartial, Key)
          .then((partial) => partial.createVersion(form, false))
          // unfortunately we now have to grab the form from the database again
          // to find out the new draft id to log audit.
          .then(() => project.getFormByXmlFormId(form.xmlFormId))
          .then((updatedForm) => Audit.log(auth.actor(), 'form.update.draft.set', form, { newDraftDefId: updatedForm.draftDefId })))
        .then(success))));

  service.post('/projects/:projectId/forms/:id/draft/publish', endpoint(({ Audit, Form }, { params, auth }) =>
  ...
  service.delete('/projects/:projectId/forms/:id/draft', endpoint(({ Audit, Form }, { params, auth }) =>
    Form.getByProjectAndXmlFormId(params.projectId, params.id)
      ...
      .then((form) => Promise.all([
        ...
        Audit.log(auth.actor(), 'form.update.draft.clear', form, { oldDraftDefId: form.draftDefId })
      ]))
      .then(success)));
  ...
  service.get('/projects/:projectId/forms/:id/versions', endpoint(({ Form }, { auth, params }) =>
    Form.getByProjectAndXmlFormId(params.projectId, params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('form.read', form))
      .then((form) => form.getVersions())));
