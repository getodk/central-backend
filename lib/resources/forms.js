...
const streamAttachment = async (container, attachment, auth, response) => {
  ...
  if (attachment.blobId == null && attachment.datasetId == null) {
    ...
    const actorId = !dataset.ownerOnly || auth == null || (await auth.can('entity.list', dataset))
      ? null
      : auth.actor.get().id;
    ...
    return withEtag(serverEtag, async () => {
      const options = actorId != null
        ? QueryOptions.none.withCondition({ 'entities.creatorId': actorId })
        : QueryOptions.none;
    ...
    service.get(`${base}/attachments/:name`, endpoint((container, { params, auth }, request, response) => {
      ...
      return getInstance(Forms, params)
        ...
        .then((form) => FormAttachments.getByFormDefIdAndName(form.def.id, params.name)
          .then(getOrNotFound)
          .then(attachment => streamAttachment(container, attachment, auth, response)));
    }));
  };
