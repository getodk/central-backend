...
const streamAttachment = async (container, attachment, filterActorId, response) => {
  ...
  if (attachment.blobId == null && attachment.datasetId == null) {
    ...
    return withEtag(attachment.openRosaHash, async () => {
      const options = attachment.ownerOnly
...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const formResource = (base, getInstance) => {
    ...
    service.get(`${base}/manifest`, endpoint.openRosa(async ({ Projects, FormAttachments, Forms, env }, { auth, params, originalUrl }) => {
      ...
      const attachments = await FormAttachments.getAllByFormDefIdForOpenRosa(form.def.id, QueryOptions.none, filterActorId);
      return formManifest({
        attachments,
    ...
    service.get(`${base}/attachments/:name`, endpoint(async (container, { params, auth }, request, response) => {
      ...
      const project = await Projects.getById(form.projectId).then(getOrNotFound);
      const filterActorId = await filteredAccessActorId(auth, project);
      const attachment = await FormAttachments.getByFormDefIdAndNameForOpenRosa(form.def.id, params.name, QueryOptions.none, filterActorId)
        .then(getOrNotFound);
