...
module.exports = (service, endpoint) => {
  ...
  service.post('/projects/:projectId/forms/draft', endpoint(({ Audit, FormPartial, Key, Project }, { params, auth }, request) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => project.getFormByXmlFormId(params.id)
        ...
        .then(rejectIf(
          ...
          Problem.user.draftAlreadyExists
        ))
        .then((form) => getPartial(request, project, FormPartial, Key)
          .then((partial) => partial.createVersion(form))
          ...
          .then((updatedForm) => Audit.log(auth.actor(), 'form.update', form, { newDraftDefId: updatedForm.draftDefId })))
        .then(success))));

  service.post('/projects/:projectId/forms/draft/publish', endpoint(({ Audit, Form }, { params, auth }) =>
    Form.getByProjectAndXmlFormId(params.projectId, params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('form.update', form)
        .then(() => Promise.all([
          form.publish(),
          Audit.log(auth.actor(), 'form.publish', form, { oldDefId: form.currentDefId, newDefId: form.draftDefId })
        ])))
      .then(success)));
