...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  service.post('/projects/:projectId/forms/:xmlFormId/restore', endpoint(async ({ Forms }, { auth, params, queryOptions }) => {
    const form = await Forms.getByProjectAndNumericId(params.projectId, params.xmlFormId, Form.PublishedVersion, Form.IncludeXml, QueryOptions.none, true)
      .then(getOrNotFound);
    await auth.canOrReject('form.restore', form);
    await Forms.restore(form);

    if (!isTrue(queryOptions.argData.ignoreWarnings)) {
      const unpublishedDatasetsAndProps = await Forms.getUnpublishedDatasetsAndProps(form);
      if (unpublishedDatasetsAndProps.length) {
        throw Problem.user.formRestoreWarnings([{ warning: 'Restoring this Form will create new datasets/properties.', datasets: unpublishedDatasetsAndProps }]);
      }
    } else {
      await Forms.createAndPublishDatasetsOnRestore(form);
    }
