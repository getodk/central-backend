...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/forms', endpoint(({ Forms, Projects }, { auth, params, queryOptions }) =>
    Projects.getById(params.projectId)
      ...
      .then((project) => Forms.getByProjectId(project.id, queryOptions))));

  // non-REST openrosa endpoint for project-specialized formlist.
  service.get('/projects/:projectId/formList', endpoint.openRosa(({ Projects, env }, { auth, params, originalUrl, queryOptions }) =>
    Projects.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => (auth.actor().isEmpty() ? reject(Problem.user.openRosaAuthenticationRequired()) : project))
      .then((project) => Projects.getByAuthForOpenRosa(project.id, auth, queryOptions.allowArgs('formID')))
  ...
  // the projectId.
  const getPartial = (input, project, Keys) =>
    ...
    (
      ...
      (isExcel(input.headers['content-type']) ? Form.fromXls(input, input.headers['content-type'],
  ...
  service.post('/projects/:projectId/forms', endpoint(({ Forms, Keys, Projects }, { params, auth, query }, request) =>
    Projects.getById(params.projectId)
      ...
      .then((project) => getPartial(request, project, Keys)
        ...
        .then(() => ((request.is('*/*') === false) // false only if no request body.
          ? Forms.getByProjectAndXmlFormId(params.projectId, params.id, true)
            ...
            .then((published) => ((published.xml == null)
              ...
              : getPartial(published.xml, project, Keys)))
          : getPartial(request, project, Keys)))
  ...
  service.delete('/projects/:projectId/forms/:id/draft', endpoint(({ Audit, Form }, { params, auth }) =>
    Form.getByProjectAndXmlFormId(params.projectId, params.id)
      ...
      .then((form) => auth.canOrReject('form.update', form))
      .then(rejectIf(
        (form) => (form.currentDefId == null),
        () => Problem.user.noPublishedVersion()
      ))
      .then(rejectIf(
  ...
  service.patch('/projects/:projectId/forms/:id', endpoint(({ Forms, Audit }, { auth, params, body }) =>
