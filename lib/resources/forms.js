...
const { xmlServiceDocumentFor, jsonServiceDocumentFor, edmxFor, rowStreamToAtom } = require('../outbound/odata');
...
module.exports = (service, { Actee, Form, Audit, Submission }) => {
  ...
  // serves table data.
  service.get('/forms/:id.svc/Submissions', odataEndpoint(({ auth, params, originalUrl }) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => Submission.streamRowsByFormId(form.id)
        .then((stream) => xml(rowStreamToAtom(form, originalUrl, stream))))));
