...
const { getOrNotFound, reject, resolve, rejectIf, ignoringResult } = require('../util/promise');
...
module.exports = (service, endpoint) => {
  ...
  service.post('/projects/:projectId/forms/:id/draft', endpoint(({ Audit, Form, FormPartial, Key, Project }, { params, auth }, request) =>
    Promise.all([
      ...
      Form.getWithXmlByProjectAndXmlFormId(params.projectId, params.id).then(getOrNotFound)
    ])
      .then(([ project, form ]) => auth.canOrReject('form.update', form)
        .then(() => ((request.is('*/*') === false) // false only if no request body.
          ? ((form.def.xml == null)
            ...
            : getPartial(form.def.xml, project, FormPartial, Key))
      ...
      .then((form) => auth.canOrReject('form.update', form)
        ...
        .then(() => (isPresent(query.version)
          ...
          ? setVersion(form.def.xml, query.version)
            ...
            .then((partial) => partial.createVersion(form, false))
            // this sort of sucks but we have to refetch the form with the new def.
            .then(() => Form.getWithXmlByProjectAndXmlFormId(params.projectId, params.id, undefined, Form.DraftVersion()))
            .then(getOrNotFound)
            .then(ignoringResult((updatedForm) => Audit.log(auth.actor(), 'form.update.draft.set', updatedForm, { newDraftDefId: updatedForm.draftDefId, automated: true })))
          : form)))
