  ...
  service.get('/projects/:projectId/forms/:id/attachments', endpoint(({ FormAttachment, Project }, { params, auth, queryOptions }) =>
    Project.getById(params.projectId)
      ...
      .then((project) => auth.canOrReject('form.read', project)
        .then(() => project.getFormByXmlFormId(params.id))
        .then(getOrNotFound)
        .then((form) => FormAttachment.getAllByFormDefId(form.def.id, queryOptions)))));
    ...
    Project.getById(params.projectId)
      ...
      .then((project) => auth.canOrReject('form.update', project)
        ...
        .then((form) => Promise.all([
          ...
          FormAttachment.getByFormDefIdAndName(form.def.id, params.name)
            .then(getOrNotFound)
            .then((attachment) => Audit.log(auth.actor(), 'form.attachment.update', attachment)
              .then(() => attachment))
        ]).then(([ blob, attachment ]) => Promise.all([
          // TODO: could bail out early if there was no change in blobId.
          attachment.with({ blobId: blob.id }).update(),
          Audit.log(auth.actor(), 'form.attachment.update', attachment, { oldBlobId: attachment.blobId, newBlobId: blob.id })
        ]))
        .then(success)))));
        ...
        .then((form) => FormAttachment.getByFormDefIdAndName(form.def.id, params.name)
          ...
          .then((attachment) => ((attachment.blobId == null)
            ...
            : Promise.all([
              ...
              Audit.log(auth.actor(), 'form.attachment.update', attachment, { oldBlobId: attachment.blobId, newBlobId: null })
            ])))
          .then(success)))));
};
