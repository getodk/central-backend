...
module.exports = (service, endpoint) => {
  ...
  service.post('/projects/:projectId/forms', endpoint(({ Audit, FormPartial, Key, Project }, { params, auth, query }, request) =>
    Project.getById(params.projectId)
      ...
      .then((form) => Audit.log(auth.actor(), 'form.create', form)
        // iff ?publish=true then also log the publish action. this is for
        // both auditing reasons, and async worker reasons (so the form is
        // pushed to enketo as expected)
        .then(() => ((form.currentDefId != null)
          ? Audit.log(auth.actor(), 'form.update.publish', form, { newDefId: form.currentDefId })
          : null))
        .then(always(form)))));
  ...
  // create an Enketo preview of the form by creating a 15-minute session on behalf
  // of the authenticated actor, POSTing to Enketo's `/api/v2/survey/preview`, and
  // returning the response
  service.post('/projects/:projectId/forms/:id/preview', endpoint(({ Form, Session, enketo, env }, { params, auth }, request, response) =>
    Form.getByProjectAndXmlFormId(params.projectId, params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('form.read', form))
      .then((form) => {
        ...
        // TODO: can't happen yet but what if there is no actor?
        return Session.fromActor(auth.actor().get(), expiresAt).create()
          .then((session) => {
            const openRosaUrl = `${env.domain}/v1/projects/${params.projectId}`;
            return enketo.preview(openRosaUrl, form.xmlFormId, session.token, response);
          });
      })));
