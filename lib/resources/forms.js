...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  formResource('/projects/:projectId/forms/:xmlFormId', (Forms, params, withXml = false, options = QueryOptions.none) =>
    Forms.getByProjectAndXmlFormId(params.projectId, params.xmlFormId, Form.PublishedVersion, Form.withXml(withXml), options)
      .then(getOrNotFound)
      .then(ensureDef));
  ...
  formResource('/projects/:projectId/forms/:xmlFormId/versions/:version', (Forms, params, withXml = false, options = QueryOptions.none) =>
    Forms.getByProjectAndXmlFormId(params.projectId, params.xmlFormId, params.version, Form.withXml(withXml), options)
      .then(getOrNotFound)
      .then(ensureDef));
  service.get('/projects/:projectId/forms/:xmlFormId/versions/:version', endpoint(({ Forms }, { auth, params, queryOptions }) =>
    Forms.getByProjectAndXmlFormId(params.projectId, params.xmlFormId, params.version, Form.WithoutXml, queryOptions ?? QueryOptions.none)
      .then(getOrNotFound)
      .then(ensureDef)
      .then((form) => canReadForm(auth, form))));
