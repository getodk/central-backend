...
module.exports = (service, endpoint) => {
  ...
  // TODO: per-form read auth.
  service.get('/formList', endpoint.openRosa(({ Actee, Form, env }, { auth, originalUrl }) =>
    auth.canOrReject('list', Actee.species('form'))
  ...
  service.post('/forms', endpoint(({ Actee, Audit, Form }, { body, auth }) =>
    auth.canOrReject('create', Actee.species('form'))
  ...
  service.get('/forms/:id/manifest', endpoint.openRosa(({ Form, FormAttachment, env }, { auth, params, originalUrl }) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
  ...
  service.get('/forms/:id/attachments/:name', endpoint(({ Blob, Form, FormAttachment }, { params, auth }, _, response) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
  ...
  service.post('/forms/:id/attachments/:name', endpoint(({ all, Audit, Blob, Form, FormAttachment }, { auth, headers, params }, request) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('update', form)
        .then(() => all.do([
          Blob.fromStream(request, headers['content-type']).then((blob) => blob.create()),
          FormAttachment.getByFormAndName(form.id, params.name)
            .then(getOrNotFound)
            .then((attachment) => Audit.log(auth.actor(), 'update', attachment)
              .then(() => attachment))
