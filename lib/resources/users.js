...
module.exports = (service, endpoint) => {
  ...
  service.post('/users', endpoint(({ Users, mail, Audit }, { body, auth }) =>
    auth.canOrReject('user.create', User.species)
      ...
      .then(() => Users.withPassword(User.fromApi(body), body.password))
      .then((user) => user.forV1OnlyCopyEmailToDisplayName())
      .then(Users.create)
      .then((savedUser) => Users.provisionPasswordResetToken(savedUser)
  ...
  service.post('/users/reset/verify', endpoint(({ Actors, Users }, { body, auth }) =>
    resolve(auth.actor)
      .then(getOrNotFound)
  ...
  service.patch('/users/:id', endpoint(({ Users, mail }, { params, body, auth }) =>
    Users.getByActorId(params.id)
      ...
      .then((user) => auth.canOrReject('user.update', user.actor)
        ...
        .then((patchData) => Users.update(user.with(patchData))
  ...
  service.put('/users/:id/password', endpoint(({ Audits, Users, mail, bcrypt }, { params, body, auth }) =>
    Users.getByActorId(params.id)
      .then(getOrNotFound)
      .then((user) => auth.canOrReject('user.update', user.actor)
        .then(() => bcrypt.verify(body.old, user.password)
  ...
  service.delete('/users/:id', endpoint(({ Actors, Assignments, Sessions, Users }, { params, auth }) =>
    Users.getByActorId(params.id)
      .then(getOrNotFound)
