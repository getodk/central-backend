...
module.exports = (service, endpoint) => {
  ...
  service.post('/users', endpoint(({ User, mail }, { body, auth }) =>
    auth.canOrReject('user.create', User.species())
      ...
      .then((user) => user.provisionPasswordResetToken()
  ...
  service.put('/users/:id/password', endpoint(({ User, mail, crypto }, { params, body, auth }) =>
    User.getByActorId(params.id)
      .then(getOrNotFound)
      .then((user) => auth.canOrReject('user.update', user.actor)
  ...
  service.delete('/users/:id', endpoint(({ User, Audit, Session }, { params, auth }) =>
    User.getByActorId(params.id)
      .then(getOrNotFound)
      .then((user) => auth.canOrReject('user.delete', user.actor)
        .then(() => Promise.all([
          user.delete(),
          Session.getActiveByActor(user.actor)
            .then((sessions) => Promise.all(sessions.map((s) => s.delete()))),
