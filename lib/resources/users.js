...
module.exports = (service, { Actee, Actor, Membership, User, mail }) => {
  ...
  // TODO: paging.
  service.get('/users', endpoint(({ body, auth }) =>
  ...
  service.post('/users/reset/initiate', endpoint(({ params, body }) =>
  ...
  service.post('/users/reset/verify', endpoint(({ params, body, auth }) =>
    getOrNotFound(auth.actor())
