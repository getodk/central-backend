...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  service.get('/users/current', endpoint(async ({ Auth, Users, UserPreferences }, { auth, queryOptions }) => {
    const user = await Users.getByActorId(auth.actor.get().id).then(getOrNotFound);
