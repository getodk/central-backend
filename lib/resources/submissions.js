...
module.exports = (service, endpoint) => {
  ...
  const dataOutputs = (base, draft, getForm) => {
    ...
    const fullzip = ({ s3, Audits, ClientAudits, Keys, Forms, SubmissionAttachments, Submissions },
          ...
          .then((decryptor) => {
            ...
            return Promise.all([
              ...
              return zipStreamFromParts(
                ...
                streamBriefcaseCsvs(s3, rows, fields, form.xmlFormId, selectValues, decryptor, false, options),
                streamAttachments(s3, attachments, decryptor),
                streamClientAudits(s3, clientAudits, form, decryptor)
              );
            });
          }));

    const csv = ({ s3, Audits, Keys, Forms, Submissions },
        ...
        .then((form) => {
          ...
          return Promise.all([
            ...
            .then(([fields, rows, selectValues, decryptor]) => {
              ...
              return envelope(streamBriefcaseCsvs(s3, rows, fields, form.xmlFormId, selectValues, decryptor, rootOnly, options));
            });
        });
