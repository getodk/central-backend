...
module.exports = (service, endpoint) => {
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), endpoint.openRosa(async ({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, headers }) => {
      const partial = await Submission.fromXml(findMultipart(files).buffer);
      let form;
      try {
        form = await getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
      } catch(err) {
        await forceAuthFailed(err); // throws
      }
      const deprecatedId = partial.deprecatedId.orNull();
      if (deprecatedId != null) {
        const def = await Submissions.getAnyDefByFormAndInstanceId(form.id, deprecatedId, draft)
        const deprecated = await getOrReject(Problem.user.deprecatedIdNotFound({ deprecatedId }))(def);
        if (deprecated.current !== true) {
          // new submissions not allowed for closed forms
          ...
          const maybeUsurper = await Submissions.getAnyDefByFormAndInstanceId(form.id, partial.instanceId, draft)
          const usurper = maybeUsurper.orNull();
          if (usurper?.current !== true)
          ...
          SubmissionAttachments.upsert(usurper, files);
        } else {
          // SUBMISSION EDIT REQUEST: find the now-deprecated submission and supplant it.
          const [ saved, binaryFields, deprecatedAtts ] = await Promise.all([
        ...
        const maybeExtant = await Submissions.getAnyDefByFormAndInstanceId(form.id, partial.instanceId, draft);
        ...
        if (extant != null) {
          ...
          SubmissionAttachments.upsert(extant, files);
        } else {
          ...
          const [ saved, binaryFields ] = await Promise.all([
            Submissions.createNew(partial, form, query.deviceID, headers['user-agent']),
            Forms.getBinaryFields(form.def.id)
          ]);
          SubmissionAttachments.create(saved, form, binaryFields, files);
        }
      }
      return always(createdMessage({ message: 'full submission upload was successful!' }));
    }));
  };
  ...
  service.patch('/projects/:projectId/forms/:formId/submissions/:instanceId', endpoint(({ Forms, Submissions }, { params, auth, body }) =>
    Forms.getByProjectAndXmlFormId(params.projectId, params.formId, false)
      .then(getOrNotFound)
      .then(auth.canOrReject('submission.update'))
      .then((form) => Submissions.getByIds(params.projectId, params.formId, params.instanceId, false)
        .then(getOrNotFound)
