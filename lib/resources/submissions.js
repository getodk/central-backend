...
module.exports = (service) => {
  ...
  // Nonstandard REST; OpenRosa-specific API.
  service.post('/submission', multipart.any(), openRosaEndpoint(({ all, Audit, Form, Submission }, { files, auth }) =>
  ...
  service.get('/forms/:id/submissions.csv.zip', endpoint(({ all, Form, Submission }, { params, auth }, response) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
  ...
  service.get('/forms/:formId/submissions/:instanceId/attachments',
  ...
  service.get('/forms/:formId/submissions/:instanceId/attachments/:name',
    endpoint(({ Blob, Form, Submission, SubmissionAttachment }, { params, auth }, response) =>
    Form.getByXmlFormId(params.formId)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
