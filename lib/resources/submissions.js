  ...
  const dataOutputs = (base, draft, getForm) => {
    ...
    service.get(`${base}/:instanceId/attachments/:name`, endpoint(({ Forms, Submissions, SubmissionAttachments }, context) =>
      getOrRedirect(Forms, Submissions, context)
        ...
        .then(getOrNotFound)
        .then(blobOrS3Redirect(context.params.name))));
    ...
    service.get(`${base}/:rootId/versions/:instanceId/attachments/:name`, endpoint(({ Forms, Submissions, SubmissionAttachments }, { params, auth }) =>
      getForm(params, Forms)
        ...
        .then(([ blob ]) => blobOrS3Redirect(params.name, blob))));
...
function blobOrS3Redirect(filename, blob) {
  console.log('blobOrS3Redirect', filename, blob);
  if (arguments.length === 1) {
    return blob => blobOrS3Redirect(filename, blob);
  }

  // FIXME just testing
  blob.s3 = 'objectname';
  console.log('Set blob.s3 to:', blob.s3);

  if (blob.s3 || true) {
    // TODO move s3 URL creation to a utility function?
    const s3url = 'http://example.com/bucket/' + blob.sha;
    console.log('blobOrS3Redirect()', 'returning redirect to:', s3url);
    return redirect(307, s3url);
  }

  return binary(blob.contentType, filename, blob.content);
}
