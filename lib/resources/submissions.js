...
module.exports = (service, { Audit, Submission, Form }) => {
  ...
  service.head('/submission', (_, response) =>
    response.status(204).send({ message: 'You should use POST to submit to /submission.' }));
  ...
  service.post('/submission', multipart.single('xml_submission_file'), openRosaEndpoint(({ file, body, auth }) =>
    ...
    Submission.fromXml((file != null) ? readFileSync(file.path).toString() : body)
