        ...
        .then((form) => Keys.getDecryptor(passphrases)
          .then((decryptor) => {
            ...
            return Promise.all([
              ...
              // Don't know the reason why streamForExport can't be added to Promise.All
              // + Submissions.streamForExport has to be the last one
              return SubmissionAttachments.streamForExport(form.id, draft, keys, options)
                .then(attachmentTransformer => {
                  const attachmentsPart = streamAttachments(attachmentTransformer, decryptor);
                  return ClientAudits.streamForExport(form.id, draft, keys, options)
                    .then(clientAuditTransformer => {
                      const clientAuditsPart = streamClientAudits(clientAuditTransformer, form, decryptor);
                      return Submissions.streamForExport(form.id, draft, keys, options)
                        .then(submissionTransformer => {
                          const submissionsPart = streamBriefcaseCsvs(submissionTransformer, fields, form.xmlFormId, selectValues, decryptor, false, options);
                          return zipStreamFromParts(submissionsPart, attachmentsPart, clientAuditsPart);
                        });
                    });
                });
            });
      ...
      getForm(params, Forms)
        ...
        .then((form) => {
          ...
          return Promise.all([
            ...
            .then(([ fields, selectValues, decryptor, , rows ]) => {
              const filename = sanitize(form.xmlFormId);
