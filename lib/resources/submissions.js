...
const { always, map, identity } = require('ramda');
...
module.exports = (service, endpoint) => {
  ...
  const dataOutputs = (base, draft, getForm) => {
    ...
    const fullzip = ({ ClientAudit, Key, Form, SubmissionAttachment, SubmissionDef },
      auth, params, passphrases, response) =>
      getForm(params, Form)
        .then((form) => auth.canOrReject('submission.read', form))
        .then((form) => Promise.all([
          form.def.getFields(),
          SubmissionDef.streamForExport(form.id, draft, Object.keys(passphrases)),
          ...
          Key.getDecryptor(map(decodeURIComponent, passphrases)) // will do nothing if {}
    ...
    const csv = ({ Key, Form, SubmissionDef }, auth, params, passphrases, response, rootOnly) =>
      getForm(params, Form)
        .then((form) => auth.canOrReject('submission.read', form))
        .then((form) => Promise.all([
          form.def.getFields(),
          ...
          const filename = sanitize(form.xmlFormId);
          response.append('Content-Disposition', `attachment; filename="${filename}.csv"`);
    ...
    service.get(`${base}.csv.zip`, endpoint((container, { params, auth, query }, _, response) =>
      select(query)(container, auth, params, getPassphrases(query), response)));
    service.post(`${base}.csv.zip`, formParser, endpoint((container, { params, auth, body, query }, _, response) =>
      select(query)(container, auth, params, getPassphrases(body), response)));

    service.get(`${base}.csv`, endpoint((container, { params, auth, query }, _, response) =>
      csv(container, auth, params, getPassphrases(query), response, true)));
    service.post(`${base}.csv`, endpoint((container, { params, auth, body }, _, response) =>
      csv(container, auth, params, getPassphrases(body), response, true)));
    ...
    service.get(`${base}/submitters`, endpoint(({ Form }, { params, auth }) =>
      getForm(params, Form)
        .then((form) => auth.canOrReject('submission.read', form))
        .then((form) => form.getAllSubmitters())));
