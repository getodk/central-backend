...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const openRosaSubmission = (path, endpointToUse, draft, getForm) => {
    ...
    service.get(path, endpointToUse.openRosa(({ Projects, Forms }, { params, auth }) =>
    ...
    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), multerUtf, multipartErrorHandler, endpointToUse.openRosa(({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, userAgent }) =>
