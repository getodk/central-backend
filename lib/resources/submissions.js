...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/forms/:id/submissions.csv.zip', endpoint(({ Project, SubmissionAttachment, SubmissionDef }, { params, auth }, _, response) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('submission.read', project)
        ...
        .then((form) => Promise.all([
          SubmissionDef.streamForExport(form.id),
  ...
  service.post(
    '/projects/:projectId/forms/:formId/submissions/:instanceId/attachments/:name',
    endpoint(({ Blob, Project, SubmissionDef }, { params, headers, auth }, request) =>
      Promise.all([
        Project.getById(params.projectId)
          .then(getOrNotFound)
        ...
        .then(([ def, blob ]) => def.attach(params.name, blob))
        ...
        .then(([ form, def ]) =>
          SubmissionAttachment.getBySubmissionDefIdAndName(def.id, params.name)
            ...
            .then((attachment) => Promise.all([
              attachment.clear(),
              Audit.log(auth.actor(), 'submission.attachment.update', form, { instanceId: params.instanceId, submissionDefId: def.id, name: attachment.name, oldBlobId: attachment.blobId })
            ])))
        .then(success))
  );
};
