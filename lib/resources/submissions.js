          ...
          .then((decryptor) => {
            ...
            return Promise.all([
              ...
              return zipStreamFromParts(
                () => streamBriefcaseCsvs(Submissions.streamForExport(form.id, draft, keys, options),
                  fields, form.xmlFormId, selectValues, decryptor, false, options),
                () => streamAttachments(SubmissionAttachments.streamForExport(form.id, draft, keys, options),
                  decryptor),
                () => streamClientAudits(ClientAudits.streamForExport(form.id, draft, keys, options),
                  form, decryptor)
              );
            });
          }));
        ...
        .then((form) => {
          ...
          return Promise.all([
            ...
            .then(([ fields, selectValues, decryptor ]) => {
              ...
              return envelope(() => streamBriefcaseCsvs(
                Submissions.streamForExport(form.id, draft, Object.keys(passphrases), options),
                fields, form.xmlFormId, selectValues, decryptor, rootOnly, options
              ));
            });
        });
