...
module.exports = (service, endpoint) => {
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    service.get(path, endpoint.openRosa(({ Project }, { params }) =>
      Project.getById(params.projectId)
        .then(getOrNotFound)
        .then(always({ code: 204, body: '' }))));

    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), endpoint.openRosa(({ Audit, Form, Submission, SubmissionAttachment, SubmissionPartial, submissionDefs }, { params, files, auth, query }) =>
        ...
        .then((partial) => getForm(auth, params, partial.xmlFormId, Form, partial.version)
          ...
          .then((form) => Promise.all([
            ...
            .then(([ maybeExtant, binaryFields ]) => maybeExtant
              ...
              .map((extant) => submissionDefs.getCurrentBySubmissionId(extant.id) // TODO/SL: temp subst // TODO: save this extra request?
