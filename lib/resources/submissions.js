...
const { Form, Submission } = require('../model/frames');
const { createdMessage } = require('../outbound/openrosa');
const { getOrNotFound, resolve, getOrReject, rejectIf, reject } = require('../util/promise');
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    service.post(path, multipart.any(), endpoint.openRosa(({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query }) =>
      ...
      resolve(Option.of(files).map((xs) => xs.find((file) => file.fieldname === 'xml_submission_file')))
        ...
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.version)
          ...
          .then((form) => Promise.all([
            Submissions.getById(form.id, partial.instanceId, draft, QueryOptions.extended),
            ...
            .then(([ maybeExtant, binaryFields ]) => maybeExtant
              ...
              .map((extant) => Submissions.getCurrentBySubmissionId(extant.id) // TODO: save this extra request?
                ...
                .then((extantVersion) => ((Buffer.compare(Buffer.from(extantVersion.xml), Buffer.from(partial.xml)) !== 0)
                  ...
                  : SubmissionAttachments.upsert(extantVersion, form, files))))
              // otherwise, this is the first POST for this submission. create the
              // submission and the expected attachments:
              .orElseGet(() => Submissions.createNew(partial, form, query.deviceID)
                .then((submission) => SubmissionAttachments.create(submission, form, binaryFields, files)))
  ...
  const restSubmission = (path, getForm) => {
    service.post(path, endpoint(({ Forms, Submissions, SubmissionAttachments }, { params, auth }, request) =>
      Submission.fromXml(request)
        .then((partial) => getForm(params, Forms, partial.version)
          .then((form) => auth.canOrReject('submission.create', form))
          .then((form) => {
