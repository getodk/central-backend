...
const { success, xml, isFalse, binary, redirect } = require('../util/http');
...
module.exports = (service, endpoint) => {
  ...
  const restSubmission = (base, draft, getForm) => {
    service.post(`${base}/submissions`, endpoint(({ Forms, Submissions, SubmissionAttachments }, { params, auth }, request) =>
  ...
  const dataOutputs = (base, draft, getForm) => {
          ...
          maybeSub.map(resolve).orElseGet(() =>
            Submissions.getRootForInstanceId(form.id, params.instanceId, draft)
              ...
              .then((rootId) => redirect(originalUrl.replace(
                ...
                `/submissions/${rootId}/versions/${params.instanceId}`
              ))))
        ]));
    ...
    service.get(`${base}/:instanceId/attachments`, endpoint(({ Forms, Submissions, SubmissionAttachments }, context) =>
      getOrRedirect(Forms, Submissions, context)
        .then(([ , submission ]) => SubmissionAttachments.getCurrentForSubmissionId(submission.id, draft))));

    service.get(`${base}/:instanceId/attachments/:name`, endpoint(({ Forms, Submissions, SubmissionAttachments }, context) =>
      getOrRedirect(Forms, Submissions, context)
        .then(([ , submission ]) => SubmissionAttachments.getCurrentBlobByIds(submission.id, context.params.name, draft))
        .then(getOrNotFound)
