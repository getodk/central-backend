...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const dataOutputs = (base, draft, getForm) => {
    ...
    service.get(`${base}/:instanceId.geojson`, endpoint(async ({ Forms, Submissions, GeoExtracts }, context) => {
      const [form, submission] = await getOrRedirect(Forms, Submissions, context); // will 403/404 if necessary
      return GeoExtracts.getSubmissionFeatureCollectionGeoJson(
        form.id,
        [submission.instanceId],
        Sanitize.queryParamToArray(context.query.fieldpath),
      );
    }));
    ...
    service.get(`${base}/:rootId/versions/:instanceId.geojson`, endpoint.plain(async ({ Forms, GeoExtracts }, { params, auth }) => {
      const form = await getForm(params, Forms)
        .then(auth.canOrReject('submission.read'));
      return GeoExtracts.getSubmissionFeatureCollectionGeoJson(form.id, [params.instanceId], null, null, null, null, false, null, false);
    }));
