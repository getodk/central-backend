...
module.exports = (service, endpoint) => {
    ...
    Promise.all([
      ...
      .then(([ form, partial ]) => { // this syntax sucks but.. the alternatives do too.
        if (partial.xmlFormId !== params.formId)
          return Problem.user.unexpectedValue({ field: 'form id', value: partial.xmlFormId, reason: 'did not match the form ID in the URL' });
        if (!form.acceptsSubmissions())
          return Problem.user.notAcceptingSubmissions();
  ...
  const csvzip = ({ Key, Form, SubmissionAttachment, SubmissionDef },
