    ...
    endpoint(({ Project, Submission }, { params, auth }) =>
      Project.getById(params.projectId)
        ...
        .then((project) => auth.canOrReject('submission.read', project)
          ...
          .then((submission) => submission.getCurrentVersion())
          .then(getOrNotFound)
          .then((submissionVersion) => submissionVersion.getAttachmentMetadata())))
  );
    ...
    endpoint(({ Blob, Project, Submission, SubmissionAttachment }, { params, auth }, _, response) =>
      Project.getById(params.projectId)
        ...
        .then((project) => auth.canOrReject('submission.read', project)
          ...
          .then((submission) => submission.getCurrentVersion())
          .then(getOrNotFound)
          .then((version) => SubmissionAttachment.getBySubVersionId(version.id, params.name))
          .then(getOrNotFound)
  ...
  service.post(
    ...
    endpoint(({ Blob, Project, Submission }, { params, headers, auth }, request) =>
      Promise.all([
        ...
        .then(([ submission, blob ]) => submission.getCurrentVersion()
          .then(getOrNotFound)
          .then((version) => version.attach(params.name, blob)))
