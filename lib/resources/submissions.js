...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/forms/:id/submissions.csv.zip', endpoint(({ Key, Project, SubmissionAttachment, SubmissionDef }, { params, auth, query }, _, response) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('submission.read', project)
        ...
        .then((form) => Promise.all([
          SubmissionDef.streamForExport(form.id, (Object.keys(query).length > 0)), // join decrypts iff decrypting
          SubmissionAttachment.streamByFormId(form.id),
          Key.getPrivates(map(decodeURIComponent, query)) // will do nothing if no passphrases
        ]).then(([ rows, attachments, privs ]) => {
          ...
          return streamBriefcaseCsvs(rows, form, privs).then((csvStream) =>
