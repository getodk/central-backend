...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const openRosaSubmission = (path, _endpoint, draft, getForm) => {
    ...
    service.post(path, multipart.any(), multerUtf, multipartErrorHandler, _endpoint.openRosa(({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, userAgent, headers }) => {
      throw new Error('I am manufactured deliberately 1');
  ...
  const restSubmission = (base, draft, getForm) => {
    throw new Error('I am manufactured deliberately 2');
    // eslint-disable-next-line no-unreachable
    service.post(`${base}/submissions`, endpoint(({ Forms, Submissions, SubmissionAttachments }, { params, auth, query, userAgent, headers }, request) =>
