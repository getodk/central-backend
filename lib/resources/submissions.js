...
module.exports = (service, endpoint) => {
  ...
  service.get('/submission', endpoint((_, context, request, response) => {
    response.status(204);
    return false; // TODO: we need to return something non-null but this is a lousy choice.
  }));

  // Nonstandard REST; OpenRosa-specific API.
  service.post('/submission', multipart.any(), endpoint.openRosa(({ all, Audit, Form, Submission }, { files, auth }) =>
  ...
  service.post('/forms/:id/submissions', endpoint(({ Audit, Form, Submission }, { params, body, auth }) =>
  ...
  service.get('/forms/:id/submissions.csv.zip', endpoint(({ all, Form, Submission }, { params, auth }, _, response) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
  ...
  // TODO: paging.
  service.get('/forms/:id/submissions', endpoint(({ Form, Submission }, { params, auth, queryOptions }) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
  ...
  service.get(
    '/forms/:formId/submissions/:instanceId/attachments',
    endpoint(({ Form, Submission }, { params, auth }) =>
      Form.getByXmlFormId(params.formId)
        .then(getOrNotFound)
        .then((form) => auth.canOrReject('read', form)
  ...
  service.get(
    '/forms/:formId/submissions/:instanceId/attachments/:name',
    endpoint(({ Blob, Form, Submission, SubmissionAttachment }, { params, auth }, _, response) =>
      Form.getByXmlFormId(params.formId)
        .then(getOrNotFound)
        .then((form) => auth.canOrReject('read', form)
