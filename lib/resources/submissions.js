...
module.exports = (service, { Attachment, Audit, Blob, Form, Submission, all }) => {
    ...
    Form.getByXmlFormId(params.id)
      ...
      .then((form) => auth.canOrReject('read', form)
        .then(() => all.do([
          ...
          return zipStreamFromParts(streamBriefcaseCsvs(rows, form), streamAttachments(attachments));
        })))));

  // TODO: paging.
  service.get('/forms/:id/submissions', endpoint(({ params, auth, extended }) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
        .then(() => Submission.getAllByFormId(form.id, extended)))));

  service.get('/forms/:formId/submissions/:instanceId', endpoint(({ params, auth, extended }) =>
    Form.getByXmlFormId(params.formId)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
        .then(() => Submission.getById(form.id, params.instanceId, extended))
        .then(getOrNotFound))));
  ...
  service.get('/forms/:formId/submissions/:instanceId/attachments/:name', endpoint(({ params, auth }, response) =>
    Form.getByXmlFormId(params.formId)
      ...
      .then((form) => auth.canOrReject('read', form)
        ...
        .then(() => Submission.getById(form.id, params.instanceId))
        .then(getOrNotFound)
        .then((submission) => Attachment.getBySubmission(submission.id, params.name))
        .then(getOrNotFound)
