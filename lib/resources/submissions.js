        ...
        .then(([ form, def ]) =>
          SubmissionAttachment.getBySubmissionDefIdAndName(def.id, params.name)
            ...
            .then((attachment) => Promise.all([
              attachment.clear(),
              Audit.log(auth.actor(), 'submission.attachment.clear', form, { name: attachment.name, blobId: attachment.blobId, instanceId: params.instanceId, submissionDefId: def.id })
            ])))
        .then(success))
  );
};
