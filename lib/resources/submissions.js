...
const { success, xml, isFalse, isTrue, contentDisposition, redirect, url, withEtag } = require('../util/http');
        ...
        .then(([ form, maybeSub ]) => Promise.all([
          ...
          maybeSub.map(resolve).orElseGet(() =>
            Submissions.getRootForInstanceId(form.id, params.instanceId, draft)
              .then(getOrNotFound)
              .then((rootId) => {
                const originalUrlNormalized = originalUrl.replace(encodeURIComponent(params.instanceId), params.instanceId);
                redirect(originalUrlNormalized.replace(
