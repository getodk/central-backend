...
const backup = (keys, response) => {
  ...
  const dump = (tmpdir) => pgdump(tmpdir.path).then(() => {
    // halt the keepalive.
    release();

    // TODO: mostly copypasta for now from lib/task/fs:
    promisify(readdir)(tmpdir.path).then((files) => {
  ...
  tmp.withDir(dump, { unsafeCleanup: true });
...
module.exports = (service, endpoint) => {
  service.get('/backup', endpoint(({ Configs }, { auth }, _, response) =>
