...
module.exports = (service, endpoint) => {
  ...
  service.put('/user-preferences/project/:projectId/:propertyName', endpoint(({ UserPreferences }, { body, auth, params }) => {
    checkBody(body);
    return UserPreferences.writeProjectProperty(auth.actor.get(), params.projectId, params.propertyName, body.propertyValue)
      .then(success);
  }));

  service.delete('/user-preferences/project/:projectId/:propertyName', endpoint(({ UserPreferences }, { auth, params }) =>
    UserPreferences.removeProjectProperty(auth.actor.get(), params.projectId, params.propertyName)
      .then(getOrNotFound)));
