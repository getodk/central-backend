...
module.exports = (service, endpoint) => {
  ...
  //////////////
  // Per-project
  service.put('/user-preferences/project/:projectId/:propertyName', endpoint(({ Projects, UserPreferences }, { body, auth, params }) => {
    checkBody(body);
    return Projects.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => UserPreferences.writeProjectProperty(auth.actor.get(), project.id, params.propertyName, body.propertyValue))
      .then(success);
  }));

  service.delete('/user-preferences/project/:projectId/:propertyName', endpoint(({ Projects, UserPreferences }, { auth, params }) =>
    Projects.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => UserPreferences.removeProjectProperty(auth.actor.get(), project.id, params.propertyName))
      .then(getOrNotFound)));
  ...
  service.put('/user-preferences/site/:propertyName', endpoint(({ UserPreferences }, { body, auth, params }) => {
    checkBody(body);
    return UserPreferences.writeSiteProperty(auth.actor.get(), params.propertyName, body.propertyValue)
      .then(success);
  }));

  service.delete('/user-preferences/site/:propertyName', endpoint(({ UserPreferences }, { auth, params }) =>
    UserPreferences.removeSiteProperty(auth.actor.get(), params.propertyName)
      .then(getOrNotFound)));
};
