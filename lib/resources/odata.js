...
module.exports = (service) => {
  ...
  service.get('/forms/:id.svc', odataEndpoint.json(({ Form, env }, { auth, params, originalUrl }) =>
  ...
  service.get('/forms/:id.svc/([$])metadata', odataEndpoint.xml(({ Form }, { auth, params }) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
  ...
  service.get(/^\/forms\/([a-z0-9-_]+).svc\/Submissions\((?:'|%27)((?:uuid:)?[a-z0-9-]+)(?:'|%27)\)(\/.*)*$/i,
    odataEndpoint.json(({ Form, Submission, env }, { auth, params, query, originalUrl }) =>
  ...
  // serves table data.
  service.get('/forms/:id.svc/:table',
    odataEndpoint.json(({ all, Form, Submission, env }, { auth, params, originalUrl, query }) =>
    Form.getByXmlFormId(params.id)
      .then(getOrNotFound)
      .then((form) => auth.canOrReject('read', form)
