...
module.exports = (service, endpoint) => {
  ...
  const odataResource = (base, getForm) => {
    // serves a service document comprising the primary dataset and any implicit
    // subtables created via repeats (section 11.1.1).
    service.get(base, endpoint.odata.json(({ Project, env }, { auth, params, originalUrl }) =>
      getForm(Project, auth, params)
    ...
    service.get(`${base}/([$])metadata`, endpoint.odata.xml(({ Project }, { auth, params }) =>
      getForm(Project, auth, params)
        .then((form) => form.def.getFields()
    ...
    const singleRecord = endpoint.odata.json(({ Project, SubmissionDef, env }, { auth, params, query, originalUrl }) =>
      getForm(Project, auth, params)
        .then((form) => Promise.all([
          form.def.getFields(),
          SubmissionDef.getForExport(form.id, getUuid(params.uuid), false).then(getOrNotFound)
        ])
    ...
    // serves table data.
    service.get(`${base}/:table`, endpoint.odata.json(({ Project, Submission, SubmissionDef, env }, { auth, params, originalUrl, query }) =>
      getForm(Project, auth, params)
        .then((form) => {
  ...
  odataResource('/projects/:projectId/forms/:id.svc', (Project, auth, params) =>
