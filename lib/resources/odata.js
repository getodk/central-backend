...
module.exports = (service, endpoint) => {
  ...
  service.get(singleRowRegex, endpoint.odata.json(({ Project, Submission, env }, { auth, params, query, originalUrl }) =>
    Project.getById(params[0]) // first regexp match
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('form.read', project)
        .then(() => project.getFormByXmlFormId(params[1])) // second match
        .then(getOrNotFound)
        .then((form) => Submission.getById(form.id, params[2], QueryOptions.extended)
          .then(getOrNotFound)
          .then((submission) => singleRowToOData(form, submission, env.domain, decodeURI(originalUrl), query))))));

  // serves table data.
  service.get('/projects/:projectId/forms/:id.svc/:table', endpoint.odata.json(({ Project, Submission, env }, { auth, params, originalUrl, query }) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('form.read', project)
        .then(() => project.getFormByXmlFormId(params.id))
