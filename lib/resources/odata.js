...
module.exports = (service, endpoint) => {
  ...
  service.get('/projects/:projectId/forms/:id.svc', endpoint.odata.json(({ Project, env }, { auth, params, originalUrl }) =>
    Project.getById(params.projectId)
      ...
      .then((project) => auth.canOrReject('form.read', project)
        ...
        .then(getOrNotFound)
        .then((form) => serviceDocumentFor(form.xform, env.domain, originalUrl))
        .then((doc) => contentType('application/json; odata.metadata=minimal')(doc)))));
  ...
  service.get(singleRowRegex, endpoint.odata.json(({ Project, SubmissionVersion, env }, { auth, params, query, originalUrl }) =>
    Project.getById(params[0]) // first regexp match
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('form.read', project)
        .then(() => project.getFormByXmlFormId(params[1])) // second match
        .then(getOrNotFound)
        .then((form) => SubmissionVersion.getForExport(form.id, params[2])
          .then(getOrNotFound)
          .then((row) => singleRowToOData(form, row, env.domain, decodeURI(originalUrl), query))))));

  // serves table data.
  service.get('/projects/:projectId/forms/:id.svc/:table', endpoint.odata.json(({ Project, Submission, SubmissionVersion, env }, { auth, params, originalUrl, query }) =>
    Project.getById(params.projectId)
      .then(getOrNotFound)
      .then((project) => auth.canOrReject('form.read', project)
        .then(() => project.getFormByXmlFormId(params.id))
        ...
        .then((form) => {
          ...
          return Promise.all([
            SubmissionVersion.streamForExport(form.id, options),
