  ...
  service.get('/forms/:id.svc', odataEndpoint.json(({ all, auth, params, originalUrl }) =>
    Form.getByXmlFormId(params.id)
      ...
      .then((form) => auth.canOrReject('read', form)
        .then(() => contentType('application/json; odata.metadata=minimal')(serviceDocumentFor(form, originalUrl))))));
    ...
    Form.getByXmlFormId(params[0]) // first regexp match
      ...
      .then((form) => auth.canOrReject('read', form)
        .then(() => Submission.getById(form.id, params[1])
          .then(getOrNotFound)
          .then((submission) => json(singleRowToOData(form, submission, originalUrl, query)))))));
    ...
    Form.getByXmlFormId(params.id)
      ...
      .then((form) => auth.canOrReject('read', form)
        .then(() => all.do([
          ...
          .then(([ stream, count ]) => json(rowStreamToOData(form, params.table, originalUrl, query, stream, count)))))));
};
