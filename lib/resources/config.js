...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const getConfig = async ({ s3, Blobs, Configs }, key) => {
    ...
    return maskedBlobResponse(s3, blob);
  };

  service.get('/config/public/:key', anonymousEndpoint(async (container, { params }, request, response) => {
    const { key } = params;
    const frame = Config.forKey(key);

    if (!frame.isPublic) return Problem.user.insufficientRights();

    setCachePolicy(response, request.query.ts ? cachePolicy.public.immutable : cachePolicy.public.cacheButRevalidate);

    return getConfig(container, key);
  }));
