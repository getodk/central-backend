...
const { isBlank, isPresent, printPairs } = require('../util/util');
...
module.exports = (service, endpoint) => {
  ...
  service.post('/config/analytics', endpoint(({ Configs }, { auth, body }) =>
    auth.canOrReject('config.set', Config.species)
      .then(() => {
        const value = { enabled: !!body.enabled };
        if (value.enabled) {
          if (isPresent(body.email)) value.email = body.email.toString();
          if (isPresent(body.organization))
            value.organization = body.organization.toString();
        }
        return Configs.set('analytics', value);
      })));

  const checkKey = (key) => () => {
    if (reservedKeys.some((k) => key === k || key.startsWith(`${k}.`)))
      throw Problem.user.unexpectedValue({
        field: 'key',
        value: key,
        reason: 'the key is reserved for internal use.'
      });
  };

  service.get('/config/:key', endpoint(({ Configs }, { auth, params }) =>
    auth.canOrReject('config.read', Config.species)
      .then(checkKey(params.key))
