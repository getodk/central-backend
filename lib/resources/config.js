...
module.exports = (service, endpoint) => {
  ...
  service.get('/config/backups', endpoint(({ all, Audit, Config }, { auth }) =>
    auth.canOrReject('read', Config.species()) // TODO: goofy.
  ...
  service.delete('/config/backups', endpoint(({ Config }, { auth }) =>
    auth.canOrReject('terminateBackup', Config.species()) // TODO: maybe goofy.
  ...
  service.post('/config/backups/initiate', endpoint(({ Actor, Config, Session, google }, { auth, body }) =>
    auth.canOrReject('createBackup', Config.species()) // TODO: maybe goofy
      ...
      .then((keys) => {
        ...
        return (new Actor({ type: Actor.types().singleUse, expiresAt, displayName, meta }))
          .create()
          .then((actor) => actor.grant('backup.create', Config.species())
  ...
  service.post('/config/backups/verify', endpoint(({ all, Config, google }, { auth, body }) =>
    auth.canOrReject('backup.create', Config.species())
      .then(() => auth.actor())
