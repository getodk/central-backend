...
module.exports = (service) => {
  ...
  service.get('/config/backups', endpoint(({ all, Actee, Audit, Config }, { auth }) =>
  ...
  service.delete('/config/backups', endpoint(({ Actee, Config }, { auth }) =>
  ...
  service.post('/config/backups/initiate', endpoint(({ Actee, Actor, Session, google }, { auth, body }) =>
  ...
  service.post('/config/backups/verify', endpoint(({ all, Actee, Config, google }, { auth, body }) =>
    auth.canOrReject('createBackup', Actee.species('config'))
      ...
      .then((actor) => (!isBlank(body.code)
        ? promisify(google.auth.getToken)(body.code)
